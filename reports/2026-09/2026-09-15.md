...
### 1. Route Me If You Can: A Benchmark for Query Reformulation Selection

- **LLM Score**: 0
- **Keyword Score**: 22
- **Authors**: Hai Son Le, Negar Arabzadeh, Amin Bigdeli, Radin Hamidi Rad, Sajad Ebrahimi, Charles L. A. Clarke, Ebrahim Bagheri
- **URL**: <http://arxiv.org/abs/2609.14885v1>
- **Submitted**: 2026-09-14 01:06:41
- **Topic Keywords**: retriever, query, queries, ranking, relevance, retrieval, rank, acl, trec
- **Reason**: LLM scoring failed.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: QueryRoute: a frozen benchmark for per‑query query‑reformulation selection
- **Aim**: To formalise the problem of choosing the best LLM‑generated query variant at inference time, provide a reproducible dataset of all candidate outcomes, and establish a rigorous evaluation framework for selectors.
- **Rationale**: No single reformulation strategy dominates across queries, domains, retrievers, or LLM backbones, so inference‑time routing is essential. Existing work lacks a shared action‑outcome matrix, making fair comparison difficult.
- **Ground**: 619,905 retrieval outcomes from 3,757 TREC DL/BEIR/BRIGHT queries, 11 reformulation systems (including the original), 5 LLM backbones, 3 retrievers. All ranked lists, scores, per‑query oracle labels, and intermediate features are publicly released.
- **Experiment**: Baseline selectors evaluated: Original, Best‑single, Random, Oracle, supervised classifiers, router‑based selectors (Similarity‑Weighted, Matrix‑Factorization), QPP selectors (Pre‑, Post‑, Neural‑QPP), outcome‑level judges (e.g., DeepSeek‑V3). Metrics: selection accuracy, near‑oracle rate, regret, oracle‑gap closure, help/hurt rate, pairwise accuracy, rank correlation, selection entropy. Findings: Oracle headroom >0.1 nDCG; selectors recover only a fraction; performance varies by retriever, dataset, and metric; domain sensitivity and ranking vs query‑level trade‑offs observed.
- **Takeaway**: QueryRoute supplies the first frozen, reproducible benchmark for per‑query reformulation selection, revealing substantial oracle headroom and highlighting the limitations of current selectors. It enables systematic comparison of routing strategies and underscores the need for per‑query, domain‑aware selection mechanisms.
...
### 2. Top-K Is Not a Budget for Hybrid Retrieval

- **LLM Score**: 0
- **Keyword Score**: 17
- **Authors**: Chunran Zhang
- **URL**: <http://arxiv.org/abs/2609.15143v1>
- **Submitted**: 2026-09-14 07:18:40
...
#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: DiBud: Budget-Constrained Exact-Prefix Fusion for Hybrid Retrieval in Retrieval-Augmented Generation
- **Aim**: Produce a certified prefix of the full Reciprocal Rank Fusion (RRF) ranking within a user-specified access budget B, guaranteeing that each emitted document truly precedes all remaining documents (seen or unseen) without reading the entire dense and sparse result lists.
- **Rationale**: Fixed truncation depth L in standard RRF truncates cross‑channel contributions, while completing an exact Top‑K list leads to highly variable access costs (long‑tailed distribution) unsuitable for latency‑sensitive Retrieval‑Augmented Generation pipelines. An explicit budgeted approach can bound latency while preserving exactness.
- **Ground**: Hybrid retrieval combines a dense neural retriever (bge‑small‑en‑v1.5) and a sparse lexical retriever (BM25).  RRF score: F(x)=∑_{i∈{D,S}}1/(c+r_i(x)) with c=59.  Full ranking T* sorted by F(x).  Certification uses lower bound ℓ(x)=∑_{i:x∈S_i}g(r_i(x)) and upper bound h(x)=ℓ(x)+∑_{i:x∉S_i}u_i, where u_i=g(d_i+1) if channel i not exhausted.  Unseen bound h_∅=u_D+u_S.  A document is certified if ℓ(x) exceeds all remaining competitors’ upper bounds (strict for unseen).  Budget constraint: d_D+d_S≤B.  Reading strategy: advance channel that resolves the highest blocker or yields greatest reduction in unread bound; exhausted channels skipped; ties broken by dense-first order.
- **Experiment**: Datasets: 770 queries from TREC‑DL 2019/2020, NFCorpus, SciFact, TREC‑COVID (30 queries × 5 snapshots).  Retrieval models: dense bge‑small‑en‑v1.5 (exact rankings) and sparse BM25 (k1=0.9, b=0.4).  Fusion: equal‑weight RRF with g(r)=1/(59+r).  Algorithms compared: DiBud vs Balanced (alternating one‑by‑one reads) using same certification rules.  Budgets ranged 128–10 000 accesses.  Metrics: certified results up to 20 and 100 positions, reference exact Top‑20.  Evaluation modes: static (reconstructed full rankings) and temporal (frozen snapshot rankings).  95% nested bootstrap CIs from 2 000 query bootstrap samples.  Calibration: five‑fold on held‑out queries to select smallest budget retaining 95% of reference mean nDCG@20; calibrated budget applied to held‑out queries to assess quality retention and access savings.
- **Takeaway**: DiBud delivers the same or higher ranking quality as the balanced baseline while dramatically reducing the number of document accesses, especially for queries with heavy-tailed exact‑Top‑K costs.  By certifying each emitted document, it guarantees exactness under a hard latency budget, making it well‑suited for real‑time Retrieval‑Augmented Generation systems.
...
### 3. IROH: Insightful Ranking Of Humor using Multi-Stage Hybrid Retrieval with Rationale-Distilled LLM Judges for JOKER 2026 Track Task 1 English

- **LLM Score**: 0
- **Keyword Score**: 15
- **Authors**: Ana-Maria Luisa Mocanu, Sebastian Mocanu, Ciprian-Octavian Truică, Elena-Simona Apostol
- **URL**: <http://arxiv.org/abs/2609.15618v1>
- **Submitted**: 2026-09-14 14:12:38
- **Topic Keywords**: dense retrieval, query, ranking, rerank, retrieval, rank
- **Reason**: LLM scoring failed.
...
### 4. The Magnitude Mirage: Rethinking Confidence for Reasoning-Intensive Retrieval

- **LLM Score**: 0
- **Keyword Score**: 13
- **Authors**: Jamie Holdcroft, Abdelrahman Abdallah, Adam Jatowt
- **URL**: <http://arxiv.org/abs/2609.15578v1>
- **Submitted**: 2026-09-14 13:49:24
...
### 5. EviQE: Evidence Selection for LLM-Based Query Expansion

- **LLM Score**: 0
- **Keyword Score**: 12
- **Authors**: Hai Son Le, Amin Bigdeli, Shirin Seyedsalehi, Morteza Zihayat, Ebrahim Bagheri
- **URL**: <http://arxiv.org/abs/2609.14875v1>
- **Submitted**: 2026-09-14 00:52:45
- **Topic Keywords**: query, relevance, rag, retrieval, trec
- **Reason**: LLM scoring failed.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Query Expansion for Ad-hoc Search using Large Language Models
- **Aim**: To evaluate and establish best practices for leveraging LLMs in query expansion to enhance ad-hoc search performance
- **Rationale**: Ad-hoc search suffers from vocabulary mismatch; LLMs can generate semantically relevant expansions, potentially improving retrieval effectiveness
- **Ground**: Builds on prior work such as the arXiv preprint "Effective Adhoc Search" and the EMNLP 2024 study on LLM-based query expansion
- **Experiment**: Conduct systematic experiments on standard ad-hoc search benchmarks, comparing baseline retrieval, traditional expansion methods, and various LLM-driven expansion strategies, measuring MAP, NDCG, and precision
- **Takeaway**: LLM-based query expansion, when guided by the identified best practices, yields measurable gains in retrieval effectiveness, offering actionable guidelines for practitioners
...
### 6. RAG-CT: Mitigating Privacy Risks on Retrieval-Augmented Generation Systems via Scanning Prompt Distribution

- **LLM Score**: 0
- **Keyword Score**: 10
- **Authors**: Xingyu Lyu, Jiayimei Wang, Jianfeng He, Ning Wang, Yidan Hu, Yimin Chen
- **URL**: <http://arxiv.org/abs/2609.16095v1>
- **Submitted**: 2026-09-14 14:44:39
...
### 7. Generate to Explore, Select to Exploit: Aligning LLM-based Headline Generation with Personalized Recommendation

- **LLM Score**: 0
- **Keyword Score**: 10
- **Authors**: Yi Chen, Rufeng Cheng, Qiang Xie, Tao Li
- **URL**: <http://arxiv.org/abs/2609.15094v1>
- **Submitted**: 2026-09-14 06:14:38
- **Topic Keywords**: ranking, rag, ctr, recommend, personalization, rank
- **Reason**: LLM scoring failed.
...
- **Submitted**: 2026-09-14 17:58:27
- **Topic Keywords**: rerank, ltr, retrieval, rank
- **Reason**: LLM scoring failed.
...
### 9. MarKey: Marginal Utility Guided Greedy Keyframe Selection for Long Video Understanding

- **LLM Score**: 0
- **Keyword Score**: 8
- **Authors**: Hongchang Shi, Jinpeng Hu, Ao Wang, Wenzheng Zhou, Hui Ma, Feng Li, Zenglin Shi
- **URL**: <http://arxiv.org/abs/2609.15408v1>
- **Submitted**: 2026-09-14 11:39:20
...
### 10. PACE: Progressive Angular-to-Norm Contrastive Embedding
...
### 12. Beyond Retrieval: Scaffolding Children's Online Learning

- **LLM Score**: 0
- **Keyword Score**: 6
- **Authors**: Diletta Micol Tobia, Hrishita Chakrabarti, Maria Soledad Pera, Monica Landoni
- **URL**: <http://arxiv.org/abs/2609.15568v1>
- **Submitted**: 2026-09-14 13:46:02
...
### 13. Pull: Lazy Materialization of Working Memory for Stateful LLM Conversations

- **LLM Score**: 0
- **Keyword Score**: 6
- **Authors**: Jiangang Chen
- **URL**: <http://arxiv.org/abs/2609.14773v1>
- **Submitted**: 2026-09-13 20:26:13
...
### 14. Mind2Dialogue: Training Human-Aware Language Models by Simulating User Mental States
...
### 15. What Limits Us? Analyzing Self-Reported Limitations in NLP Research

- **LLM Score**: 0
- **Keyword Score**: 5
- **Authors**: Tawan Thaepprasit, Peeranuth Kehasukcharoen, Ding Wang, Remi Denton, Peerapon Vateekul, Piyawat Lertvittayakumjorn
- **URL**: <http://arxiv.org/abs/2609.15191v1>
- **Submitted**: 2026-09-14 08:11:57
- **Comment**: EMNLP 2026 Findings
- **Topic Keywords**: search, acl, emnlp
- **Reason**: LLM scoring failed.
...
### 16. CITECHOICE: A Causal Audit of How Document Presentation Redistributes Citation Credit in Agentic Search

- **LLM Score**: 0
- **Keyword Score**: 5
- **Authors**: Sriram Selvam, Anneswa Ghosh
- **URL**: <http://arxiv.org/abs/2609.15164v1>
- **Submitted**: 2026-09-14 07:47:47
...
### 18. LazFormer: Scaling Transformers for Industrial Recommendation via Transferable Generative Pre-training
...
### 20. CiteShade: Citation Laundering in Multi-Source Retrieval-Augmented Generation and Its Counterfactual Defense

- **LLM Score**: 0
- **Keyword Score**: 4
- **Authors**: Guo Fuzheng
- **URL**: <http://arxiv.org/abs/2609.15660v1>
- **Submitted**: 2026-09-14 14:41:37
...
### 21. Benchmarking Embedding Models for ESG Data

- **LLM Score**: 0
- **Keyword Score**: 4
- **Authors**: Motaz Saad, Veronica Cretì, Ivan Gentile, Kianna Kazemi, Antonella Longo
- **URL**: <http://arxiv.org/abs/2609.15434v1>
- **Submitted**: 2026-09-14 11:59:58
- **Topic Keywords**: rag, retrieval
- **Reason**: LLM scoring failed.
...
### 22. SALUTE: Benchmarking and Adapting LLMs for the Defense Domain
...
### 23. MedTRACE: Tool-Augmented Multimodal Clinical Reasoning Agents for Evidence-Grounded Decision-Making

- **LLM Score**: 0
- **Keyword Score**: 4
- **Authors**: Ji Lu, Lifei Liu, Haoran Yu, Xianglong Wang, Yiru Fang, Kuo Yang, Huiran Duan, Jianping Gou
- **URL**: <http://arxiv.org/abs/2609.14823v1>
- **Submitted**: 2026-09-13 22:32:49
...
### 25. EvoOntology: A Self-Evolving Ontology Layer for Data Agents

- **LLM Score**: 0
- **Keyword Score**: 3
- **Authors**: Meiduo Chong, Shaolei Zhang, Ju Fan, Xiaoyong Du
- **URL**: <http://arxiv.org/abs/2609.15779v1>
- **Submitted**: 2026-09-14 15:59:24
...
### 26. Enabling Streaming User Transcription in Full-Duplex Speech-to-Speech Models
...
- **URL**: <http://arxiv.org/abs/2609.15759v1>
- **Submitted**: 2026-09-14 15:44:18
...
### 27. Can We Trust the Judges? Validation of Factuality Evaluation Methods via Answer Perturbation
...
- **URL**: <http://arxiv.org/abs/2609.15561v1>
- **Submitted**: 2026-09-14 13:41:35
...
- **Submitted**: 2026-09-14 13:08:29
- **Comment**: EMNLP 2026 Main Conference
- **Topic Keywords**: listwise
- **Reason**: LLM scoring failed.
...
### 30. When the Wrong Key Wins: Understanding and Detecting Hallucinations in LLMs

- **LLM Score**: 0
- **Keyword Score**: 3
- **Authors**: Xuhan Tong, Jiawei Zhang
- **URL**: <http://arxiv.org/abs/2609.15106v1>
- **Submitted**: 2026-09-14 06:35:28
- **Topic Keywords**: query
- **Reason**: LLM scoring failed.
...
### 31. OpenAI4S: Code as Action, Science as Sessions
...
### 32. A primer on evaluation methods for large language models in healthcare
...
### 33. Disentangling Representation Evolution in Transformers through Directional Decomposition

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Shwai He, Haichao Zhang, Shen Yan
- **URL**: <http://arxiv.org/abs/2609.15975v1>
- **Submitted**: 2026-09-14 17:56:19
...
### 35. Merging the Knowledge of LLMs for Automatic Speech Recognition

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Hayato Futami, Tatsuya Kawahara
- **URL**: <http://arxiv.org/abs/2609.15743v1>
- **Submitted**: 2026-09-14 15:35:22
...
### 36. RESKILL: Explicit Failure Attribution and Structured Repair for Interactive Language Agents

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Mengyi Deng, Xin Li, Duyi Pan, Zilin Wang, Zhiwei Li, Zhijiang Guo, Wei Wang
- **URL**: <http://arxiv.org/abs/2609.15684v1>
- **Submitted**: 2026-09-14 14:55:13
...
### 37. Human-Grounded Calibration for Long-Text Image-Text Congruence in Vision-Language Models

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Alessandro Gambetti, Qiwei Han
- **URL**: <http://arxiv.org/abs/2609.15640v1>
- **Submitted**: 2026-09-14 14:26:53
...
### 38. Psychosis involves a deficit of information compression in connected speech
...
- **URL**: <http://arxiv.org/abs/2609.15522v1>
- **Submitted**: 2026-09-14 13:13:30
- **Topic Keywords**: ctr
- **Reason**: LLM scoring failed.
...
### 39. Temperature Fragility and the Conditional Benefits of Truncation Sampling

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Francesco La Rosa
- **URL**: <http://arxiv.org/abs/2609.15476v1>
- **Submitted**: 2026-09-14 12:33:36
...
### 40. Turkish MMLU Pro: Traceable Option Augmentation and Its Validity Limits in Turkish Multiple-Choice Evaluation

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: M. Ali Bayram
- **URL**: <http://arxiv.org/abs/2609.15467v1>
- **Submitted**: 2026-09-14 12:28:09
...
### 41. Reason What Matters: Retrieval-Grounded Reasoning for Universal Multimodal Embeddings
...
### 42. Improving Mathematical Reasoning Capabilities in Large Language Models via Reasoning Process Error Classification

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Runa Yoshida, Kosuke Nishida, Kyosuke Nishida
- **URL**: <http://arxiv.org/abs/2609.15145v1>
- **Submitted**: 2026-09-14 07:20:44
- **Comment**: 10 pages
- **Topic Keywords**: rag
- **Reason**: LLM scoring failed.
...
### 43. The average-farmer illusion in language-model simulations of agricultural decisions
...
### 44. ABSOL: Aggregated Bayesian Subsampling Orchestrated with LLMs

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Jackson Hassell, Chen Shen, Estevam Hruschka
- **URL**: <http://arxiv.org/abs/2609.15007v1>
- **Submitted**: 2026-09-14 04:15:05
- **Topic Keywords**: rag
- **Reason**: LLM scoring failed.
...
### 45. MTAC-IFBench: Benchmarking Instruction-Following in Multi-Turn Agentic Coding
...
### 46. Online Language Adaptive Sampling for Better Distributed Cross-lingual Gains
...
### 47. Forty Shades of Blue: Quality-Diversity Alignment via Mode-Conditioned Reinforcement Learning
...
### 48. One Example Is Enough to Pass Fairness Benchmarks: Rethinking Fairness Evaluation for Aligned LLMs

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Naihao Deng, Samee Arif, Shuaichen Chang, Yulong Chen, Rada Mihalcea
- **URL**: <http://arxiv.org/abs/2609.14860v1>
- **Submitted**: 2026-09-14 00:21:26
...
### 49. Self-Orchestrating Language Models: Leveraging Semantic Dependence for Efficient Inference

- **LLM Score**: 0
- **Keyword Score**: 2
- **Authors**: Tian Jin
- **URL**: <http://arxiv.org/abs/2609.14850v1>
- **Submitted**: 2026-09-13 23:48:33
- **Topic Keywords**: rag
- **Reason**: LLM scoring failed.
...
### 50. LLMs as Oracles: Reliance on LLMs for Subjective Personal Questions
