# Daily Papers Report - 2025-12-23
...
- **Reason**: This paper explores the features used for summary evaluation by humans and GPT, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on summary evaluation and large language models is not a central match to your primary research interests, but it does touch on areas of deep semantic understanding and real-time relevance optimization.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Human‑vs. LLM‑Based Evaluation of Summaries
- **Aim**: Discover unsupervised textual attributes that correlate strongly with both human and LLM (GPT) quality judgments, thereby revealing concrete, reference‑free metrics for relevance, coherence, and overall summarization quality.
- **Rationale**: LLMs can approximate human ratings when metrics are clear, yet the internal features driving LLM judgments are poorly understood. Mapping these features enables systematic alignment of automated judges with human perception and improves the reliability of LLM‑as‑judge systems.
- **Ground**: The study builds on prior work showing GPTs can mimic human scores with chain‑of‑thought prompting. It formulates the task as predicting a quality score y for a summary x, using a dataset of human‑rated and LLM‑predicted scores. A comprehensive suite of metrics—readability indices (Flesch, ARI, Gunning Fog, etc.), information‑theoretic measures (entropy, perplexity, conditional perplexity), diversity metrics (Gini, sparsity), and semantic similarity (sentence‑transformer cosine)—is extracted. Correlation analyses (Pearson, Spearman, Kendall) identify which metrics align with human and GPT scores. Instruction‑based prompting adds a “Hint” section listing the top human‑aligned metrics to guide GPT evaluation. Experiments compare several GPT variants (GPT‑4.1‑nano, GPT‑4.1‑mini, GPT‑4o‑mini, GPT‑4‑0613) under CoT prompting with and without hints, measuring correlation improvements. Key findings highlight conditional perplexity as the strongest predictor, the dominance of information‑theoretic metrics over traditional reference‑free scores, and the benefit of explicit metric instructions. The work situates itself among fine‑tuning, preference‑based training, and multimodal evaluation research, emphasizing the broader generalization of GPT‑4 over domain‑specific fine‑tuned judges.
- **Experiment**: 1. Extract 20+ metrics per summary. 2. Compute Pearson, Spearman, Kendall correlations with human and GPT scores. 3. Identify top metrics per dimension (relevance, coherence). 4. Construct prompts with a Hint section listing these metrics. 5. Evaluate GPT variants using CoT prompting, recording correlation coefficients. 6. Measure the effect of hints on alignment (e.g., GPT‑4o‑mini relevance Pearson ↑0.01). 7. Compare against baseline reference‑free metrics (MoverScore, BERTScore). 8. Conduct supervised fine‑tuning of LLaMA on GPT‑4‑generated data to assess domain specificity.
- **Takeaway**: Conditional perplexity and related information‑theoretic features consistently predict both human and GPT judgments, outperforming traditional reference‑free metrics. Explicitly instructing GPTs to consider these human‑preferred metrics via a Hint section narrows the LLM‑human gap, especially for larger, reasoning‑capable models like GPT‑4‑0613. The study provides a framework for unsupervised feature discovery, enabling reference‑free, reliable evaluation of summarization quality and guiding future LLM‑as‑judge system design.
...
### 2. Generative vector search to improve pathology foundation models across multimodal vision-language tasks
...
- **Reason**: The paper explores generative vector search for improving pathology foundation models across multimodal vision-language tasks. While it touches on retrieval-augmented generation and vector search, which are related to information retrieval, the focus on pathology and multimodal vision-language tasks is somewhat distant from the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 3. A Large Language Model Based Method for Complex Logical Reasoning over Knowledge Graphs
...
- **Reason**: The paper explores knowledge graph reasoning with a large language model, which is somewhat related to information retrieval and NLP. However, the focus on knowledge graph reasoning and logical inference is not directly aligned with the user's primary interests in query understanding, ranking models, and user behavior modeling.
...
- **Reason**: This paper is somewhat related to information retrieval, but its focus on dynamic retrieval-augmented generation and uncertainty quantification in large language models is not directly aligned with your core research themes. While it touches on aspects of query understanding and real-time relevance optimization, the context is primarily within the realm of natural language processing and generation, rather than search technologies or user behavior modeling.
...
### 5. A Large-Language-Model Framework for Automated Humanitarian Situation Reporting
...
- **Reason**: The paper is somewhat related to the user's interests in Natural Language Processing (NLP) and related topics, but it does not directly align with the user's primary focus on Information Retrieval (IR), especially in areas that require deep semantic understanding and real-time relevance optimization. The paper's application in humanitarian situation reporting is also outside the user's typical domain of e-commerce.
...
### 6. Event Extraction in Large Language Model
...
- **Reason**: The paper focuses on event extraction, which is related to information retrieval, but its primary emphasis on large language models and multimodal settings is not directly aligned with the user's core research themes. While it touches on aspects of query understanding and real-time relevance optimization, the connection is somewhat loose. The paper's focus on event extraction and its applications in text and multimodal settings is not a central match for the user's research interests.
...
### 7. From Retrieval to Reasoning: A Framework for Cyber Threat Intelligence NER with Explicit and Adaptive Instructions
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, as it involves Named Entity Recognition and Large Language Models. However, the focus on Cyber Threat Intelligence and the specific framework proposed (TTPrompt) is not directly aligned with your core research themes, which include query understanding, ranking models, and user behavior modeling.
...
### 8. Auto-Prompting with Retrieval Guidance for Frame Detection in Logistics
...
- **Topic Keywords**: rag, retrieval
- **Reason**: The paper explores the application of large language models in logistics, focusing on prompt engineering and optimization for frame detection. While it touches on retrieval-augmented generation, which is related to information retrieval, the primary focus is on NLP and prompt engineering, making it somewhat relevant to your interests but not a central match.
...
### 9. Exploring Zero-Shot ACSA with Unified Meaning Representation in Chain-of-Thought Prompting
...
- **Reason**: The paper explores a specific NLP task (Aspect-Category Sentiment Analysis) and proposes a novel approach using large language models. While it touches on aspects of query understanding and ranking models indirectly, its primary focus is on NLP and not directly related to the user's core research themes in IR and search technologies.
...
### 10. Bottom-up Policy Optimization: Your Language Model Policy Secretly Contains Internal Policies
...
- **Topic Keywords**: rag
- **Reason**: This paper explores reinforcement learning and large language models, but its focus on internal policy decomposition and optimization doesn't directly align with your core research themes in Information Retrieval and Search technologies. While it touches on deep semantic understanding, the context is more related to NLP and RL, making it somewhat relevant but not a central match.
...
### 11. MAGIC: Achieving Superior Model Merging via Magnitude Calibration
...
- **Topic Keywords**: search
- **Reason**: This paper proposes a framework for model merging in pre-trained models, focusing on magnitude calibration. While it touches on the topic of model merging, which is related to ranking models, the primary focus is on model merging and its application in computer vision and NLP tasks. The connection to information retrieval and query understanding is not explicitly mentioned, making it somewhat relevant but not a central match.
...
### 12. Affordance RAG: Hierarchical Multimodal Retrieval with Affordance-Aware Embodied Memory for Mobile Manipulation
...
- **Topic Keywords**: rerank, rag, retrieval, rank, acl
- **Reason**: This paper focuses on mobile manipulation and multimodal retrieval for robots, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves understanding visual semantics, the context is robotics and not e-commerce or general search, making it somewhat tangential to the user's research areas.
...
### 13. Remedy-R: Generative Reasoning for Machine Translation Evaluation without Error Annotations
...
- **Reason**: This paper focuses on machine translation evaluation, which is outside the primary scope of your research interests in Information Retrieval and Search technologies. While it involves some NLP aspects, the main topic is not related to query understanding, ranking models, or user behavior modeling, which are core areas of your expertise.
...
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The paper focuses on benchmarking autonomous mobile agents in agent-user interactive environments, which is outside your primary areas of interest.
...
### 15. BanglaForge: LLM Collaboration with Self-Refinement for Bangla Code Generation
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is not relevant to your research interests as it focuses on code generation for a low-resource language (Bangla) and does not address information retrieval, search technologies, or query understanding. Although it mentions a retrieval-augmented paradigm, its application is specific to code generation and not related to your core areas of interest.
...
- **Reason**: This paper focuses on chart understanding and generation, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some form of data parsing and question answering, the context is specific to charts and does not align with your primary focus on deep semantic understanding and real-time relevance optimization in the e-commerce domain or other areas.
...
### 17. SAP: Syntactic Attention Pruning for Transformer-based Language Models
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on pruning attention heads in Transformer models for language understanding, which is not directly related to Information Retrieval, Search technologies, or user behavior modeling. While it touches on NLP, it doesn't address query understanding, ranking models, or real-time relevance optimization, making it only loosely relevant to your research interests.
...
### 18. DramaBench: A Six-Dimensional Evaluation Framework for Drama Script Continuation
...
- **Reason**: This paper is not relevant to your core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves language models, the focus is on creative writing evaluation and drama script continuation, which is unrelated to your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 19. GenEnv: Difficulty-Aligned Co-Evolution Between LLM Agents and Environment Simulators
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Large Language Model (LLM) agents and environment simulators, which is not directly related to your core research interests in Information Retrieval and Search technologies. While it involves some aspects of query understanding and ranking models, the context is more aligned with NLP and data mining, but not specifically with your areas of focus.
...
### 20. MauBERT: Universal Phonetic Inductive Biases for Few-Shot Acoustic Units Discovery
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on speech representation learning and phonetic properties, which is not directly related to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves self-supervised learning, the context is speech modeling rather than text or query understanding.
...
### 21. Efficient Jailbreak Mitigation Using Semantic Linear Classification in a Multi-Staged Pipeline
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on security challenges in large language model systems and proposes a defense architecture to mitigate jailbreaking attacks. While it involves text normalization and classification, the context is security and not directly related to information retrieval, search technologies, or user behavior modeling, which are the primary areas of interest.
...
### 22. FASTRIC: Prompt Specification Language for Verifiable LLM Interactions
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Large Language Models (LLMs) and their interactions, which is outside your primary research area of Information Retrieval and Search technologies. While it touches on aspects of model behavior and interaction, it does not directly relate to your core interests in query understanding, ranking models, or user behavior modeling.
...
### 23. Diacritic Restoration for Low-Resource Indigenous Languages: Case Study with Bribri and Cook Islands Māori
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on diacritic restoration for low-resource languages, which is a specific task in Natural Language Processing (NLP). While it touches on the use of deep learning models, it does not directly relate to the user's core research themes in Information Retrieval, query understanding, ranking models, or user behavior modeling.
...
- **Topic Keywords**: search
- **Reason**: This paper is off-topic as it focuses on sentiment analysis and dialectal Arabic NLP, which, while related to NLP, does not align with the user's primary interests in Information Retrieval, query understanding, ranking models, and user behavior modeling.
...
### 25. Identifying Features Associated with Bias Against 93 Stigmatized Groups in Language Models and Guardrail Model Safety Mitigation
...
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves language models, its focus on bias and social stigma is more aligned with NLP applications in social sciences and ethics, rather than your core areas of interest.
...
### 26. Context-Aware Initialization for Reducing Generative Path Length in Diffusion Language Models
...
### 27. Toward Human-Centered AI-Assisted Terminology Work
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on the application of AI in terminology work, emphasizing the need for a human-centered approach. While it touches on AI adoption, it does not directly relate to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 28. Kunnafonidilaw ka Cadeau: an ASR dataset of present-day Bambara
...
- **Topic Keywords**: rag
- **Reason**: This paper is unrelated to Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or recommender systems. It focuses on Automatic Speech Recognition (ASR) for Bambara, a language, and does not align with your research interests.
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be unrelated to Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on a mathematical model of linguistic structure formation and transformation in generative linguistics.
