...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in ranking models and user behavior modeling. The proposed RIA framework integrates pointwise and listwise evaluation, which is closely related to Learning to Rank. However, the focus on recommender systems and advertising systems is slightly outside your primary focus on e-commerce and deep semantic understanding.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: RIA (Ranking-Infused Architecture), a framework for listwise Click-Through Rate (CTR) prediction that bridges ranking and reranking stages in recommendation systems.
- **Aim**: To address combinatorial sparsity (exponential decay in co-exposure frequency with list size) and architectural decoupling between ranking and reranking stages in existing CTR prediction models.
- **Rationale**: Current methods fail to model position-sensitive user preferences and cross-item dependencies effectively, leading to suboptimal CTR predictions and poor scalability in real-world systems.
- **Ground**: RIA integrates four components: (1) User and Candidate Dual-Transformer (UCDT) for user-item interaction modeling, (2) Context-aware User History and Target (CUHT) for position-sensitive preference capture, (3) Listwise Multi-HSTU (LMH) for cross-item dependency modeling, and (4) Embedding Cache (EC) for inference efficiency. It uses a sigmoid-based pointwise CTR prediction with a binary cross-entropy loss function.
- **Experiment**: Evaluated on public datasets and deployed in Meituan’s advertising system (September 6–14, 2025). RIA_small achieved +1.69% CTR and +4.54% Cost Per Mille (CPM) improvements over the baseline, while RIA_big achieved +2.11% CTR and +5.83% CPM, with latency increases of 0.1–0.2 ms.
- **Takeaway**: RIA demonstrates practical effectiveness in enhancing user engagement (CTR) and revenue (CPM) while maintaining low computational overhead, validating its robustness for large-scale recommendation systems.
...
- **Reason**: The paper focuses on generative engine optimization for e‑commerce, which aligns with IR and NLP interests, especially query understanding and relevance. It introduces a new benchmark and optimization methods, but it does not directly address traditional ranking models or click‑through behavior, making it somewhat related rather than central.
...
### 3. Beyond Patch Aggregation: 3-Pass Pyramid Indexing for Vision-Enhanced Document Retrieval
...
- **Reason**: The paper presents a novel vision‑first retrieval system that addresses document indexing and ranking, aligning with the user’s interest in IR and ranking models. However, it focuses on multimodal, OCR‑free techniques rather than traditional query understanding, learning‑to‑rank, or click‑model approaches, making it a somewhat peripheral but still relevant contribution.
...
### 4. Context-Aware Pragmatic Metacognitive Prompting for Sarcasm Detection
...
- **Reason**: The paper is relevant to your research interests in Natural Language Processing (NLP) and specifically addresses sarcasm detection, a task that requires deep semantic understanding. However, the focus on sarcasm detection and the use of pre-trained language models (PLMs) and large language models (LLMs) is somewhat tangential to your primary interest in information retrieval, especially in areas that require real-time relevance optimization.
...
### 5. FITRep: Attention-Guided Item Representation via MLLMs
...
- **Topic Keywords**: ctr
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of item representation and deduplication. However, the focus on multimodal large language models and fine-grained item deduplication is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 6. Chatty-KG: A Multi-Agent AI System for On-Demand Conversational Question Answering over Knowledge Graphs
...
- **Reason**: The paper discusses conversational question answering over knowledge graphs, which is related to information retrieval and natural language processing. However, it focuses on dialogue systems and multi-agent systems, which is somewhat tangential to the user's primary interests in query understanding, ranking models, and user behavior modeling.
...
### 7. TrackList: Tracing Back Query Linguistic Diversity for Head and Tail Knowledge in Open Large Language Models
...
- **Reason**: This paper explores the limitations of Large Language Models in providing diverse types of answers, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on linguistic diversity and pre-training data impact on LLMs' performance is not directly aligned with the user's core research themes, such as user behavior modeling and real-time relevance optimization.
...
- **Reason**: The paper focuses on large language model (LLM) agents and their ability to learn and evolve memory, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the paper's emphasis on test-time evolution and self-evolving memory in LLM agents is not directly aligned with the user's core research themes. The connection to user behavior modeling and click models is also not explicitly mentioned.
...
- **Reason**: This paper introduces a method for training small orchestrators to coordinate intelligent tools, which is somewhat related to information retrieval and search technologies. However, the focus on large language models and tool orchestration is not directly aligned with the user's primary research interests in query understanding, ranking models, and user behavior modeling. The paper's emphasis on efficiency and cost-effectiveness in solving complex problems is also somewhat tangential to the user's interests in deep semantic understanding and real-time relevance optimization.
...
- **Reason**: The paper explores generalization of large language models across different task difficulties, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on language models and educational testing is not directly aligned with the user's core research themes in IR and Search technologies. The connection to NLP is relevant, but the paper's scope is more focused on model evaluation rather than user behavior modeling or real-time relevance optimization.
...
### 11. Gated KalmaNet: A Fading Memory Layer Through Test-Time Ridge Regression
...
- **Topic Keywords**: rag
- **Reason**: This paper proposes a novel approach to improving the performance of linear state-space models in tasks requiring deep semantic understanding, such as language understanding. However, it does not directly relate to information retrieval, query understanding, or ranking models, which are core areas of interest. The paper's focus on NLP and fading memory layers is somewhat relevant, but not a central match to the user's research themes.
...
### 12. Emergence and Localisation of Semantic Role Circuits in LLMs
...
- **Topic Keywords**: ctr
- **Reason**: This paper explores the internal mechanisms of large language models (LLMs) and their ability to ground abstract semantic structure. While it touches on semantic understanding, which is related to your interests in Information Retrieval and NLP, the focus on LLMs and semantic role circuits is somewhat tangential to your core research themes. The paper's findings on compact, causally isolated mechanisms for abstract semantic structure may have some relevance to your work on deep semantic understanding, but it is not a direct match.
...
### 13. Can Finetuing LLMs on Small Human Samples Increase Heterogeneity, Alignment, and Belief-Action Coherence?
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and Information Retrieval (IR), particularly in the context of query understanding and user behavior modeling. However, the focus on large language models and their limitations in simulating human behavior is not directly aligned with your primary interests in search technologies and ranking models. The paper's findings on fine-tuning LLMs may have some indirect implications for IR, but it is not a central match for your research themes.
...
### 14. Unsupervised Memorability Modeling from Tip-of-the-Tongue Retrieval Queries
...
- **Reason**: This paper focuses on visual content memorability and introduces a new dataset for modeling visual memorability signals. Although it involves natural language processing and retrieval, it is not directly related to information retrieval, search technologies, or query understanding, which are core areas of your research interests.
...
### 15. Hierarchical Ranking Neural Network for Long Document Readability Assessment
...
- **Reason**: This paper focuses on readability assessment, which is not a core area of interest in Information Retrieval or Search technologies. While it involves some NLP aspects, the primary goal is not query understanding, ranking models, or user behavior modeling, making it only loosely relevant to your research interests.
...
### 16. Structured Prompting Enables More Robust, Holistic Evaluation of Language Models
...
- **Reason**: This paper focuses on language model benchmarking and evaluation, which is somewhat related to information retrieval and search technologies. However, the specific context of language models and their evaluation does not directly align with the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 17. Generating Querying Code from Text for Multi-Modal Electronic Health Record
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text processing and query generation, the context is specific to Electronic Health Records and does not align with the user's primary focus on deep semantic understanding and real-time relevance optimization in e-commerce or general IR applications.
...
### 18. Prune4Web: DOM Tree Pruning Programming for Web Agent
...
- **Reason**: This paper focuses on web automation and Large Language Model (LLM)-based web agents, but it doesn't directly relate to Information Retrieval (IR), Search technologies, query understanding, ranking models, or user behavior modeling, which are core areas of your research interests.
...
### 19. TrafficLens: Multi-Camera Traffic Video Analysis Using LLMs
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on traffic video analysis using Large Language Models (LLMs), which is outside the core areas of Information Retrieval and Search technologies that you're primarily interested in. While it does involve text generation, the context is specific to video analysis and not directly related to your research themes of query understanding, ranking models, or user behavior modeling.
...
### 20. BanglaASTE: A Novel Framework for Aspect-Sentiment-Opinion Extraction in Bangla E-commerce Reviews Using Ensemble Deep Learning
...
- **Reason**: This paper focuses on Aspect-Based Sentiment Analysis in the Bangla language, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it involves text processing and analysis, the specific application and language are not aligned with the user's interests.
...
### 21. Towards Audio Token Compression in Large Audio Language Models
...
- **Reason**: This paper focuses on audio token compression in large audio language models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves language models, the context is audio-based and does not align with your primary focus on deep semantic understanding and real-time relevance optimization in the e-commerce domain or other areas.
...
### 22. Memories Retrieved from Many Paths: A Multi-Prefix Framework for Robust Detection of Training Data Leakage in Large Language Models
...
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or query understanding, which are core areas of your research interests. While it involves Natural Language Processing and large language models, its focus on data leakage detection and memorization is not a central match for your research themes.
...
### 23. G$^2$VLM: Geometry Grounded Vision Language Model with Unified 3D Reconstruction and Spatial Reasoning
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Vision-Language Models and 3D reconstruction, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some form of understanding and reasoning, it is more related to computer vision and spatial intelligence, which is not a central match for your research.
...
### 24. Bangla Sign Language Translation: Dataset Creation Challenges, Benchmarking and Prospects
...
- **Reason**: This paper is not relevant to your research interests as it focuses on Bangla Sign Language Translation, which is outside the scope of Information Retrieval, Search technologies, and Natural Language Processing. The paper's focus on dataset creation and benchmarking for assistive tools for deaf and hard of hearing people also does not align with your interests in query understanding, ranking models, and user behavior modeling.
...
### 25. Voice, Bias, and Coreference: An Interpretability Study of Gender in Speech Translation
...
- **Topic Keywords**: ctr
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on speech translation, bias, and coreference in speech, which is a distinct area of study.
...
### 26. Subjective Depth and Timescale Transformers: Learning Where and When to Compute
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on adaptive safety alignment for reasoning models, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves model fine-tuning and optimization, the context is specific to adversarial safety and not aligned with the user's primary focus on query understanding, ranking models, and user behavior modeling.
...
### 28. AnchorOPT: Towards Optimizing Dynamic Anchors for Adaptive Prompt Learning
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on prompt learning and CLIP models, which is somewhat related to your interests in NLP and related topics. However, it does not directly address information retrieval, query understanding, or ranking models, making it less relevant to your core research themes.
...
### 29. ASR Error Correction in Low-Resource Burmese with Alignment-Enhanced Transformers using Phonetic Features
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on automatic speech recognition (ASR) error correction in low-resource Burmese, which is unrelated to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves sequence-to-sequence models and feature integration, the context is specific to ASR and not directly applicable to the user's interests in query understanding, ranking models, or user behavior modeling.
...
### 30. RosettaSpeech: Zero-Shot Speech-to-Speech Translation from Monolingual Data
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on speech-to-speech translation and zero-shot learning from monolingual data. While it involves NLP, the context and application are quite different from your areas of focus.
...
### 31. Length-MAX Tokenizer for Language Models
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on optimizing language model tokenization for efficiency, which is not directly related to information retrieval, query understanding, or ranking models. While it touches on NLP, the primary goal is to improve language model performance, not to address user behavior modeling or deep semantic understanding.
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on developing a conversational speech corpus for the Isan language, which is not directly related to Information Retrieval, Search technologies, or Natural Language Processing. While it touches on the topic of conversational speech, it does not address query understanding, ranking models, or user behavior modeling, making it an off-topic paper for your research interests.
...
### 33. How to Correctly Report LLM-as-a-Judge Evaluations
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on Large Language Models (LLMs) and their evaluation, which is not directly related to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on aspects of model evaluation, it does not address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to your interests.
...
### 34. Zipf Distributions from Two-Stage Symbolic Processes: Stability Under Stochastic Lexical Filtering
...
- **Comment**: 16 pages
- **Topic Keywords**: rank
- **Reason**: This paper appears to be related to Natural Language Processing (NLP), but its focus on Zipf's law and linguistic elements does not align with your primary research interests in Information Retrieval, query understanding, ranking models, and user behavior modeling. The paper's emphasis on symbolic processes and geometric constraints also does not seem to be directly relevant to your work in real-time relevance optimization and deep semantic understanding.
...
### 35. The author is dead, but what if they never lived? A reception experiment on Czech AI- and human-authored poetry
...
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or Natural Language Processing. The paper focuses on the perception and aesthetic evaluation of AI-generated poetry in the Czech language, which does not align with your areas of expertise.
