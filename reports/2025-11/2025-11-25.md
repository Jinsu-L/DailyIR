# Daily Papers Report - 2025-11-25

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. STORE: Semantic Tokenization, Orthogonal Rotation and Efficient Attention for Scaling Up Ranking Models
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in ranking models and scalability. The authors propose a unified and scalable token-based ranking framework, STORE, which addresses key bottlenecks in model scalability and efficiency. The paper's focus on semantic tokenization, orthogonal rotation, and efficient attention aligns well with your interests in deep semantic understanding and real-time relevance optimization.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: STORE: Semantic Tokenization, Orthogonal Rotation, and Efficient Attention for Scaling Up Ranking Models
- **Aim**: Propose a unified framework to address representation and computational bottlenecks in large-scale ranking models for recommendation systems.
- **Rationale**: High-cardinality features (e.g., item IDs) cause low-rank embeddings and 'One-Epoch'/'Interaction-Collapse' phenomena. Attention mechanisms face computational costs and dispersion due to heterogeneous features.
- **Ground**: Builds on prior work in semantic tokenization (OPMQ network), orthogonal transformations, and attention sparsification. Extends methods like DeepFM and AutoInt with novel components.
- **Experiment**: Evaluated on industrial datasets (7B user interactions) and public datasets (Avazu). Baselines include DeepFM, AutoInt, and DSSM. Metrics: AUC, CTR, training throughput, and online A/B tests.
- **Takeaway**: STORE improves AUC by 1.195%, CTR by 2.71%, and training throughput by 1.84×. Efficient attention reduces computational costs by 50% with minimal accuracy loss. Deployed with K=32 codebooks and 300 SIDs in production.
...
### 2. Revisiting Feedback Models for HyDE
...
- **Topic Keywords**: retriever, query, queries, relevance, relevance feedback, rag
- **Reason**: This paper is highly relevant to Information Retrieval, specifically query expansion and pseudo-relevance feedback, with a focus on leveraging large language models. The use of traditional feedback models like Rocchio and RM3 is also of interest. However, the specific domain of HyDE and its application to sparse retrievers like BM25 limits its generalizability to the user's broader research interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Evaluation of traditional pseudo-relevance feedback (PRF) models in HyDE
- **Aim**: Assess whether traditional PRF models (e.g., Rocchio, RM3) outperform string concatenation baselines in enhancing HyDE's query expansion
- **Rationale**: Recent LLM-based PRF methods for HyDE have prioritized string concatenation over traditional feedback models, potentially neglecting their robust term selection and weighting capabilities
- **Ground**: 14 datasets (MS MARCO, BEIR) with Recall@20 metric; models compared include Rocchio, RM3, average vector, MuGI, and string concatenation baselines (Query2Doc)
- **Experiment**: Comprehensive evaluation of HyDE with feedback models vs. concatenation baselines, analyzing term selection, parameterized weighting, and low-resource task performance
- **Takeaway**: Traditional models like Rocchio improve HyDE by up to 4.2% average (6% in low-resource tasks), outperforming concatenation due to noise filtering and stable term weighting
...
### 3. What Drives Cross-lingual Ranking? Retrieval Approaches with Multilingual Language Models
...
- **Topic Keywords**: information retrieval, dense retrieval, query, ranking, retrieval, rank, search
- **Reason**: This paper is highly relevant to Information Retrieval, specifically cross-lingual ranking and retrieval approaches, which aligns with your research interests. The focus on multilingual language models and semantic understanding also resonates with your background in NLP and e-commerce domain. The paper's emphasis on real-time relevance optimization and evaluation of different retrieval approaches is particularly relevant to your interests.
...
### 4. Generative Query Expansion with Multilingual LLMs for Cross-Lingual Information Retrieval
...
- **Topic Keywords**: information retrieval, dense retrieval, query, queries, retrieval
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding and cross-lingual retrieval. The use of multilingual LLMs for query expansion aligns with your focus on deep semantic understanding and real-time relevance optimization. The study's evaluation of generative expansion strategies and factors that drive cross-lingual retrieval performance is also closely related to your interests in ranking models and user behavior modeling.
...
### 5. CLaRa: Bridging Retrieval and Generation with Continuous Latent Reasoning
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of query understanding and ranking models. The proposed CLaRa framework combines retrieval and generation, which is a key aspect of your focus on deep semantic understanding and real-time relevance optimization. However, the paper's primary application is in question-answering, which is somewhat outside your e-commerce domain expertise, but still aligns with your broader interests in NLP and IR.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: CLaRa (Continuous Latent Reasoning), a novel retrieval-augmented generation (RAG) approach
- **Aim**: Address limitations of RAG by enabling embedding-based compression and joint optimization in a shared continuous space
- **Rationale**: Traditional RAG systems struggle with balancing retrieval relevance and answer quality due to disjointed retrieval-generation pipelines and suboptimal compression methods
- **Ground**: 1. SCP framework for semantically rich compressed vectors using QA and paraphrase supervision
2. Unified end-to-end training of reranker and generator via differentiable top-k estimation
- **Experiment**: Evaluated across multiple QA benchmarks, demonstrating state-of-the-art compression and reranking performance surpassing text-based fine-tuned baselines
- **Takeaway**: CLaRa's unified optimization and continuous space alignment achieve superior retrieval-generation synergy, setting new standards for RAG efficiency and effectiveness
...
### 6. Look It Up: Analysing Internal Web Search Capabilities of Modern LLMs
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of real-time relevance optimization and query understanding. The focus on large language models and their internal web search capabilities aligns with your interests in search technologies and deep semantic understanding. However, the paper's primary focus on NLP applications and language models slightly reduces its relevance to your broader interests in IR and e-commerce.
...
### 7. HyperbolicRAG: Enhancing Retrieval-Augmented Generation with Hyperbolic Representations
...
- **Reason**: This paper is highly relevant to the field of Information Retrieval, particularly in the area of query understanding and ranking models. The use of hyperbolic geometry to represent abstraction relationships in knowledge graphs is a novel approach that aligns with the user's interests in deep semantic understanding and real-time relevance optimization. The focus on retrieval-augmented generation and its applications to question-answering benchmarks also resonates with the user's background in e-commerce and NLP.
...
### 8. PRInTS: Reward Modeling for Long-Horizon Information Seeking
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding and ranking models. The paper introduces a new reward modeling approach (PRInTS) for long-horizon information seeking, which aligns with your focus on deep semantic understanding and real-time relevance optimization. While the paper's focus is on AI agents and language models, the underlying concepts and techniques are applicable to search technologies and information retrieval.
...
### 9. Skeletons Matter: Dynamic Data Augmentation for Text-to-Query
...
- **Reason**: This paper is highly relevant to Information Retrieval, specifically query understanding and ranking models, as it deals with translating natural language questions into query languages. The use of Large Language Models and dynamic data augmentation is also a strong match for the user's interests in NLP and related topics. While the focus is on semantic parsing, it has implications for real-time relevance optimization in search technologies.
...
### 10. A Multi-Agent LLM Framework for Multi-Domain Low-Resource In-Context NER via Knowledge Retrieval, Disambiguation and Reflective Analysis
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the area of query understanding and ranking models. Although it focuses on Named Entity Recognition (NER) and Natural Language Processing (NLP), the use of knowledge retrieval and disambiguation mechanisms shares some similarities with your interests in deep semantic understanding and real-time relevance optimization. However, the paper's primary focus on NLP and NER limits its direct relevance to your core research themes in IR and search technologies.
...
### 11. Concept than Document: Context Compression via AMR-based Conceptual Entropy
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper explores context compression using Abstract Meaning Representation (AMR) graphs, which is relevant to information retrieval and natural language processing. Although it doesn't directly focus on query understanding, ranking models, or user behavior modeling, it does address a related issue of context overload in Retrieval-Augmented Generation (RAG). The use of AMR-based conceptual entropy for context compression is a novel approach that could be useful in information retrieval applications.
...
### 12. Prompt Optimization as a State-Space Search Problem
...
- **Reason**: This paper explores prompt optimization as a state-space search problem, which is related to query understanding and ranking models in Information Retrieval. However, its focus on NLP tasks and prompt optimization is somewhat tangential to the user's core research themes, although it does involve search and optimization techniques.
...
### 13. CoreEval: Automatically Building Contamination-Resilient Datasets with Real-World Knowledge toward Reliable LLM Evaluation
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and data quality, but it doesn't directly address your core focus on Information Retrieval (IR), query understanding, or ranking models. The paper's emphasis on data contamination and robust evaluation is relevant, but it's more aligned with NLP and model evaluation rather than IR and search technologies.
...
### 14. DR Tulu: Reinforcement Learning with Evolving Rubrics for Deep Research
...
- **Topic Keywords**: query, search
- **Reason**: The paper discusses a reinforcement learning approach for deep research models, which is somewhat related to information retrieval and search technologies. However, the focus is on long-form research tasks and question answering, which is not a central match to the user's interests in query understanding, ranking models, and user behavior modeling. The connection to NLP and data mining is also present, but the paper's primary contribution is in the domain of deep research models, making it only loosely relevant to the user's research interests.
...
### 15. Large Language Models Require Curated Context for Reliable Political Fact-Checking -- Even with Reasoning and Web Search
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and NLP, as it involves evaluating the performance of large language models on fact-checking tasks. However, the focus on fact-checking and political context is not directly aligned with your primary interests in query understanding, ranking models, and user behavior modeling. The paper's findings on the importance of curated context are also relevant to broader NLP and IR topics, but not a central match for your research focus.
...
### 16. From Raw Features to Effective Embeddings: A Three-Stage Approach for Multimodal Recipe Recommendation
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the area of recommender systems. However, it focuses on multimodal recipe recommendation, which is a specific application domain, and does not directly address query understanding, ranking models, or user behavior modeling. While it involves learning-based enhancement and effective embeddings, it is not a central match for your core research themes.
...
### 17. Large Language Models for the Summarization of Czech Documents: From History to the Present
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and text summarization, but it does not directly align with your focus on Information Retrieval (IR), query understanding, or ranking models. The use of Large Language Models for summarization is an interesting application, but it does not seem to be directly connected to your core research themes.
...
### 18. Multimodal Large Language Models with Adaptive Preference Optimization for Sequential Recommendation
...
- **Reason**: The paper focuses on sequential recommendation and multimodal large language models, which is somewhat related to your interests in information retrieval and natural language processing. However, the specific application domain and focus on recommendation systems, while tangentially related to your background in e-commerce, is not a central match for your primary research themes.
...
### 19. A Unified BERT-CNN-BiLSTM Framework for Simultaneous Headline Classification and Sentiment Analysis of Bangla News
...
- **Topic Keywords**: rag, search
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and deep semantic understanding, but it focuses on Bangla news headline classification and sentiment analysis, which is not a central match for the user's research themes in Information Retrieval (IR) and Search technologies.
...
### 20. Scalable Parameter-Light Spectral Method for Clustering Short Text Embeddings with a Cohesion-Based Evaluation Metric
...
- **Topic Keywords**: ctr
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and clustering, but it focuses on clustering short text embeddings, which is not a central match to the user's primary focus on information retrieval, especially in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 21. Learning to Reason: Training LLMs with GPT-OSS or DeepSeek R1 Reasoning Traces
...
- **Topic Keywords**: rag
- **Reason**: This paper explores the use of Large Language Models (LLMs) for reasoning tasks, specifically focusing on the generation of reasoning traces. While it touches on aspects of model performance and inference efficiency, it does not directly relate to the user's core research themes in Information Retrieval, Search technologies, or user behavior modeling. The connection to Natural Language Processing is present, but the paper's focus on LLMs and reasoning tasks is somewhat tangential to the user's interests.
...
### 22. Emotion-Enhanced Multi-Task Learning with LLMs for Aspect Category Sentiment Analysis
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Aspect Category Sentiment Analysis, which is a related topic to Information Retrieval, but it doesn't directly address query understanding, ranking models, or user behavior modeling. The use of LLMs and multi-task learning is relevant to NLP, but the paper's primary contribution is in sentiment analysis, not search technologies or real-time relevance optimization.
...
### 23. GraphMind: Theorem Selection and Conclusion Generation Framework with Dynamic GNN for LLM Reasoning
...
- **Topic Keywords**: rag
- **Reason**: The paper explores a novel framework for multi-step reasoning using large language models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on mathematical proving and theorem selection is not directly aligned with the user's core research themes. The connection to Natural Language Processing is relevant, but the paper's emphasis on structured reasoning and graph neural networks is not a central match for the user's interests.
...
### 24. Heterogeneous Multi-treatment Uplift Modeling for Trade-off Optimization in Short-Video Recommendation
...
- **Reason**: The paper focuses on recommender systems for short-video recommendations, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the specific application and approach differ significantly from your core research themes, and there is no apparent connection to query understanding, ranking models, or user behavior modeling.
...
### 25. RhinoInsight: Improving Deep Research through Control Mechanisms for Model Behavior and Context
...
- **Reason**: The paper discusses a framework for improving deep research using control mechanisms, which is somewhat related to information retrieval, particularly in the context of search technologies and query understanding. However, the focus on large language models and deep research tasks is not directly aligned with the user's primary interests in IR and e-commerce. The paper's relevance is mostly in the broader context of search and NLP, but it does not specifically address ranking models or user behavior modeling.
...
### 26. Robust Multimodal Sentiment Analysis with Distribution-Based Feature Recovery and Fusion
...
- **Topic Keywords**: search
- **Reason**: The paper focuses on multimodal sentiment analysis, which is somewhat related to query understanding and ranking models in Information Retrieval. However, it primarily deals with sentiment analysis and feature recovery, which is not a central match to the user's core research themes. The use of distribution-based feature recovery and fusion is an interesting aspect, but it does not directly relate to the user's interests in search technologies and user behavior modeling.
...
### 27. SCoTER: Structured Chain-of-Thought Transfer for Enhanced Recommendation
...
- **Reason**: This paper focuses on recommender systems, which is a related area to information retrieval. However, it primarily deals with Large Language Models (LLMs) and their application in recommender systems, which is not a central match to your interests in query understanding, ranking models, and user behavior modeling. While it touches on some related concepts like deep semantic understanding, the context is different and not as directly relevant to your core research themes.
...
### 28. Majority of the Bests: Improving Best-of-N via Bootstrapping
...
- **Topic Keywords**: search
- **Reason**: This paper is somewhat related to information retrieval and search technologies, but its focus on Large Language Models and selection mechanisms for discrete final answers is not directly aligned with the user's core research themes. While it touches on aspects of ranking models, the context is different from traditional Learning to Rank approaches. The paper's relevance to the user's interests is limited, but it may be of interest for those exploring the intersection of NLP and IR.
...
### 29. OpenGloss: A Synthetic Encyclopedic Dictionary and Semantic Knowledge Graph
...
- **Topic Keywords**: search
- **Reason**: This paper presents a synthetic encyclopedic dictionary and semantic knowledge graph, which is related to Natural Language Processing (NLP) and data mining. However, it does not directly align with your primary focus on Information Retrieval (IR), query understanding, ranking models, and user behavior modeling. The paper's emphasis on lexical resources and foundation models is somewhat relevant to your broader interests in NLP, but it does not strongly connect to your core research themes.
...
### 30. MultiBanAbs: A Comprehensive Multi-Domain Bangla Abstractive Text Summarization Dataset
...
- **Reason**: This paper focuses on Bangla abstractive text summarization, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. Although it involves NLP, the specific domain and task are not aligned with the user's interests.
...
### 31. From Pixels to Posts: Retrieval-Augmented Fashion Captioning and Hashtag Generation
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on fashion captioning and hashtag generation using a retrieval-augmented framework, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some NLP components like Large Language Model prompting, the context and application are quite different from the user's areas of focus.
...
### 32. Studying Maps at Scale: A Digital Investigation of Cartography and the Evolution of Figuration
...
- **Topic Keywords**: rag, search
- **Reason**: The paper focuses on large‑scale cartographic analysis using computer vision and semantic segmentation, which is peripheral to your core interests in IR, ranking, and click models. It offers limited relevance to search technologies or query understanding, though it does involve large‑scale data mining of map records.
...
### 33. Knowledge-based Graphical Method for Safety Signal Detection in Clinical Trials
...
- **Topic Keywords**: ltr
- **Reason**: This paper is not relevant to your core research themes in Information Retrieval, Search technologies, or Natural Language Processing. Although it involves data analysis and visualization, its focus on clinical trials and safety signal detection is outside your primary areas of interest.
...
### 34. Generating Reading Comprehension Exercises with Large Language Models for Educational Applications
...
- **Reason**: This paper is not relevant to your research interests as it focuses on generating reading comprehension exercises using large language models for educational applications, which is outside your primary focus on information retrieval, search technologies, and natural language processing in the context of e-commerce or real-time relevance optimization.
...
### 35. Cognitive Alpha Mining via LLM-Driven Code-Based Evolution
...
- **Topic Keywords**: rag, search
- **Reason**: This paper appears to be focused on a specific application of large language models (LLMs) in financial data analysis, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve some form of optimization and search, the context and methodology are quite different from the user's areas of focus.
...
### 36. AutoEnv: Automated Environments for Measuring Cross-Environment Agent Learning
...
- **Topic Keywords**: rag
- **Reason**: This paper appears to be unrelated to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus on cross-environment agent learning and environment-adaptive selection of learning methods seems to be more relevant to Artificial Intelligence and Machine Learning, which is not a primary area of interest for you.
...
### 37. Representational Stability of Truth in Large Language Models
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on the representational stability of truth in Large Language Models, which is a topic in Natural Language Processing. While it touches on the idea of semantic understanding, it is not directly related to information retrieval, search technologies, or query understanding, which are the core areas of your research interests.
...
### 38. A symbolic Perl algorithm for the unification of Nahuatl word spellings
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on a symbolic model for Nahuatl text unification, which is not directly related to Information Retrieval, Search technologies, or your other areas of interest. While it involves Natural Language Processing, the specific application and methodology are quite distinct from your core research themes.
...
### 39. How Learning Rate Decay Wastes Your Best Data in Curriculum-Based LLM Pretraining
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your core research interests in Information Retrieval, Search technologies, or query understanding. While it touches on optimization methods, it is focused on large language models and pretraining, which is not a primary area of interest for you.
...
### 40. A Recommender System Based on Binary Matrix Representations for Cognitive Disorders
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval and Search technologies, despite its use of a recommender system. The focus on cognitive disorder diagnosis and symptom analysis is outside the user's primary areas of interest, which include query understanding, ranking models, and user behavior modeling.
...
### 41. SWAN: Sparse Winnowed Attention for Reduced Inference Memory via Decompression-Free KV-Cache Compression
...
- **Topic Keywords**: rank
- **Reason**: This paper focuses on optimizing the memory usage of Large Language Models, specifically through a novel compression technique. While it touches on attention computation, it does not relate to query understanding, ranking models, or user behavior modeling, which are core areas of interest for your research. The paper's emphasis on NLP and LLMs does align with your broader interests in NLP, but it is not directly relevant to your primary focus on information retrieval.
...
### 42. A Reproducible Framework for Neural Topic Modeling in Focus Group Analysis
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on neural topic modeling in focus group analysis, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP techniques, the context and application are quite different from your areas of focus.
...
### 43. When and What to Recommend: Joint Modeling of Timing and Content for Active Sequential Recommendation
...
- **Topic Keywords**: recommend
- **Reason**: This paper focuses on sequential recommendation and active recommendation, which is somewhat related to information retrieval. However, it does not align with the user's primary focus on query understanding, ranking models, and user behavior modeling in the context of search technologies. The paper's emphasis on recommender systems and sequential recommendation does not strongly overlap with the user's interests in deep semantic understanding and real-time relevance optimization in information retrieval.
...
- **Reason**: The paper focuses on evaluating large language models on a mathematics exam, which is unrelated to information retrieval, ranking, or user behavior modeling. It does not address search technologies or NLP tasks relevant to the user’s core interests.
...
### 45. Context-Aware Whisper for Arabic ASR Under Linguistic Varieties
...
- **Reason**: This paper focuses on speech recognition for Arabic languages, which is outside your primary research interests in Information Retrieval and Search technologies, and related areas like NLP and data mining.
