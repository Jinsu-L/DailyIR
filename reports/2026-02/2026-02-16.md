# Daily Papers Report - 2026-02-16

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. DeepMTL2R: A Library for Deep Multi-task Learning to Rank
...
- **Topic Keywords**: ranking, learning to rank, relevance, rag, rank
- **Reason**: This paper aligns closely with your research interests in Information Retrieval, specifically Learning to Rank and multi-task learning. The framework DeepMTL2R is designed for optimizing multiple relevance criteria simultaneously, which is a key aspect of your research focus. The use of transformer architectures and self-attention mechanisms also demonstrates a deep semantic understanding, a key area of interest for you.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: DeepMTL2R – Transformer‑Based Open‑Source Framework for Multi‑Task Learning to Rank
- **Aim**: Provide a unified, scalable library that jointly optimizes multiple ranking objectives using a transformer backbone and 21 state‑of‑the‑art MTL algorithms, enabling practitioners to explore Pareto‑optimal trade‑offs.
- **Rationale**: Multi‑task ranking leverages diverse relevance signals (clicks, dwell time, sales, ratings) to improve overall ranking quality; transformers capture long‑range dependencies and higher‑order interactions, while a modular toolkit lowers the barrier to experiment with advanced MTL strategies.
- **Ground**: Architecture: transformer encoder treating list items as tokens, shared FC projection, multi‑head self‑attention, shared scoring head, modular preprocessing, training, evaluation, visualization. MTL algorithms: scalarization (linear, scale‑invariant), gradient‑manipulation (MGDA, GradDrop, PCGrad, LOG_MGDA, CAGrad, NashMTL, SDMGrad), Pareto‑front finding (Weighted Chebyshev, Soft WC, EPO, WC_MGDA, EC). Problem formulation: Pareto‑optimality across K tasks. Dataset: MSLR‑WEB30K with 5 relevance labels (primary + 4 auxiliary) constructed as multi‑label tasks. Experiments: 10 bi‑objective pairs, 10 reference vectors, listwise training, evaluation with NDCG@30, Δm%, Hypervolume Indicator. Findings: Pareto‑front methods (WC, LOG_MGDA) achieve superior NDCG and clearer trade‑offs; non‑PF methods cluster near center. Contributions: open‑source transformer‑based framework, 21 algorithms, modular pipelines, rich visualizations, systematic evaluation, practical guidance.
- **Experiment**: Bi‑objective experiments on MSLR‑WEB30K: 10 label pairs, 10 reference vectors, listwise training, evaluation of NDCG@30, Δm%, HVI; comparison of 21 MTL algorithms; visualization of loss trajectories and Pareto fronts.
- **Takeaway**: DeepMTL2R shows that transformer‑based shared representations combined with Pareto‑front and gradient‑based MTL techniques yield state‑of‑the‑art multi‑objective ranking performance, offering a practical, extensible platform for both research and industrial deployment.
...
### 2. Intent-Driven Dynamic Chunking: Segmenting Documents to Reflect Predicted Information Needs
...
- **Topic Keywords**: information retrieval, queries, rag, retrieval, search
...
#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Intent-Driven Dynamic Chunking (IDC) for document segmentation
- **Aim**: Align chunk boundaries with user intent to maximize retrieval recall and precision in QA and RAG pipelines
- **Rationale**: Static segmentation (fixed windows, paragraph breaks, coherence‑based) ignores the user’s information need, often splitting answers or adding noise, which degrades recall and precision, especially for long heterogeneous documents
- **Ground**: 1) Generate diverse intent queries per document using an LLM (Gemini 2.5 Flash/GPT‑4‑style) with top‑k sampling and redundancy pruning. 2) Encode sentences and queries into a shared 1536‑dim transformer space. 3) Use dynamic programming to find boundaries that maximize intent relevance while penalizing large or many chunks: U(S)=∑R(Cm)−λ∑|Cm|²−β(k−1). 4) Post‑process to merge short chunks or split overly long ones. 5) Store chunks in a vector index offline, no segmentation at query time.
- **Experiment**: Datasets: NewsQA, SQuAD 1‑doc, SQuAD 2‑doc, arXiv, Qasper, Fiori. Baselines: Fixed‑Length (6‑sent), Sliding Window (6‑sent, 50 % overlap), Coherence‑Based (TextTiling), Paragraph‑Based. Metrics: Recall@1, Recall@5, MRR, chunk count, answer coverage. Results: IDC outperforms all baselines on 5/6 datasets; relative gains 67 % (arXiv), 60 % (Fiori); matches best baseline on Qasper. Statistical significance: SQuAD 2‑doc p<0.05, Cohen’s d≈0.41. Chunk reduction: 53 % (arXiv), 42 % (Fiori). Cost: 1–2 s per short doc, 10–15 s for very long; API cost $0.0002–$0.005 per doc; query latency ≈500 ms.
- **Takeaway**: IDC reframes segmentation as intent‑driven, yielding higher recall, fewer chunks, lower indexing overhead, especially for long heterogeneous texts, while remaining computationally efficient and cost‑effective. Limitations include dependence on LLM quality and offline preprocessing; future work: domain‑specific intent generators, joint training with retrieval models, adaptive hyper‑parameters, real‑time re‑segmentation.
...
### 3. LLM-Confidence Reranker: A Training-Free Approach for Enhancing Retrieval-Augmented Generation Systems
...
- **Topic Keywords**: retriever, query, queries, ranking, rerank, relevance, rag, retrieval, rank, trec
...
### 4. Pailitao-VL: Unified Embedding and Reranker for Real-Time Multi-Modal Industrial Search
...
- **Topic Keywords**: ranking, rerank, listwise, pointwise, relevance, retrieval, commerce, e-commerce, rank, search
...
### 5. HyperRAG: Reasoning N-ary Facts over Hypergraphs for Retrieval Augmented Generation
...
### 6. PT-RAG: Structure-Fidelity Retrieval-Augmented Generation for Academic Papers
...
### 8. Learning User Interests via Reasoning and Distillation for Cross-Domain News Recommendation
...
### 9. DRAMA: Domain Retrieval using Adaptive Module Allocation
...
### 10. Orcheo: A Modular Full-Stack Platform for Conversational Search
...
### 11. LiveNewsBench: Evaluating LLM Web Search Capabilities with Freshly Curated News
...
### 12. Query as Anchor: Scenario-Adaptive User Representation via Large Language Model
...
### 13. Behavioral Feature Boosting via Substitute Relationships for E-commerce Search
...
- **Topic Keywords**: ranking, relevance, rag, click, commerce, e-commerce, rank, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of e-commerce search. The focus on behavioral feature boosting and substitute relationships aligns with your background in e-commerce and interest in query understanding and ranking models. However, the paper's specific application to cold-start products in e-commerce search is a somewhat narrow extension of your broader interests.
...
### 14. Differentially Private Retrieval-Augmented Generation
...
- **Topic Keywords**: rag, click, ctr, recommend, shopping, commerce, e-commerce
- **Reason**: The paper is somewhat related to your research interests in Information Retrieval, particularly in the context of recommender systems and user behavior modeling. However, it focuses more on CTR prediction and trigger-induced recommendation, which is a specific application rather than a core theme in your research. The use of deep learning and intent-aware models is also relevant to your interests in NLP and data mining.
...
### 17. A Multi-Agent Framework for Medical AI: Leveraging Fine-Tuned GPT, LLaMA, and DeepSeek R1 for Evidence-Based and Bias-Aware Clinical Query Processing
...
- **Reason**: This paper is somewhat related to information retrieval, particularly in the context of medical AI and question answering. However, it focuses more on leveraging large language models and multi-agent frameworks for evidence-based and bias-aware clinical query processing, which is not a central match to your core research themes in IR and search technologies.
...
### 18. InfoCIR: Multimedia Analysis for Composed Image Retrieval
...
### 19. Agentic Assistant for 6G: Turn-based Conversations for AI-RAN Hierarchical Co-Management
...
### 20. Tool-Aware Planning in Contact Center AI: Evaluating LLMs through Lineage-Guided Query Decomposition
...
### 21. High Precision Audience Expansion via Extreme Classification in a Two-Sided Marketplace
...
### 22. Whom to Query for What: Adaptive Group Elicitation via Multi-Turn LLM Interactions
...
### 23. Evaluating Prompt Engineering Techniques for RAG in Small Language Models: A Multi-Hop QA Approach
...
### 24. The Wikidata Query Logs Dataset
...
### 25. Climber-Pilot: A Non-Myopic Generative Recommendation Model Towards Better Instruction-Following
...
### 26. ADAB: Arabic Dataset for Automated Politeness Benchmarking -- A Large-Scale Resource for Computational Sociopragmatics
...
### 27. Text Style Transfer with Parameter-efficient LLM Finetuning and Round-trip Translation
...
### 28. Precedent-Informed Reasoning: Mitigating Overthinking in Large Reasoning Models via Test-Time Precedent Learning
...
### 29. A Tale of Two Graphs: Separating Knowledge Exploration from Outline Structure for Open-Ended Deep Research
...
### 30. StackingNet: Collective Inference Across Independent AI Foundation Models
...
### 31. A Geometric Analysis of Small-sized Language Model Hallucinations
...
### 32. Explainable Token-level Noise Filtering for LLM Fine-tuning Datasets
...
- **Reason**: This paper is somewhat related to your interests in Natural Language Processing (NLP) and deep semantic understanding, as it involves fine-tuning Large Language Models (LLMs) and optimizing their performance. However, it is not directly related to your core research themes in Information Retrieval (IR), query understanding, or ranking models, and the focus is more on dataset optimization rather than search technologies or user behavior modeling.
...
### 33. Parameter-Efficient Fine-Tuning of LLMs with Mixture of Space Experts
...
### 34. MixFormer: Co-Scaling Up Dense and Sequence in Industrial Recommenders
...
### 35. We can still parse using syntactic rules
...
- **Reason**: This paper is primarily focused on parsing and syntactic rules, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve NLP, the specific topic of parsing and syntactic rules is not a central match for your research themes.
...
### 36. Additive Control Variates Dominate Self-Normalisation in Off-Policy Evaluation
...
- **Topic Keywords**: ranking, rag, recommend, rank, nips
- **Reason**: This paper is primarily focused on off-policy evaluation and control variates, which is somewhat related to information retrieval and search technologies. However, the topic is more closely aligned with recommender systems and does not directly address query understanding, ranking models, or user behavior modeling, making it less relevant to the user's core research interests.
...
### 37. Neuromem: A Granular Decomposition of the Streaming Lifecycle in External Memory for LLMs
...
- **Reason**: This paper focuses on External Memory Modules for Large Language Models (LLMs), which is not directly related to Information Retrieval or Search technologies. While it does involve query formulation and retrieval, the context is specific to LLMs and does not align with the user's core research themes.
...
### 38. Hunt Globally: Deep Research AI Agents for Drug Asset Scouting in Investing, Business Development, and Search & Evaluation
...
- **Reason**: This paper is not directly related to the user's research interests in Information Retrieval, Search technologies, and Natural Language Processing. Although it involves AI agents and ranking models, the focus is on drug asset scouting in the biopharmaceutical industry, which is not a primary area of interest for the user.
...
### 39. Index Light, Reason Deep: Deferred Visual Ingestion for Visual-Dense Document Question Answering
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves multimodal document question answering, the focus is on visual-dense documents and a specific framework for deferred visual ingestion, which does not align with the user's interests in query understanding, ranking models, or user behavior modeling.
...
### 40. Open Rubric System: Scaling Reinforcement Learning with Pairwise Adaptive Rubric
...
- **Reason**: This paper appears to be primarily focused on reinforcement learning and reward modeling in the context of open-ended alignment, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it touches on the idea of 'rubrics' and 'judges', it doesn't seem to be relevant to query understanding, ranking models, or user behavior modeling, which are key areas of interest for the user.
...
### 41. GRRM: Group Relative Reward Modeling for Machine Translation
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves ranking models, the context is Machine Translation, which is not the primary focus of the user's research interests. The paper's relevance to recommender systems is also not a central match.
...
### 42. From Fluent to Verifiable: Claim-Level Auditability for Deep Research Agents
...
- **Reason**: This paper focuses on deep research agents and auditability in scientific reports, which is not directly related to information retrieval, search technologies, or user behavior modeling. Although it touches on NLP and data mining, the context is not aligned with the user's core research themes.
...
### 43. KorMedMCQA-V: A Multimodal Benchmark for Evaluating Vision-Language Models on the Korean Medical Licensing Examination
...
### 44. Testimole-Conversational: A 30-Billion-Word Italian Discussion Board Corpus (1996-2024) for Language Modeling and Sociolinguistic Research
...
- **Reason**: This paper appears to be primarily focused on creating a large corpus for language modeling and sociolinguistic research in the Italian language. While it touches on NLP applications, it does not seem to directly relate to the user's core research themes in Information Retrieval, query understanding, ranking models, or user behavior modeling.
...
### 45. Detecting LLM Hallucinations via Embedding Cluster Geometry: A Three-Type Taxonomy with Measurable Signatures
...
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves large language models, the focus is on detecting hallucinations in LLMs, which is a specific aspect of NLP, and does not align with your primary focus on query understanding, ranking models, and user behavior modeling.
...
### 46. Investigation for Relative Voice Impression Estimation
...
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus on voice impression estimation and speech analysis is outside your primary areas of interest.
...
### 47. Chain-of-Thought Reasoning with Large Language Models for Clinical Alzheimer's Disease Assessment and Diagnosis
...
### 48. GradMAP: Faster Layer Pruning with Gradient Metric and Projection Compensation
...
- **Comment**: 19 pages
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on layer pruning for Large Language Models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on model performance and efficiency, the context and methods are primarily from the deep learning and neural network pruning domain, making it less relevant to your research.
...
### 49. Measuring and Mitigating Post-hoc Rationalization in Reverse Chain-of-Thought Generation
...
### 50. Annotation-Efficient Vision-Language Model Adaptation to the Polish Language Using the LLaVA Framework
...
- **Topic Keywords**: rag, search
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves multimodal models, the focus is on vision-language models for low-resource languages, which is not a central match to your core research themes.
