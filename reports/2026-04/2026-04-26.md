# Daily Papers Report - 2026-04-26

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. ResRank: Unifying Retrieval and Listwise Reranking via End-to-End Joint Training with Residual Passage Compression

- **LLM Score**: 9
- **Keyword Score**: 20
- **Authors**: Xiaojie Ke, Shuai Zhang, Liansheng Sun, Yongjin Wang, Hengjun Jiang, Xiangkun Liu, Cunxin Gu, Jian Xu, Guanjun Jiang
- **URL**: <http://arxiv.org/abs/2604.22180v1>
- **Submitted**: 2026-04-24 03:11:51
- **Topic Keywords**: information retrieval, query, ranking, rerank, listwise, retrieval, rank, trec
- **Reason**: This paper presents a novel framework, ResRank, for information retrieval and listwise reranking, addressing challenges in large language model-based approaches. It combines retrieval and reranking through end-to-end joint training, achieving competitive or superior ranking effectiveness while reducing inference latency and computational complexity. The paper's focus on query understanding, ranking models, and real-time relevance optimization aligns well with the user's research interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: ResRank: A Unified, End‑to‑End Retrieval‑Reranking Framework
- **Aim**: Address the two main bottlenecks of LLM‑based listwise rerankers—lost‑in‑the‑middle attention degradation and super‑linear inference latency—by replacing the traditional concatenate‑then‑rank pipeline with a lightweight, two‑stage encoder‑reranker architecture that compresses passages into dense embeddings and scores them via cosine similarity.
- **Rationale**: By independently encoding each passage into a fixed‑dimensional vector, the model avoids long prompt sequences that dilute attention. The reranker then applies causal attention over the compressed list, re‑contextualizes each embedding, and fuses it residually with the original encoder output. A single global aggregation vector is used to compute relevance scores through cosine similarity, eliminating autoregressive decoding and its linear cost. Joint multi‑task training aligns the encoder and reranker, ensuring the compressed space remains query‑aware.
- **Ground**: Architecture: Encoder‑LLM (e.g., Qwen3‑Embedding‑4B) produces (e_i); Reranker‑LLM receives query, instruction, and sequence of (e_i) tokens; residual fusion (	ilde{e}_i = e_i + h_i^{reranker}); global vector (h_{EOS}) used for cosine similarity scoring. Training: Stage 1 – retrieval alignment via contrastive loss; Stage 2 – joint ranking with weighted listwise + retrieval loss. Innovations: residual passage compression, one‑step cosine similarity, dual‑stage joint training, end‑to‑end pipeline.
- **Experiment**: Benchmarks on TREC DL 2019/2020 and BEIR (8 datasets) show ResRank matches or surpasses RankGPT, RankVicuna, and E2Rank while reducing latency from >200 ms to ~30 ms per 10‑candidate list. Token efficiency improves from ~200 tokens per passage to 1 token, a 200× reduction. Ablation studies confirm residual fusion (3–5 % NDCG drop), cosine similarity (4–6 % drop), and joint training (2–4 % drop) are critical components.
- **Takeaway**: ResRank delivers state‑of‑the‑art retrieval‑reranking performance with dramatically lower inference latency and token consumption, making it a practical, scalable solution for industrial search systems.
...
- **Submitted**: 2026-04-24 17:18:56
- **Topic Keywords**: retriever, ranking, rag, retrieval, rank, search
- **Reason**: This paper aligns well with your research interests in Information Retrieval, particularly in query understanding and ranking models. The proposed Utility-Aligned Embeddings (UAE) framework merges the advantages of dense vector retrieval and utility-based approaches, which is a key area of focus in your research. The paper's emphasis on real-time relevance optimization and its application to the Retrieval-Augmented Generation (RAG) task also resonates with your interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Utility‑Aligned Embeddings (UAE) for Retrieval‑Augmented Generation
- **Aim**: Embed generative utility into bi‑encoder vectors so that ANN retrieval can be performed without LLM inference at query time.
- **Rationale**: Semantic similarity is a poor proxy for generative utility; utility is stochastic, heavy‑tailed, and expensive to compute, making existing LLM‑based re‑ranking slow and noisy.
- **Ground**: Two‑stage distillation: (A) train a cross‑encoder reward model with margin‑based pairwise loss to approximate LLM perplexity‑based utility; (B) train a bi‑encoder with utility‑modulated InfoNCE/KL loss to match the reward model’s ranking distribution, using utility‑gated hard‑negative mining and in‑batch encoding.
- **Experiment**: Evaluated on QASPER and NewsQA: UAE achieves Recall@1 48.15% (vs 59.84% for BGE‑Base), MAP 49.49% (vs 50.12%), Token F1 27.0 (vs 32.6), and latency 9 ms (vs 1696 ms). Ablation shows UAE outperforms dense retrievers while being ~180× faster.
- **Takeaway**: UAE delivers utility‑aligned embeddings that enable fast, high‑utility retrieval for RAG, achieving a sweet spot of performance and speed; scalable to millions of passages and suitable for production systems.
...
### 3. Can QPP Choose the Right Query Variant? Evaluating Query Variant Selection for RAG Pipelines

- **LLM Score**: 8
- **Keyword Score**: 19
- **Authors**: Negar Arabzadeh, Andrew Drozdov, Michael Bendersky, Matei Zaharia
- **URL**: <http://arxiv.org/abs/2604.22661v1>
- **Submitted**: 2026-04-24 15:36:40
- **Topic Keywords**: retriever, query, ranking, relevance, rag, retrieval, rank, trec
- **Reason**: This paper is highly relevant to Information Retrieval, specifically query understanding and ranking models, as it investigates query variant selection for Retrieval-Augmented Generation (RAG) pipelines. The focus on query performance prediction and its application in real-time relevance optimization aligns with your research interests. However, the specific domain of RAG pipelines is not directly related to your e-commerce background, which slightly reduces the score.
...
### 4. Bridging the Long-Tail Gap: Robust Retrieval-Augmented Relation Completion via Multi-Stage Paraphrase Infusion

- **LLM Score**: 8
- **Keyword Score**: 4
- **Authors**: Fahmida Alam, Mihai Surdeanu, Ellen Riloff
- **URL**: <http://arxiv.org/abs/2604.22261v1>
- **Submitted**: 2026-04-24 06:10:48
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper explores relation completion via retrieval-augmented generation, which aligns with your interests in Information Retrieval and query understanding. The use of paraphrases to expand lexical coverage and guide reasoning is particularly relevant to your focus on deep semantic understanding and real-time relevance optimization. However, the specific application to relation completion and language models may not be a central match to your core research themes.
...
- **URL**: <http://arxiv.org/abs/2604.22195v1>
- **Submitted**: 2026-04-24 03:47:12
- **Comment**: Accepted by SIGIR 2026
- **Topic Keywords**: recommend, acl
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of query understanding and ranking models. The focus on recommender systems and the critique of the current alignment-based approach aligns with your interests in real-time relevance optimization and deep semantic understanding. However, the paper's primary focus on recommender systems and the e-commerce domain is not a central match to your broader interests in Information Retrieval and Search technologies.
...
- **URL**: <http://arxiv.org/abs/2604.22193v1>
- **Submitted**: 2026-04-24 03:43:44
- **Comment**: Findings of ACL 2026
- **Topic Keywords**: rag
- **Reason**: This paper is highly relevant to your interests in Information Retrieval, particularly in the context of query understanding and ranking models. The focus on large language models and their ability to balance internal knowledge with external information, such as user and document assertions, aligns with your research themes. However, the paper's primary focus on NLP and LLMs is slightly outside your core e-commerce domain expertise.
...
- **URL**: <http://arxiv.org/abs/2604.22239v1>
- **Submitted**: 2026-04-24 05:28:51
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is relevant to information retrieval, specifically multi-document analytical QA, which involves extracting and synthesizing information across numerous documents. The proposed benchmark, MuDABench, and evaluation protocol are aligned with the user's interests in query understanding and ranking models. However, the focus on analytical QA and distant supervision is somewhat different from the user's primary focus on real-time relevance optimization and deep semantic understanding.
...
- **URL**: <http://arxiv.org/abs/2604.22294v1>
- **Submitted**: 2026-04-24 07:16:44
- **Comment**: 49 pages (14 main), preprint
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of question answering and scalable reasoning over long document sets. Although it focuses on question answering and structured reasoning, it touches on aspects of query understanding and real-time relevance optimization. However, it does not directly address your primary focus areas of ranking models and user behavior modeling.
...
### 9. CLARITY: A Framework and Benchmark for Conversational Language Ambiguity and Unanswerability in Interactive NL2SQL Systems
...
- **URL**: <http://arxiv.org/abs/2604.22313v1>
- **Submitted**: 2026-04-24 07:47:16
- **Comment**: Accepted at ACL 2026 (Industry Track)
- **Topic Keywords**: queries, user behavior
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and Information Retrieval (IR), particularly in the context of NL2SQL systems and conversational ambiguity. However, it does not directly address your core focus on query understanding, ranking models, and user behavior modeling. The paper's emphasis on ambiguity detection and resolution in NL2SQL systems is relevant, but it is not a central match for your research themes.
...
### 10. Dynamically Acquiring Text Content to Enable the Classification of Lesser-known Entities for Real-world Tasks

- **LLM Score**: 6
- **Keyword Score**: 2
- **Authors**: Fahmida Alam, Ellen Riloff
- **URL**: <http://arxiv.org/abs/2604.22325v1>
- **Submitted**: 2026-04-24 07:58:17
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and related topics, but it focuses more on text acquisition and classification for real-world tasks, which is not a central match for the user's primary focus on information retrieval and query understanding.
...
### 11. STEM: Structure-Tracing Evidence Mining for Knowledge Graphs-Driven Retrieval-Augmented Generation

- **LLM Score**: 4
- **Keyword Score**: 11
- **Authors**: Peng Yu, En Xu, Bin Chen, Haibiao Chen, Yinfei Xu
- **URL**: <http://arxiv.org/abs/2604.22282v1>
- **Submitted**: 2026-04-24 06:56:58
- **Comment**: 34 pages, 16 figures, accepted to ACL 2026 (Main Conference)
- **Topic Keywords**: query, queries, rag, retrieval, search
- **Reason**: This paper is somewhat related to the user's interests in Information Retrieval, particularly in the context of Knowledge Graph-based Question Answering. However, the focus on Knowledge Graphs and multi-hop reasoning is not directly aligned with the user's primary research themes of query understanding, ranking models, and user behavior modeling. While the paper touches on aspects of retrieval and generation, it is more focused on the Knowledge Graphs-driven approach, which is not a central match for the user's research interests.
...
### 12. BERAG: Bayesian Ensemble Retrieval-Augmented Generation for Knowledge-based Visual Question Answering

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Jinghong Chen, Jingbiao Mei, Guangyu Yang, Bill Byrne
- **URL**: <http://arxiv.org/abs/2604.22678v1>
- **Submitted**: 2026-04-24 16:01:19
- **Topic Keywords**: ranking, rag, retrieval, rank
- **Reason**: The paper discusses a novel approach to retrieval-augmented generation for knowledge-based visual question answering, which is somewhat related to information retrieval and query understanding. However, the focus on visual question answering and multimodal needle-in-a-haystack benchmarks limits its direct relevance to the user's core research themes in IR and search technologies.
...
### 13. Large Language Models Decide Early and Explain Later

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Ayan Datta, Zhixue Zhao, Bhuvanesh Verma, Radhika Mamidi, Mounika Marreddy, Alexander Mehler
- **URL**: <http://arxiv.org/abs/2604.22266v1>
- **Submitted**: 2026-04-24 06:26:24
- **Topic Keywords**: query, queries, rag
- **Reason**: This paper explores the behavior of Large Language Models, specifically when they determine their final answer during generation. While it touches on aspects of query understanding and real-time relevance optimization, it is primarily focused on the efficiency of chain-of-thought generation, which is not a central match to your research interests in Information Retrieval and Search technologies.
...
### 14. Lightweight Retrieval-Augmented Generation and Large Language Model-Based Modeling for Scalable Patient-Trial Matching
...
- **URL**: <http://arxiv.org/abs/2604.22061v1>
- **Submitted**: 2026-04-23 20:36:38
- **Comment**: 31 pages, 7 figures
- **Topic Keywords**: ctr, retrieval, sigir, trec
- **Reason**: This paper is somewhat related to your interests in Information Retrieval, particularly in the context of retrieval-augmented generation and large language model-based modeling. However, the focus on patient-trial matching and electronic health records (EHRs) is not directly aligned with your core research themes in e-commerce, query understanding, and ranking models.
...
### 15. ReLeVAnT: Relevance Lexical Vectors for Accurate Legal Text Classification

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Ishaan Gakhar, Harsh Nandwani
- **URL**: <http://arxiv.org/abs/2604.22292v1>
- **Submitted**: 2026-04-24 07:13:24
- **Comment**: 9 Pages, 2 figures
- **Topic Keywords**: relevance, rag, retrieval
- **Reason**: The paper focuses on legal text classification, which is somewhat related to information retrieval, but it doesn't directly address query understanding, ranking models, or user behavior modeling. The use of discriminative features and shallow neural networks is relevant to NLP, but the application to legal text classification is not a central match for the user's research interests.
...
### 16. Evaluating LLM-Based Goal Extraction in Requirements Engineering: Prompting Strategies and Their Limitations

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Anna Arnaudo, Riccardo Coppola, Maurizio Morisio, Flavio Giobergia, Andrea Bioddo, Angelo Bongiorno, Luca Dadone
- **URL**: <http://arxiv.org/abs/2604.22207v1>
- **Submitted**: 2026-04-24 04:22:17
...
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and its applications, but it does not directly align with the user's primary focus on Information Retrieval (IR), query understanding, ranking models, and user behavior modeling. The paper explores the use of Large Language Models (LLMs) for goal extraction in Requirements Engineering, which is a specific domain and task, but it does not address the user's core research themes.
...
### 17. An End-to-End Ukrainian RAG for Local Deployment. Optimized Hybrid Search and Lightweight Generation

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Mykola Trokhymovych, Yana Oliinyk, Nazarii Nyzhnyk
- **URL**: <http://arxiv.org/abs/2604.22095v1>
- **Submitted**: 2026-04-23 21:59:59
- **Comment**: To appear at UNLP'26
- **Topic Keywords**: rag, retrieval, search
- **Reason**: The paper discusses a Retrieval-Augmented Generation (RAG) system for Ukrainian document question answering, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on a specific language and deployment on resource-constrained hardware is not directly aligned with the user's core research themes.
...
### 18. Source-Modality Monitoring in Vision-Language Models

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Etha Tianze Hua, Tian Yun, Ellie Pavlick
- **URL**: <http://arxiv.org/abs/2604.22038v1>
- **Submitted**: 2026-04-23 19:49:36
...
- **Reason**: This paper explores source-modality monitoring in vision-language models, which is somewhat related to information retrieval, particularly in the context of multimodal search. However, the focus on vision-language models and the binding problem is not directly aligned with the user's core research themes in IR and search technologies. While it touches on aspects of query understanding and real-time relevance optimization, the connection is not strong enough to warrant a higher score.
...
### 19. Knowledge-driven Augmentation and Retrieval for Integrative Temporal Adaptation

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Weisi Liu, Guangzeng Han, Xiaolei Huang
- **URL**: <http://arxiv.org/abs/2604.22098v1>
- **Submitted**: 2026-04-23 22:12:18
- **Comment**: Accepted at ACL 2026
- **Topic Keywords**: rag, retrieval
- **Reason**: The paper discusses a method for temporal adaptation in machine learning, which is somewhat related to information retrieval, particularly in the context of real-time relevance optimization. However, the focus on knowledge integration and temporal shifts in classification tasks does not directly align with the user's primary interests in query understanding, ranking models, and user behavior modeling.
...
### 20. PrivUn: Unveiling Latent Ripple Effects and Shallow Forgetting in Privacy Unlearning

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Xiaoyi Chen, Haoyuan Wang, Siyuan Tang, Sijia Liu, Liya Su, XiaoFeng Wang, Haixu Tang
- **URL**: <http://arxiv.org/abs/2604.22076v1>
- **Submitted**: 2026-04-23 21:01:53
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is somewhat related to your interests in Information Retrieval and Natural Language Processing, as it deals with large language models and their privacy concerns. However, the focus on unlearning and forgetting in machine learning models is not directly aligned with your primary research themes of query understanding, ranking models, and user behavior modeling.
...
- **Submitted**: 2026-04-24 08:20:43
- **Comment**: Accepted at ACL 2026
- **Topic Keywords**: rag, personalization
- **Reason**: The paper explores personalization in Large Language Models, which is somewhat related to your interests in Information Retrieval and Natural Language Processing. However, the focus on interpretability and personalization in LLMs is not directly aligned with your primary research themes of query understanding, ranking models, and user behavior modeling.
...
### 22. Objective Shaping with Hard Negatives: Windowed Partial AUC Optimization for RL-based LLM Recommenders

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Wentao Shi, Qifan Wang, Chen Chen, Fei Liu, Dongfang Liu, Xu Liu, Wanli Ma, Junfeng Pan, Linhong Zhu, Fuli Feng
- **URL**: <http://arxiv.org/abs/2604.22504v1>
- **Submitted**: 2026-04-24 12:31:57
- **Comment**: 21 pages
- **Topic Keywords**: recommend, search
- **Reason**: The paper focuses on optimizing Large Language Model (LLM) recommenders using reinforcement learning, which is somewhat related to information retrieval and ranking models. However, the primary focus is on recommender systems, and the paper does not explicitly address query understanding, ranking models, or user behavior modeling. The connection to IR is indirect, making it only loosely relevant to the user's core research themes.
...
- **URL**: <http://arxiv.org/abs/2604.22565v1>
- **Submitted**: 2026-04-24 13:57:19
- **Topic Keywords**: recommend
- **Reason**: The paper focuses on improving the performance of Large Language Models (LLMs) by highlighting decisive evidence in long contexts. While it involves NLP and deep semantic understanding, it doesn't directly relate to information retrieval, query understanding, or ranking models, which are core areas of your research interests.
...
### 24. AgentSearchBench: A Benchmark for AI Agent Search in the Wild

- **LLM Score**: 2
- **Keyword Score**: 16
- **Authors**: Bin Wu, Arastun Mammadli, Xiaoyu Zhang, Emine Yilmaz
- **URL**: <http://arxiv.org/abs/2604.22436v1>
- **Submitted**: 2026-04-24 10:53:54
- **Topic Keywords**: queries, ranking, rerank, relevance, retrieval, rank, search
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves retrieval and reranking problems, the focus is on AI agent search, which is not a primary area of interest for the user. The paper's emphasis on execution signals and behavioral probing is also not aligned with the user's focus on semantic understanding and real-time relevance optimization.
...
### 25. Implementation and Privacy Guarantees for Scalable Keyword Search on SOLID-based Decentralized Data with Granular Visibility Constraints
...
- **URL**: <http://arxiv.org/abs/2604.22100v1>
- **Submitted**: 2026-04-23 22:16:17
- **Topic Keywords**: ranking, rank, search
- **Reason**: This paper focuses on decentralized keyword search in the context of Solid-based data ecosystems, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on search and ranking, the emphasis is on decentralized architectures and privacy guarantees, which does not align with the user's primary interests.
...
### 26. SHAPE: Unifying Safety, Helpfulness and Pedagogy for Educational LLMs

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Sihang, Zhao, Kangrui Yu, Youliang Yuan, Pinjia He, Hongyi Wen
- **URL**: <http://arxiv.org/abs/2604.22134v1>
- **Submitted**: 2026-04-24 00:39:08
- **Comment**: ACL 2026 Main
- **Topic Keywords**: queries, search
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves Large Language Models, the focus is on educational applications and pedagogical aspects, which is not a primary area of interest for the user.
...
### 27. How Do AI Agents Spend Your Money? Analyzing and Predicting Token Consumption in Agentic Coding Tasks
...
- **Submitted**: 2026-04-24 17:54:47
- **Topic Keywords**: rag, search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus on AI agents' token consumption in agentic coding tasks is outside your primary areas of interest.
...
### 28. ASPIRE: Make Spectral Graph Collaborative Filtering Great Again via Adaptive Filter Learning

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Yunhang He, Cong Xu, Zhangchi Zhu, Hongzhi Yin, Wei Zhang
- **URL**: <http://arxiv.org/abs/2604.22549v1>
- **Submitted**: 2026-04-24 13:39:51
- **Topic Keywords**: ctr, recommend
- **Reason**: This paper focuses on spectral collaborative filtering and graph neural networks, which is somewhat related to information retrieval, but the primary focus is on recommender systems rather than search technologies or query understanding. The paper's emphasis on deep learning and graph neural networks is also more aligned with NLP and data mining, but the specific application to collaborative filtering is not directly relevant to the user's core research themes.
...
### 29. CNSL-bench: Benchmarking the Sign Language Understanding Capabilities of MLLMs on Chinese National Sign Language

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Rui Zhao, Xuewen Zhong, Xiaoyun Zheng, Jinsong Su, Yidong Chen
- **URL**: <http://arxiv.org/abs/2604.22367v1>
- **Submitted**: 2026-04-24 08:59:33
- **Comment**: Accepted as the Main Conference at ACL 2026
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on sign language understanding and benchmarking large language models, which is outside your primary research interests in Information Retrieval and Search technologies. While it involves multimodal modeling, it is not directly related to your core areas of query understanding, ranking models, or user behavior modeling.
...
### 30. Context-Fidelity Boosting: Enhancing Faithful Generation through Watermark-Inspired Decoding

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Weixu Zhang, Fanghua Ye, Qiang Gao, Jian Li, Haolun Wu, Yuxing Tian, Sijing Duan, Nan Du, Xiaolong Li, Xue Liu
- **URL**: <http://arxiv.org/abs/2604.22335v1>
- **Submitted**: 2026-04-24 08:07:58
- **Comment**: Accepted at ACL 2026
- **Topic Keywords**: relevance
- **Reason**: This paper focuses on improving the faithfulness of large language models, which is a topic in Natural Language Processing (NLP). However, it does not directly relate to Information Retrieval (IR), query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
...
### 31. Recognition Without Authorization: LLMs and the Moral Order of Online Advice

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Tom van Nuenen
- **URL**: <http://arxiv.org/abs/2604.22143v1>
- **Submitted**: 2026-04-24 01:19:39
- **Topic Keywords**: rag, recommend
- **Reason**: This paper appears to be primarily focused on the social implications of Large Language Models (LLMs) in online communities, specifically their advisory defaults and moral interactions. While it touches on the idea of 'recognition' which could be loosely related to query understanding or user behavior modeling, the paper's core themes and methodologies are not aligned with your research interests in Information Retrieval, Search technologies, or Natural Language Processing.
...
### 32. Zero-Shot Morphological Discovery in Low-Resource Bantu Languages via Cross-Lingual Transfer and Unsupervised Clustering

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Hillary Mutisya, John Mugane
- **URL**: <http://arxiv.org/abs/2604.22723v1>
- **Submitted**: 2026-04-24 17:21:00
- **Topic Keywords**: rag
- **Reason**: This paper focuses on morphological discovery in low-resource Bantu languages, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the specific application and methodology are quite different from your areas of focus.
...
### 33. Thinking Without Words: Efficient Latent Reasoning with Abstract Chain-of-Thought

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Keshav Ramji, Tahira Naseem, Ramón Fernandez Astudillo
- **URL**: <http://arxiv.org/abs/2604.22709v1>
- **Submitted**: 2026-04-24 16:45:51
- **Topic Keywords**: rag
- **Reason**: This paper focuses on developing a post-training mechanism for efficient latent reasoning in language models, which is not directly related to information retrieval, search technologies, or user behavior modeling. While it involves natural language processing, the primary goal is to improve reasoning capabilities in language models, rather than addressing query understanding, ranking models, or click models.
...
- **URL**: <http://arxiv.org/abs/2604.22517v1>
- **Submitted**: 2026-04-24 12:56:54
- **Comment**: ACL 2026 Industry Track (Oral)
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your core research themes in Information Retrieval and Search technologies. Although it involves Natural Language Processing (NLP) and data analysis, its focus on business idea evaluation and expert disagreement does not align with your interests in query understanding, ranking models, and user behavior modeling.
...
### 35. Measuring and Mitigating Persona Distortions from AI Writing Assistance

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Paul Röttger, Kobi Hackenburg, Hannah Rose Kirk, Christopher Summerfield
- **URL**: <http://arxiv.org/abs/2604.22503v1>
- **Submitted**: 2026-04-24 12:31:11
- **Comment**: For supplementary information, code, and data see https://github.com/paul-rottger/ai-distortion
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or your primary focus on deep semantic understanding and real-time relevance optimization. While it involves AI and user behavior, the context is AI writing assistance and persona distortions, which is not a central match to your research interests.
...
- **URL**: <http://arxiv.org/abs/2604.22209v1>
- **Submitted**: 2026-04-24 04:26:04
- **Comment**: Accepted to ACL 2026 main conference (oral)
- **Topic Keywords**: rag
- **Reason**: This paper is primarily focused on generative audio modeling, which is not a core area of interest for you. While it does involve text instructions and some level of semantic understanding, the application domain is quite different from your expertise in information retrieval and search technologies.
...
### 37. ReCast: Recasting Learning Signals for Reinforcement Learning in Generative Recommendation

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Peiyan Zhang, Hanmo Liu, Chengxuan Tong, Yuxia Wu, Wei Guo, Yong Liu
- **URL**: <http://arxiv.org/abs/2604.22169v1>
- **Submitted**: 2026-04-24 02:44:36
- **Topic Keywords**: recommend, search
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval and Search technologies, as it focuses on Generative Recommendation and Reinforcement Learning. While it involves learning signals and optimization, the context is distinct from the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 38. PermaFrost-Attack: Stealth Pretraining Seeding(SPS) for planting Logic Landmines During LLM Training

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Harsh Kumar, Rahul Maity, Tanmay Joshi, Aman Chadha, Vinija Jain, Suranjana Trivedy, Amitava Das
- **URL**: <http://arxiv.org/abs/2604.22117v1>
- **Submitted**: 2026-04-23 23:32:36
- **Topic Keywords**: ctr
- **Reason**: This paper appears to be focused on adversarial manipulation of large language models, which is not directly related to your core research interests in Information Retrieval and Search technologies, query understanding, ranking models, and user behavior modeling. While it touches on the topic of model behavior, the context and methodology are distinct from your areas of expertise.
...
### 39. Representational Harms in LLM-Generated Narratives Against Global Majority Nationalities

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Ilana Nguyen, Harini Suresh, Thema Monroe-White, Evan Shieh
- **URL**: <http://arxiv.org/abs/2604.22749v1>
- **Submitted**: 2026-04-24 17:49:09
- **Comment**: FAccT '26, June 25-28, 2026, Montreal, QC, Canada
- **Topic Keywords**: search
- **Reason**: This paper focuses on the representation of nationalities in LLM-generated narratives, which is not directly related to information retrieval, search technologies, or query understanding. While it touches on NLP, the context is more about bias and representation rather than deep semantic understanding or real-time relevance optimization.
...
### 40. Using Embedding Models to Improve Probabilistic Race Prediction

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Noan Dasanaike, Kosuke Imai
- **URL**: <http://arxiv.org/abs/2604.22555v1>
- **Submitted**: 2026-04-24 13:49:49
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text embeddings, the focus is on predicting racial disparity using surname and first-name data, which is unrelated to your core research themes.
...
### 41. Tell Me Why: Designing an Explainable LLM-based Dialogue System for Student Problem Behavior Diagnosis

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Zhilin Fan, Deliang Wang, Penghe Chen, Yu Lu
- **URL**: <http://arxiv.org/abs/2604.22237v1>
- **Submitted**: 2026-04-24 05:23:25
- **Comment**: This paper has been accepted in AIED2026
- **Topic Keywords**: recommend
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval and Search technologies. While it involves a large language model (LLM), the focus is on explainability in educational dialogue systems, which is outside your primary areas of interest.
...
### 42. TTS-PRISM: A Perceptual Reasoning and Interpretable Speech Model for Fine-Grained Diagnosis
...
- **URL**: <http://arxiv.org/abs/2604.22225v1>
- **Submitted**: 2026-04-24 05:01:55
- **Comment**: Submitted to Interspeech 2026
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests as it focuses on text-to-speech models and fine-grained diagnosis of acoustic artifacts, which is outside the scope of information retrieval, search technologies, and natural language processing.
...
### 43. Sharpness-Aware Poisoning: Enhancing Transferability of Injective Attacks on Recommender Systems

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Junsong Xie, Yonghui Yang, Pengyang Shao, Le Wu
- **URL**: <http://arxiv.org/abs/2604.22170v1>
- **Submitted**: 2026-04-24 02:47:22
- **Topic Keywords**: recommend
- **Reason**: This paper focuses on recommender systems and injective attacks, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the paper's primary focus on recommender systems and attack transferability does not align with your core research themes, particularly in the areas of query understanding, ranking models, and user behavior modeling.
...
### 44. Fine-Grained Analysis of Shared Syntactic Mechanisms in Language Models

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Ryoma Kumon, Hitomi Yanaka
- **URL**: <http://arxiv.org/abs/2604.22166v1>
- **Submitted**: 2026-04-24 02:29:37
- **Comment**: Accepted to ACL 2026 Main
- **Topic Keywords**: search
- **Reason**: This paper focuses on the analysis of language models' syntactic mechanisms, which is a topic within Natural Language Processing (NLP). However, it does not appear to have any direct connection to Information Retrieval (IR), Search technologies, or query understanding, which are the primary areas of interest. The paper's focus on linguistic principles and model interpretability does not align with the user's core research themes.
...
### 45. Universal Transformers Need Memory: Depth-State Trade-offs in Adaptive Recursive Reasoning

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Grigory Sapunov
- **URL**: <http://arxiv.org/abs/2604.21999v1>
- **Submitted**: 2026-04-23 18:30:01
- **Comment**: 12 pages, 7 figures, 8 tables. Code: https://github.com/che-shr-cat/utm-jax
- **Topic Keywords**: recommend
- **Reason**: This paper focuses on the Universal Transformer architecture and its application to a combinatorial reasoning benchmark, which is not directly related to Information Retrieval, Search technologies, or user behavior modeling. While it explores deep semantic understanding, the context is different from the user's primary research interests.
