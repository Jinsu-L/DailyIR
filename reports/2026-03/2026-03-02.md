...
### 1. Beyond Global Similarity: Towards Fine-Grained, Multi-Condition Multimodal Retrieval

- **LLM Score**: 8
- **Keyword Score**: 21
- **Authors**: Xuan Lu, Kangle Li, Haohang Huang, Rui Meng, Wenjun Zeng, Xiaoyu Shen
- **URL**: <http://arxiv.org/abs/2603.01082v1>
- **Submitted**: 2026-03-01 12:53:47
- **Comment**: Accepted by CVPR 2026
- **Topic Keywords**: retriever, query, queries, rerank, pointwise, relevance, retrieval, rank
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of multimodal retrieval and query understanding. The focus on fine-grained, multi-condition cross-modal retrieval under natural-language queries aligns with your interests in deep semantic understanding and real-time relevance optimization. The use of large-scale benchmarks and MLLM-based multimodal retrievers also resonates with your background in e-commerce and NLP.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Multimodal Compositional Retrieval (MCMR) benchmark for e‑commerce
- **Aim**: 1) quantify complementary value of image and text modalities in retrieval; 2) benchmark a broad set of state‑of‑the‑art multimodal retrievers and rerankers; 3) provide a fully automated, modality‑aware pipeline for generating realistic, leakage‑free cross‑modal queries
- **Rationale**: Real‑world e‑commerce search requires queries that fuse visual and textual constraints; existing benchmarks lack realistic cross‑modal queries and clear modality isolation. MCMR addresses this by enforcing strict separation of visual and textual evidence and by evaluating retrieval performance under fused, text‑only, and image‑only regimes
- **Ground**: Evaluation protocol uses Recall@K, MRR, NDCG@K (K∈{1,5,10,50}); three visibility regimes (fused, text‑only, image‑only) with baseline results (Recall@1 22% fused, 2% text‑only, 20% image‑only; MRR 29.09% fused, 6.54% text‑only, 26.04% image‑only). Cross‑modal constraint ablation varies k_T=k_I∈{1,…,5} for five retrievers (CORAL, GME, LamRA, LLaVE, MM‑Embed) showing monotonic gains. 15 multimodal models (retrievers and rerankers) evaluated, including instruction‑tuned variants (e.g., GME‑Qwen2‑VL‑7B‑Instruct, Qwen‑3‑VL‑8B‑Instruct).
- **Experiment**: Five‑step automated pipeline: 1) image‑side attribute extraction (category + 4–10 objective descriptors); 2) text‑side description generation (80–120 words, strict phrasing); 3) universal leakage checker; 4) query generation (2–3 image tags + 2–3 text facts, first‑person tone); 5) query check (balanced modality coverage, valid phrasing). Pointwise scoring used for reranking. Annotation protocol for Step 1 ensures objective descriptors and category consistency. Ablation studies confirm richer constraints improve metrics. Model evaluation reports Recall@K, MRR, NDCG@K across regimes.
- **Takeaway**: MCMR provides a rigorous benchmark that quantifies the complementary value of visual and textual modalities, demonstrates monotonic performance gains with richer cross‑modal constraints, establishes strong baselines across 15 multimodal models, and supplies a fully automated, leakage‑free query‑generation pipeline that enables reproducible training and evaluation for future multimodal compositional retrieval research.
...
### 2. ReFeed: Retrieval Feedback-Guided Dataset Construction for Style-Aware Query Rewriting

- **LLM Score**: 8
- **Keyword Score**: 13
- **Authors**: Jiyoon Myung, Jungki Son, Kyungro Lee, Jihyeon Park, Joohyung Han
- **URL**: <http://arxiv.org/abs/2603.01417v1>
- **Submitted**: 2026-03-02 03:43:53
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query rewriting and style-aware query reformulation. The use of retrieval feedback and large language models to rewrite queries in the style of relevant documents aligns with your focus on deep semantic understanding and real-time relevance optimization. While the e-commerce domain is not explicitly mentioned, the paper's emphasis on domain-specific contexts and real-world data distributions suggests a broader applicability.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: ReFeed: Retrieval‑Feedback‑Guided Dataset Construction for Style‑Aware Query Rewriting
- **Aim**: Construct a high‑precision corpus of (original, rewritten) query pairs that preserve semantics while aligning with the linguistic style of target documents, enabling effective query rewriting for IR and Retrieval‑Augmented Generation (RAG) systems.
- **Rationale**: Conventional IR systems struggle when user queries differ in style or structure from target corpora, and existing rewriting methods focus only on semantic equivalence, neglecting style. Retrieval outcomes can serve as weak supervision to automatically curate style‑aware rewrites, improving recall and RAG reliability.
- **Ground**: Data‑centric pipeline: (1) Initial retrieval to identify failures; (2) LLM‑guided rewriting conditioned on retrieved negatives and the correct document’s style; (3) Verification via re‑retrieval to filter effective rewrites; (4) Assembly of verified pairs into a reusable dataset. Implemented on SQuAD v1.1 (87k Q/A), using dense embeddings (e5‑base‑v2/BGE‑large) and GPT‑5 for rewriting. 16k misses (18.7%) were processed; 67.5% yielded valid rewrites, producing 11,044 verified pairs annotated with retrieval metadata.
- **Experiment**: Pipeline evaluated on SQuAD v1.1: retriever returns top‑3 documents; GPT‑5 rewrites queries; verification accepts pairs where the correct passage appears in top‑3 after rewriting. Results show 11,044 verified pairs; few‑shot prompting with these pairs improves retrieval recall and RAG accuracy, especially for stylistically divergent or ambiguous queries. Gains are limited for queries already aligned with target style.
- **Takeaway**: Retrieval‑feedback can be leveraged to build a reusable, style‑aware query‑rewriting dataset that supports both few‑shot prompting and supervised fine‑tuning, leading to improved retrieval performance and downstream RAG quality. Future work includes selective rewriting, industrial RAG deployment, training compact models, and extending to diverse domains.
...
- **URL**: <http://arxiv.org/abs/2603.01425v1>
- **Submitted**: 2026-03-02 04:11:18
- **Comment**: Under Review
- **Topic Keywords**: retriever, dense retrieval, retrieval
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of dense retrieval and query understanding. The proposed LaSER framework internalizes explicit reasoning into the latent space of dense retrievers, which aligns with your focus on deep semantic understanding and real-time relevance optimization. However, the paper's focus on dense retrieval and LLMs is more specific than your broader interests in IR and NLP.
...
- **URL**: <http://arxiv.org/abs/2603.01710v1>
- **Submitted**: 2026-03-02 10:34:28
- **Comment**: 13 pages, 3 figures, 4 tables
- **Topic Keywords**: information retrieval, rag, retrieval
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of Real-Time Relevance Optimization. The focus on Retrieval and Reasoning Models in the legal domain aligns with your background in e-commerce and interest in deep semantic understanding. However, the specific application to the legal domain is somewhat outside your primary focus.
...
### 5. PhotoBench: Beyond Visual Matching Towards Personalized Intent-Driven Photo Retrieval
...
- **Submitted**: 2026-03-02 06:02:40
- **Comment**: Under review
- **Topic Keywords**: queries, retrieval
- **Reason**: This paper is highly relevant to Information Retrieval, particularly in the context of personalized and intent-driven photo retrieval. The focus on multi-source reasoning, visual semantics, and spatial-temporal metadata aligns with your interests in query understanding and ranking models. However, the specific domain of photo retrieval is somewhat distinct from your e-commerce background, which prevents a perfect match.
...
- **URL**: <http://arxiv.org/abs/2603.01590v1>
- **Submitted**: 2026-03-02 08:16:49
- **Topic Keywords**: ranking, rag, click, ctr, click-through rate, recommend, rank
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the area of click-through rate (CTR) prediction and ranking models. However, the focus on multimodal large language models and cold-start settings in advertising and recommendation systems is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 7. Multi-Sourced, Multi-Agent Evidence Retrieval for Fact-Checking

- **LLM Score**: 6
- **Keyword Score**: 6
- **Authors**: Shuzhi Gong, Richard O. Sinnott, Jianzhong Qi, Cecile Paris, Preslav Nakov, Zhuohan Xie
- **URL**: <http://arxiv.org/abs/2603.00267v1>
- **Submitted**: 2026-02-27 19:29:01
- **Topic Keywords**: rag, retrieval augmented generation, retrieval
- **Reason**: The paper explores evidence retrieval for fact-checking, which is related to information retrieval and query understanding. However, the focus on fact-checking and knowledge graph evidence is somewhat tangential to the user's primary interests in search technologies, ranking models, and user behavior modeling. The paper's emphasis on deep semantic understanding and real-time relevance optimization is relevant, but not a central match.
...
### 8. LaSTR: Language-Driven Time-Series Segment Retrieval
...
- **URL**: <http://arxiv.org/abs/2603.00725v1>
- **Submitted**: 2026-02-28 16:15:02
- **Topic Keywords**: retriever, query, ranking, retrieval, rank, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in query understanding and ranking models. However, the focus on time-series data and segment retrieval is not a central match to your primary interests in e-commerce and deep semantic understanding. The use of NLP and contrastive retrieval is relevant, but the application is specific to time-series data.
...
### 9. Tiny-Critic RAG: Empowering Agentic Fallback with Parameter-Efficient Small Language Models
...
- **Submitted**: 2026-03-01 00:16:31
- **Topic Keywords**: ltr, rag, retrieval, rank
- **Reason**: The paper discusses a novel approach to Retrieval-Augmented Generation (RAG) using a parameter-efficient Small Language Model (SLM) for ultra-low latency binary routing. While it touches on aspects of query understanding and ranking models, the primary focus is on improving the efficiency of RAG frameworks, which is somewhat related to information retrieval but not a central match for your research interests.
...
### 10. Super Research: Answering Highly Complex Questions with Large Language Models through Super Deep and Super Wide Research

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Yubo Dong, Nianhao You, Yuxuan Hou, Zixun Sun, Yue Zhang, Hehe Fan, Liang Zhang, Siyuan Zhao, Linyi
- **URL**: <http://arxiv.org/abs/2603.00582v1>
- **Submitted**: 2026-02-28 10:22:09
- **Topic Keywords**: queries, rag, retrieval, search
- **Reason**: The paper explores the capabilities of Large Language Models (LLMs) in complex research tasks, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the focus on LLMs and complex question-answering is not directly aligned with your primary research themes, and the paper does not explicitly address query understanding, ranking models, or user behavior modeling.
...
- **URL**: <http://arxiv.org/abs/2603.01281v1>
- **Submitted**: 2026-03-01 21:35:09
- **Comment**: Accepted to ICLR 2026 (Poster, Top 4%)
- **Topic Keywords**: query, rag, ctr
- **Reason**: The paper explores attention steering techniques, which is somewhat related to query understanding in Information Retrieval. However, the focus on prompt highlighting and spectral decomposition does not directly align with the user's primary interests in ranking models and user behavior modeling.
...
### 12. The Synthetic Web: Adversarially-Curated Mini-Internets for Diagnosing Epistemic Weaknesses of Language Agents

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Shrey Shah, Levent Ozgur
- **URL**: <http://arxiv.org/abs/2603.00801v1>
- **Submitted**: 2026-02-28 20:27:44
- **Comment**: Submitted to ICML 2026, currently under review
- **Topic Keywords**: ranking, retrieval, rank, search
- **Reason**: The paper discusses the robustness of language agents to adversarial ranking, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on language agents and epistemically humble agents is not directly aligned with the user's primary research interests in IR and NLP. The paper's emphasis on evaluating mitigation strategies under adversarial ranking is somewhat relevant, but it does not directly address the user's core research themes.
...
### 13. SODA: Semantic-Oriented Distributional Alignment for Generative Recommendation
...
- **URL**: <http://arxiv.org/abs/2603.00700v1>
- **Submitted**: 2026-02-28 15:18:28
- **Topic Keywords**: ranking, rag, recommend, rank
- **Reason**: The paper focuses on generative recommendation, which is somewhat related to information retrieval, but it doesn't directly address query understanding, ranking models, or user behavior modeling. The use of distributional alignment and Bayesian Personalized Ranking is relevant to NLP, but the primary application is in recommendation systems, which is not the user's primary focus.
...
### 14. ProtRLSearch: A Multi-Round Multimodal Protein Search Agent with Large Language Models Trained via Reinforcement Learning

- **LLM Score**: 4
- **Keyword Score**: 6
- **Authors**: Congying Liu, Taihao Li, Ming Huang, Xingyuan Wei, Peipei Liu, Yiqing Shen, Yanxu Mao, Tiehan Cui
- **URL**: <http://arxiv.org/abs/2603.01464v1>
- **Submitted**: 2026-03-02 05:25:41
- **Topic Keywords**: query, rag, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, but it focuses on a specific domain (protein analysis in healthcare) and uses multimodal inputs (protein sequence and text) which is not a central match to your core research themes. While it involves search agents and ranking models, the context and application are quite different from your typical research areas.
...
### 15. Efficient Extractive Summarization with MAMBA-Transformer Hybrids for Low-Resource Scenarios

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Nisrine Ait Khayi
- **URL**: <http://arxiv.org/abs/2603.01288v1>
- **Submitted**: 2026-03-01 21:45:13
- **Topic Keywords**: relevance, rag
- **Reason**: This paper focuses on extractive summarization, which is somewhat related to information retrieval, but it does not directly address query understanding, ranking models, or user behavior modeling. The use of transformers and state space models is relevant to NLP, but the application is limited to summarization, which is not a central interest. The paper's focus on low-resource scenarios and summarization quality is not directly aligned with the user's primary research themes.
...
### 16. Beyond the Flat Sequence: Hierarchical and Preference-Aware Generative Recommendations
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of recommender systems. However, it focuses more on generative recommendations and user behavior modeling in a specific domain, rather than query understanding, ranking models, or real-time relevance optimization.
...
### 17. RLAR: An Agentic Reward System for Multi-task Reinforcement Learning on Large Language Models

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Andrew Zhuoer Feng, Cunxiang Wang, Bosi Wen, Yidong Wang, Yu Luo, Hongning Wang, Minlie Huang
- **URL**: <http://arxiv.org/abs/2603.00724v1>
- **Submitted**: 2026-02-28 16:14:43
- **Comment**: 25 pages, 7 figures
- **Topic Keywords**: queries, rag
- **Reason**: The paper presents a reinforcement learning framework for large language models, focusing on reward function quality and generalization. While it touches on query understanding and dynamic reward assignment, its primary focus is on language model alignment and multi-task reinforcement learning, which is somewhat related to information retrieval but not a central match for your research interests.
...
### 18. RAIE: Region-Aware Incremental Preference Editing with LoRA for LLM-based Recommendation

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Jin Zeng, Yupeng Qi, Hui Li, Chengming Li, Ziyu Lyu, Lixin Cui, Lu Bai
- **URL**: <http://arxiv.org/abs/2603.00638v1>
- **Submitted**: 2026-02-28 13:12:38
- **Topic Keywords**: pointwise, recommend, rank
- **Reason**: The paper discusses a recommendation system framework, RAIE, which addresses preference drift in real-world scenarios. While it involves deep semantic understanding and incremental updates, its primary focus is on recommender systems rather than information retrieval. The paper's relevance to your research interests is somewhat related, but not a central match.
...
### 19. Reconstructing Content via Collaborative Attention to Improve Multimodal Embedding Quality

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Jiahan Chen, Da Li, Hengran Zhang, Yinqiong Cai, Lixin Su, Jiafeng Guo, Daiting Shi, Dawei Yin, Keping Bi
- **URL**: <http://arxiv.org/abs/2603.01471v1>
- **Submitted**: 2026-03-02 05:34:45
- **Topic Keywords**: rag, retrieval
- **Reason**: The paper discusses multimodal embedding models and proposes a new pre-training paradigm, CoCoA, to improve embedding quality. While it touches on retrieval tasks, the focus is on multimodal embedding optimization, which is somewhat related to information retrieval but not a central match for your research interests.
...
### 20. Individual Turing Test: A Case Study of LLM-based Simulation Using Longitudinal Personal Data

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Minghao Guo, Ziyi Ye, Wujiang Xu, Xi Zhu, Wenyue Hua, Dimitris N. Metaxas
- **URL**: <http://arxiv.org/abs/2603.01289v1>
- **Submitted**: 2026-03-01 21:46:27
- **Comment**: 5 pages, 2 figures
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper explores the capabilities of Large Language Models (LLMs) in simulating individual behavior, which is somewhat related to query understanding and user behavior modeling in Information Retrieval. However, the focus on individual simulation and longitudinal personal data is not directly aligned with the user's core research themes in IR and Search technologies.
...
### 21. When Metrics Disagree: Automatic Similarity vs. LLM-as-a-Judge for Clinical Dialogue Evaluation

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Bian Sun, Zhenjian Wang, Orvill de la Torre, Zirui Wang
- **URL**: <http://arxiv.org/abs/2603.00314v1>
- **Submitted**: 2026-02-27 21:09:43
- **Topic Keywords**: queries, search
- **Reason**: This paper is somewhat related to your interests in NLP and deep semantic understanding, but it focuses on clinical dialogue evaluation and the reliability of large language models in healthcare settings, which is not a central match to your primary research focus on information retrieval and search technologies.
...
### 22. A Comprehensive Evaluation of LLM Unlearning Robustness under Multi-Turn Interaction

- **LLM Score**: 4
- **Keyword Score**: 3
- **Authors**: Ruihao Pan, Suhang Wang
- **URL**: <http://arxiv.org/abs/2603.00823v1>
- **Submitted**: 2026-02-28 22:01:31
- **Topic Keywords**: query
- **Reason**: This paper focuses on machine unlearning in large language models, which is somewhat related to information retrieval and search technologies. However, the primary topic of unlearning and its evaluation in interactive environments does not directly align with the user's core research themes in query understanding, ranking models, and user behavior modeling.
...
### 23. QIME: Constructing Interpretable Medical Text Embeddings via Ontology-Grounded Questions

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Yixuan Tang, Zhenghong Lin, Yandong Sun, Anthony K. H. Tung
- **URL**: <http://arxiv.org/abs/2603.01690v1>
- **Submitted**: 2026-03-02 10:18:06
- **Topic Keywords**: retrieval
- **Reason**: The paper explores interpretable medical text embeddings, which is somewhat related to information retrieval, particularly in the context of deep semantic understanding. However, the focus on biomedical text and clinical decision-making is not directly aligned with the user's primary interests in e-commerce and general search technologies.
...
### 24. PanCanBench: A Comprehensive Benchmark for Evaluating Large Language Models in Pancreatic Oncology
...
- **URL**: <http://arxiv.org/abs/2603.01343v1>
- **Submitted**: 2026-03-02 00:50:39
- **Topic Keywords**: queries, ranking, rag, web search, rank, search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus is on evaluating Large Language Models in a specific domain (pancreatic oncology) and does not address query understanding, ranking models, or user behavior modeling.
...
### 25. Surgical Post-Training: Cutting Errors, Keeping Knowledge

- **LLM Score**: 2
- **Keyword Score**: 9
- **Authors**: Wenye Lin, Kai Han
- **URL**: <http://arxiv.org/abs/2603.01683v1>
- **Submitted**: 2026-03-02 10:12:56
- **Comment**: 15 pages
- **Topic Keywords**: ranking, rag, rank, search, acl
- **Reason**: This paper focuses on post-training techniques for Large Language Models, specifically addressing the trade-off between efficiency and catastrophic forgetting. While it involves optimization and data rectification, the context is not related to information retrieval, search technologies, or user behavior modeling, which are the primary areas of interest. The paper's emphasis on reasoning capabilities and binary classification does not align with the user's core research themes.
...
### 26. Stop Treating Collisions Equally: Qualification-Aware Semantic ID Learning for Recommendation at Industrial Scale
...
- **URL**: <http://arxiv.org/abs/2603.00632v1>
- **Submitted**: 2026-02-28 12:55:49
- **Topic Keywords**: ranking, retrieval, recommend, commerce, e-commerce, rank
- **Reason**: This paper focuses on recommendation systems, specifically addressing the collision problem in semantic ID learning. While it involves deep semantic understanding, the context is primarily recommender systems, which is somewhat related to your interests in information retrieval, but not a central match.
...
### 27. MuonRec: Shifting the Optimizer Paradigm Beyond Adam in Scalable Generative Recommendation
...
- **Submitted**: 2026-02-28 02:32:44
- **Comment**: Under Review
- **Topic Keywords**: ranking, rag, recommend, personalization, rank, recsys
- **Reason**: This paper is primarily focused on recommender systems and optimizer design, which is somewhat related to your interests in information retrieval and search technologies. However, the specific context and techniques presented are more aligned with recommender systems, and the paper does not directly address query understanding, ranking models, or user behavior modeling.
...
### 28. TARSE: Test-Time Adaptation via Retrieval of Skills and Experience for Reasoning Agents

- **LLM Score**: 2
- **Keyword Score**: 7
- **Authors**: Junda Wang, Zonghai Tao, Hansi Zeng, Zhichao Yang, Hamed Zamani, Hong Yu
- **URL**: <http://arxiv.org/abs/2603.01241v1>
- **Submitted**: 2026-03-01 19:31:23
- **Topic Keywords**: retriever, rag, retrieval
- **Reason**: This paper focuses on clinical decision making and test-time adaptation for reasoning agents, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves retrieval and adaptation, the context and application are quite different from your areas of focus.
...
### 29. KVSlimmer: Theoretical Insights and Practical Optimizations for Asymmetric KV Merging

- **LLM Score**: 2
- **Keyword Score**: 7
- **Authors**: Lianjun Liu, Hongli An, Weiqi Yan, Xin Du, Shengchuan Zhang, Huazhong Liu, Yunshan Zhong
- **URL**: <http://arxiv.org/abs/2603.00907v1>
- **Submitted**: 2026-03-01 04:07:36
- **Topic Keywords**: query, rag, ctr
- **Reason**: This paper focuses on optimizing Key-Value caches for Large Language Models, which is not directly related to Information Retrieval or Search technologies. While it involves some optimization techniques, the context is quite different from the user's core research themes.
...
### 30. A Study on Building Efficient Zero-Shot Relation Extraction Models

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Hugo Thomas, Caio Corro, Guillaume Gravier, Pascale Sébillot
- **URL**: <http://arxiv.org/abs/2603.01266v1>
- **Submitted**: 2026-03-01 20:44:31
- **Comment**: LREC 2026
- **Topic Keywords**: query, retrieval
- **Reason**: This paper focuses on zero-shot relation extraction models, which is a topic in Natural Language Processing (NLP). However, it does not directly relate to the user's primary research interests in Information Retrieval (IR), query understanding, ranking models, and user behavior modeling. The paper's focus on relation extraction and its application in a retrieval scenario is somewhat tangential to the user's interests.
...
### 31. Semantic XPath: Structured Agentic Memory Access for Conversational AI

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Yifan Simon Liu, Ruifan Wu, Liam Gallagher, Jiazhou Liang, Armin Toroghi, Scott Sanner
- **URL**: <http://arxiv.org/abs/2603.01160v1>
- **Submitted**: 2026-03-01 15:56:08
- **Topic Keywords**: query, rag
- **Reason**: This paper focuses on conversational AI and structured memory access, which is not directly related to your core research themes in Information Retrieval and Search technologies. While it involves query execution and structured data, the context is specific to conversational AI and does not align with your interests in query understanding, ranking models, or user behavior modeling.
...
### 32. Beyond the Grid: Layout-Informed Multi-Vector Retrieval with Parsed Visual Document Representations
...
- **Submitted**: 2026-03-02 09:55:00
- **Comment**: Under review
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on Visual Document Retrieval and introduces a novel paradigm for generating layout-informed sub-image embeddings, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 33. From Verbatim to Gist: Distilling Pyramidal Multimodal Memory via Semantic Information Bottleneck for Long-Horizon Video Agents
...
- **Reason**: This paper focuses on multimodal memory architectures for long-horizon video understanding, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves semantic information and memory mechanisms, the context is video understanding rather than text or query understanding, making it less relevant to the user's interests.
...
- **URL**: <http://arxiv.org/abs/2603.01096v1>
- **Submitted**: 2026-03-01 13:22:44
- **Comment**: ICLR 2026
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on vision-language modeling and concept understanding, which is not directly related to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it explores some related concepts, it is primarily concerned with visual and multimodal understanding, which is not a central match for the user's research themes.
...
### 35. S-VoCAL: A Dataset and Evaluation Framework for Inferring Speaking Voice Character Attributes in Literature

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Abigail Berthe-Pardo, Gaspard Michel, Elena V. Epure, Christophe Cerisara
- **URL**: <http://arxiv.org/abs/2603.00958v1>
- **Submitted**: 2026-03-01 07:24:16
- **Comment**: Accepted to LREC 2026
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text analysis and Large Language Models, its focus on character attributes in literature and synthetic audiobook narration does not align with your core research themes.
...
### 36. GeMi: A Graph-based, Multimodal Recommendation System for Narrative Scroll Paintings

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Haimonti Dutta, Pruthvi Moluguri, Jin Dai, Saurabh Amarnath Mahindre
- **URL**: <http://arxiv.org/abs/2603.00854v1>
- **Submitted**: 2026-03-01 00:54:26
- **Topic Keywords**: rag, recommend, search
- **Reason**: This paper is not relevant to your research interests as it focuses on a graph-based, multimodal recommendation system for narrative scroll paintings, which is outside your primary area of interest in Information Retrieval and Search technologies. While it does involve machine learning and deep learning techniques, the context and application are not aligned with your core research themes.
...
### 37. SSKG Hub: An Expert-Guided Platform for LLM-Empowered Sustainability Standards Knowledge Graphs
...
- **URL**: <http://arxiv.org/abs/2603.00669v1>
- **Submitted**: 2026-02-28 14:24:55
- **Comment**: 10 pages, 2 figures, 2 tables, submitted to ACL26 System Demo Track
- **Topic Keywords**: rag, search, www
- **Reason**: This paper appears to be focused on sustainability standards knowledge graphs and the application of LLMs in this domain. While it involves some aspects of data modeling and knowledge representation, it does not seem to be directly related to information retrieval, search technologies, or query understanding, which are the primary areas of interest.
...
### 38. Power Echoes: Investigating Moderation Biases in Online Power-Asymmetric Conflicts
...
- **Submitted**: 2026-03-02 05:16:11
- **Comment**: Accepted at the ACM CHI conference on Human Factors in Computing Systems (ACM CHI 2026)
- **Topic Keywords**: rag, search
- **Reason**: This paper is not relevant to your research interests as it focuses on moderation biases in online power-asymmetric conflicts, which is outside the scope of Information Retrieval, Search technologies, and Natural Language Processing. Although it involves human-AI collaboration, the context is not related to query understanding, ranking models, or user behavior modeling.
...
### 39. Enhancing Persona Following at Decoding Time via Dynamic Importance Estimation for Role-Playing Agents

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Yuxin Liu, Mingye Zhu, Siyuan Liu, Bo Hu, Lei Zhang
- **URL**: <http://arxiv.org/abs/2603.01438v1>
- **Submitted**: 2026-03-02 04:37:16
- **Comment**: ICLR 2026
- **Topic Keywords**: rag, search
- **Reason**: This paper appears to be primarily focused on Role-Playing Language Agents and persona management in sociological research, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves Large Language Models and decoding, the context and application are quite different from the user's interests.
...
### 40. SWE-Adept: An LLM-Based Agentic Framework for Deep Codebase Analysis and Structured Issue Resolution

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Kang He, Kaushik Roy
- **URL**: <http://arxiv.org/abs/2603.01327v1>
- **Submitted**: 2026-03-01 23:52:30
- **Topic Keywords**: retrieval, search
- **Reason**: This paper focuses on software engineering and code analysis using Large Language Models (LLMs), which is not directly related to Information Retrieval, Search technologies, or Natural Language Processing. While it involves deep semantic understanding, the context is software engineering, making it only loosely relevant to your research interests.
...
### 41. Catalyst-Agent: Autonomous heterogeneous catalyst screening and optimization with an LLM Agent

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Achuth Chandrasekhar, Janghoon Ock, Amir Barati Farimani
- **URL**: <http://arxiv.org/abs/2603.01311v1>
- **Submitted**: 2026-03-01 22:44:56
- **Topic Keywords**: rag, search
- **Reason**: This paper is not directly related to Information Retrieval (IR) or Search technologies, but rather focuses on catalyst screening and optimization using a Large Language Model (LLM) Agent. While it involves AI and deep learning models, the context is distinct from the user's core research themes.
...
### 42. Generative AI & Fictionality: How Novels Power Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Edwin Roland, Richard Jean So
- **URL**: <http://arxiv.org/abs/2603.01220v1>
- **Submitted**: 2026-03-01 18:34:02
- **Topic Keywords**: rag, search
- **Reason**: This paper appears to be primarily focused on the impact of fiction on large language models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the topic of language models, it does not address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to your research.
...
### 43. Piecing Together Cross-Document Coreference Resolution Datasets: Systematic Dataset Analysis and Unification

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Anastasia Zhukova, Terry Ruas, Jan Philip Wahle, Bela Gipp
- **URL**: <http://arxiv.org/abs/2603.00621v1>
- **Submitted**: 2026-02-28 12:30:44
- **Comment**: accepted to LREC 2026
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on cross-document coreference resolution, which is a task in Natural Language Processing. While it touches on dataset analysis and model evaluation, it does not directly relate to information retrieval, query understanding, or ranking models, which are core areas of your research interests.
...
### 44. CIRCUS: Circuit Consensus under Uncertainty via Stability Ensembles

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Swapnil Parekh
- **URL**: <http://arxiv.org/abs/2603.00523v1>
- **Submitted**: 2026-02-28 07:44:04
- **Topic Keywords**: relevance
- **Reason**: This paper appears to be focused on circuit discovery and uncertainty quantification in mechanistic attribution, which is unrelated to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 45. Bootstrapping Embeddings for Low Resource Languages

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Merve Basoz, Andrew Horne, Mattia Opper
- **URL**: <http://arxiv.org/abs/2603.01732v1>
- **Submitted**: 2026-03-02 10:59:33
- **Comment**: (v1 - LowResLM Camera Ready)
- **Topic Keywords**: rag
- **Reason**: This paper focuses on low-resource languages and embedding models in NLP, which is somewhat related to your interests in NLP and related topics. However, it does not directly address your core research themes in Information Retrieval, query understanding, ranking models, or user behavior modeling.
...
### 46. TopoCurate:Modeling Interaction Topology for Tool-Use Agent Training
...
- **Submitted**: 2026-03-02 10:38:54
- **Comment**: Under Review
- **Topic Keywords**: rag
- **Reason**: This paper appears to be focused on training tool-use agents, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve some aspects of modeling and optimization, the context and methodology are quite different from your areas of expertise.
...
### 47. Learning to Draft: Adaptive Speculative Decoding with Reinforcement Learning
...
- **Submitted**: 2026-03-02 09:17:48
- **Comment**: 22pages, 7 figures
- **Topic Keywords**: rag
- **Reason**: This paper focuses on optimizing the efficiency of large language model inference using reinforcement learning, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the context is more focused on model efficiency rather than semantic understanding or search relevance.
...
### 48. Understanding the Physics of Key-Value Cache Compression for LLMs through Attention Dynamics

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Samhruth Ananthanarayanan, Ayan Sengupta, Tanmoy Chakraborty
- **URL**: <http://arxiv.org/abs/2603.01426v1>
- **Submitted**: 2026-03-02 04:16:36
- **Topic Keywords**: rag
- **Reason**: This paper appears to be focused on the physics of key-value cache compression for Large Language Models (LLMs), which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the concept of attention dynamics, it is more relevant to the field of deep learning and model optimization, rather than query understanding, ranking models, or user behavior modeling.
...
### 49. End-to-End Simultaneous Dysarthric Speech Reconstruction with Frame-Level Adaptor and Multiple Wait-k Knowledge Distillation

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Minghui Wu, Haitao Tang, Jiahuan Fan, Ruizhi Liao, Yanyong Zhang
- **URL**: <http://arxiv.org/abs/2603.01382v1>
- **Submitted**: 2026-03-02 02:26:01
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on dysarthric speech reconstruction, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves speech recognition and text-to-speech, the context is specific to speech disorders and does not align with your primary focus on deep semantic understanding and real-time relevance optimization in IR.
...
### 50. I Can't Believe It's Not Robust: Catastrophic Collapse of Safety Classifiers under Embedding Drift

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Subramanyam Sahoo, Vinija Jain, Divya Chaudhary, Aman Chadha
- **URL**: <http://arxiv.org/abs/2603.01297v1>
- **Submitted**: 2026-03-01 22:02:53
- **Comment**: Accepted at the ICBINB: Where LLMs Need to Improve workshop at ICLR 2026. 12 pages and 3 Figures
- **Topic Keywords**: rag
- **Reason**: This paper focuses on the robustness of safety classifiers in AI systems, which is not directly related to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the concept of model updates, it does not address query understanding, ranking models, or user behavior modeling, making it less relevant to your interests.
