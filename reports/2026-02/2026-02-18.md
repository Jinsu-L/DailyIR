# Daily Papers Report - 2026-02-18

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. ScrapeGraphAI-100k: A Large-Scale Dataset for LLM-Based Web Information Extraction

- **LLM Score**: 8
- **Keyword Score**: 5
- **Authors**: William Brach, Francesco Zuppichini, Marco Vinciguerra, Lorenzo Padoan
- **URL**: <http://arxiv.org/abs/2602.15189v1>
- **Submitted**: 2026-02-16 20:56:59
- **Topic Keywords**: information retrieval, retrieval
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of web information extraction and the use of large language models. The dataset introduced in this paper can be useful for fine-tuning small models and benchmarking structured extraction, which aligns with your focus on query understanding and ranking models. However, the paper's primary focus on web information extraction and LLM-based extraction events is somewhat tangential to your core research themes in query understanding and ranking models.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: ScrapeGraphAI‑100k benchmark for LLM‑based web information extraction
- **Aim**: Provide a privacy‑preserving, end‑to‑end benchmark that preserves the full extraction life‑cycle, enabling evaluation of structured extraction accuracy, schema induction, and downstream error propagation.
- **Rationale**: Existing corpora strip structural signals and current benchmarks focus on boilerplate removal; a richly annotated, schema‑driven dataset mirroring real pipelines is needed to advance web information extraction research.
- **Ground**: 93,695 balanced examples derived from 9 M opt‑in telemetry events, each containing raw markdown, natural‑language prompt, target JSON‑schema, LLM response, validation flag, schema‑complexity metrics (depth, key count, element count, cyclomatic complexity, composite score), and execution metadata (LLM used, latency, response size). Privacy is ensured via opt‑in and anonymization. Validation rates drop sharply beyond structural thresholds (depth ≥ 7, key count ≥ 200, composite > 1,000). GPT‑4o‑mini dominates usage.
- **Experiment**: Baseline experiments with Qwen3 models (1.7B, 4B, 30B) fine‑tuned via QLoRA; a 1.7B model trained with completion‑only loss achieves key F1 ≈ 0.887, schema compliance ≈ 0.917, extra keys ≈ 0.33, value score ≈ 0.462, overall BLEU ≈ 0.576—within 3 % of the 30B baseline on structural metrics but lower on value extraction. Evaluation protocol includes structural validity, key extraction, value extraction, and overall BLEU. Statistical analysis shows strong collinearity among schema‑complexity metrics and modest correlations with latency and response size.
- **Takeaway**: ScrapeGraphAI‑100k fills a critical void by offering a richly annotated, privacy‑preserving benchmark that captures the entire extraction life‑cycle. It demonstrates that parameter‑efficient fine‑tuning of modest‑size LLMs can rival large baselines on structural accuracy, provides detailed diagnostics for schema complexity, and supports research into schema induction, multilingual extraction, and downstream error propagation, thereby advancing both academic and industry practices in web information extraction.
...
- **URL**: <http://arxiv.org/abs/2602.15814v1>
- **Submitted**: 2026-02-17 18:50:40
- **Topic Keywords**: retrieval
- **Reason**: This paper is highly relevant to your research interests in Natural Language Processing (NLP) and Information Retrieval (IR), particularly in the context of deep semantic understanding and real-time relevance optimization. The paper's focus on bidirectional encoders, self-attention, and neural compression aligns with your interests in query understanding and ranking models. However, the paper's primary focus on NLP applications, rather than search technologies, prevents it from being a perfect match.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: Attention‑free autoregressive encoder for efficient NLP
- **Aim**: Develop a compact, efficient encoder that matches or surpasses transformer‑based models while reducing memory and computational cost
- **Rationale**: Self‑attention is computationally heavy; an attention‑free model can provide comparable contextualization with less overhead
- **Ground**: Prior success of bidirectional encoders like BERT; industrial need for efficient models; Avey’s design with decoupled parameters, stability normalization, and neural compression
- **Experiment**: Revised Avey compared to four transformer encoders on token‑classification and information‑retrieval benchmarks, evaluating performance and long‑context scaling
- **Takeaway**: Avey’s architectural enhancements yield superior performance and efficiency, proving attention‑free encoders are viable alternatives to transformers
...
### 3. Extracting Consumer Insight from Text: A Large Language Model Approach to Emotion and Evaluation Measurement
...
- **Submitted**: 2026-02-17 02:33:51
- **Topic Keywords**: rag, recommend, search
- **Reason**: This paper aligns with your interests in Natural Language Processing (NLP) and Information Retrieval (IR), particularly in the context of query understanding and deep semantic understanding. The focus on extracting consumer insights from text and leveraging large language models is relevant to your research, although it is more focused on marketing applications rather than search technologies.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: Consumer emotion and evaluation construct extraction from unstructured text
- **Aim**: Develop a fine‑tuned large language model, Linguistic eXtractor (LX), to reliably detect 16 consumption‑related emotions and four evaluation constructs (trust, commitment, recommendation, sentiment) in consumer‑authored reviews
- **Rationale**: Existing sentiment analysis tools lack granularity for nuanced consumer emotions and marketing constructs; accurate extraction is essential for understanding how emotions influence product ratings and purchase behavior
- **Ground**: LX is trained on a curated dataset of consumer survey responses with self‑reported ratings for emotions and evaluation constructs; it is fine‑tuned on large‑scale consumer text and benchmarked against GPT‑4 Turbo, RoBERTa, and DeepSeek, achieving 81 % macro‑F1 on open‑ended responses and >95 % accuracy on Amazon and Yelp reviews
- **Experiment**: LX was applied to online retail data and analyzed via seemingly unrelated regression; results show review‑expressed emotions predict product ratings, which predict purchase behavior, with discontent and peacefulness exerting direct effects on purchases beyond ratings
- **Takeaway**: LX demonstrates that fine‑tuned LLMs can accurately detect marketing constructs from real‑world consumer text, revealing that emotional tone adds predictive value beyond star ratings; a no‑code, cost‑free web application is released to enable scalable large‑scale analysis
...
- **URL**: <http://arxiv.org/abs/2602.15273v1>
- **Submitted**: 2026-02-17 00:09:44
- **Topic Keywords**: ranking, recommend, personalization, rank, search
- **Reason**: The paper explores information health and its relation to search and recommendation systems, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the focus on framing and its effects on user behavior is not directly aligned with your primary research themes, such as query understanding and ranking models.
...
### 5. jina-embeddings-v5-text: Task-Targeted Embedding Distillation
...
- **Submitted**: 2026-02-17 12:50:50
- **Comment**: 14 pages, 8 figures. Model weights: https://huggingface.co/collections/jinaai/jina-embeddings-v5-text
- **Topic Keywords**: information retrieval, retrieval
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the area of text embedding models. However, the focus on model distillation and task-specific contrastive loss is more aligned with NLP and data mining, rather than your primary focus on query understanding, ranking models, and user behavior modeling.
...
### 6. Semantics-Aware Denoising: A PLM-Guided Sample Reweighting Strategy for Robust Recommendation

- **LLM Score**: 6
- **Keyword Score**: 5
- **Authors**: Xikai Yang, Yang Wang, Yilin Li, Sebastian Sun
- **URL**: <http://arxiv.org/abs/2602.15359v1>
- **Submitted**: 2026-02-17 04:58:21
- **Topic Keywords**: rag, click, recommend
- **Reason**: The paper explores a denoising strategy for recommender systems, leveraging semantic consistency and pre-trained language models. While it touches on user behavior modeling, its primary focus is on recommendation systems, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the paper's emphasis on recommender systems and not on query understanding or ranking models limits its relevance to your core research themes.
...
### 7. Causal Effect Estimation with Latent Textual Treatments

- **LLM Score**: 6
- **Keyword Score**: 1
- **Authors**: Omri Feldman, Amar Venugopal, Jann Spiess, Amir Feder
- **URL**: <http://arxiv.org/abs/2602.15730v1>
- **Submitted**: 2026-02-17 17:06:12
- **Topic Keywords**: search
- **Reason**: This paper explores causal effect estimation in text-as-treatment settings, which is somewhat related to information retrieval, particularly in areas requiring deep semantic understanding. However, the focus on causal estimation and text generation is not directly aligned with the user's primary interests in query understanding, ranking models, and user behavior modeling.
...
- **URL**: <http://arxiv.org/abs/2602.15353v1>
- **Submitted**: 2026-02-17 04:47:29
- **Comment**: 26 pages, 7 figures
- **Topic Keywords**: queries, rag, retrieval, search
- **Reason**: This paper explores knowledge graph question answering, which is related to information retrieval, but its focus on neural-symbolic reasoning and knowledge graphs is somewhat tangential to the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 9. AIC CTU@AVerImaTeC: dual-retriever RAG for image-text fact checking

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Herbert Ullrich, Jan Drchal
- **URL**: <http://arxiv.org/abs/2602.15190v1>
- **Submitted**: 2026-02-16 21:00:29
- **Topic Keywords**: retriever, rag, retrieval, search
- **Reason**: The paper is somewhat related to the user's research interests in Information Retrieval, specifically in the area of multimodal retrieval and fact-checking. However, it focuses on image-text fact-checking and does not directly address query understanding, ranking models, or user behavior modeling. While it involves a retrieval-augmented generation pipeline, it is not primarily focused on deep semantic understanding or real-time relevance optimization.
...
### 10. Latent Objective Induction and Diversity-Constrained Selection: Algorithms for Multi-Locale Retrieval Pipelines

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Faruk Alpay, Levent Sarioglu
- **URL**: <http://arxiv.org/abs/2602.15921v1>
- **Submitted**: 2026-02-17 12:25:22
- **Comment**: 13 pages, 2 algorithms, 3 tables
- **Topic Keywords**: queries, retrieval, rank, search
- **Reason**: The paper discusses algorithms for multi-locale retrieval pipelines, which is somewhat related to information retrieval and search technologies. However, the focus on locale allocation, diversity constraints, and Latent Objective Induction is not directly aligned with the user's core research themes of query understanding, ranking models, and user behavior modeling. While it touches on related topics, it does not demonstrate a deep semantic understanding or real-time relevance optimization.
...
### 11. GaiaFlow: Semantic-Guided Diffusion Tuning for Carbon-Frugal Search
...
- **Submitted**: 2026-02-17 08:35:11
- **Comment**: 19 pages, 7 figures
- **Topic Keywords**: information retrieval, retrieval, rank, search
- **Reason**: The paper presents a framework for carbon-frugal search, which is related to information retrieval, but its focus on environmental sustainability and neural architecture design is not directly aligned with the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 12. Seeing to Generalize: How Visual Data Corrects Binding Shortcuts
...
- **Submitted**: 2026-02-16 20:43:12
- **Comment**: Submitted to ICML 2026
- **Topic Keywords**: information retrieval, rag, retrieval
- **Reason**: This paper explores the intersection of vision and language models, specifically how visual data affects the performance of language models on text-only tasks. While it touches on information retrieval, the primary focus is on the impact of visual data on model generalization and binding strategies, which is somewhat related to your interests in query understanding and ranking models. However, the paper's emphasis on visual data and cross-modal training makes it less directly relevant to your core research themes.
...
### 13. Rethinking Metrics for Lexical Semantic Change Detection

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Roksana Goworek, Haim Dubossarsky
- **URL**: <http://arxiv.org/abs/2602.15716v1>
- **Submitted**: 2026-02-17 16:47:40
- **Comment**: Accepted to the LChange 2026 Workshop, colocated with EACL 2026
- **Topic Keywords**: pairwise, rag
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and data mining, but it is not directly focused on Information Retrieval (IR) or search technologies. The paper explores lexical semantic change detection, which is a specific area within NLP, but it does not address query understanding, ranking models, or user behavior modeling, which are core areas of interest for the user.
...
- **Submitted**: 2026-02-17 13:24:56
- **Topic Keywords**: queries, rag
- **Reason**: The paper explores dynamic workflow construction for Text-to-SQL, which is a specific application in Natural Language Processing (NLP). While it touches on aspects of query understanding and ranking models, its primary focus is on workflow adaptation rather than information retrieval. The paper's relevance to the user's core research themes in IR and Search technologies is limited.
...
### 15. Mnemis: Dual-Route Retrieval on Hierarchical Graphs for Long-Term LLM Memory
...
- **URL**: <http://arxiv.org/abs/2602.15313v1>
- **Submitted**: 2026-02-17 02:44:03
- **Comment**: 10 pages
- **Topic Keywords**: rag, retrieval, search
- **Reason**: While the paper explores a novel memory framework for Large Language Models, it primarily focuses on memory organization and retrieval, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the paper's emphasis on hierarchical graph structures and global reasoning mechanisms is not directly aligned with the user's core research themes in IR and NLP.
...
### 16. tensorFM: Low-Rank Approximations of Cross-Order Feature Interactions

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Alessio Mazzetto, Mohammad Mahdi Khalili, Laura Fee Nern, Michael Viderman, Alex Shtoff, Krzysztof Dembczyński
- **URL**: <http://arxiv.org/abs/2602.15229v1>
- **Submitted**: 2026-02-16 22:21:48
- **Topic Keywords**: click, click-through rate, rank
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of click-through rate prediction. However, it focuses on tabular categorical data and low-rank approximations, which is not directly aligned with your core interests in query understanding, ranking models, and user behavior modeling.
...
### 17. In Agents We Trust, but Who Do Agents Trust? Latent Source Preferences Steer LLM Generations
...
- **Submitted**: 2026-02-17 09:45:22
- **Comment**: ICLR 2026
- **Topic Keywords**: recommend, web search, search
- **Reason**: This paper explores the latent source preferences of Large Language Models (LLMs) in selecting and presenting information to users, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on LLMs and their biases in information selection is not a central match to the user's primary research interests in IR and Search technologies.
...
### 18. World-Model-Augmented Web Agents with Action Correction

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Zhouzhou Shen, Xueyu Hu, Xiyun Li, Tianqing Fang, Juncheng Li, Shengyu Zhang
- **URL**: <http://arxiv.org/abs/2602.15384v1>
- **Submitted**: 2026-02-17 06:37:31
- **Topic Keywords**: rag
- **Reason**: The paper explores web agents and their ability to reason out sensible actions, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on web agents and action correction is not directly aligned with the user's core research themes in IR and NLP. The paper's emphasis on multi-agent collaboration and consequence simulation is an interesting aspect, but it does not strongly connect to the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 19. Towards Expectation Detection in Language: A Case Study on Treatment Expectations in Reddit

- **LLM Score**: 4
- **Keyword Score**: 1
- **Authors**: Aswathy Velutharambath, Amelie Wührl
- **URL**: <http://arxiv.org/abs/2602.15504v1>
- **Submitted**: 2026-02-17 11:21:40
- **Topic Keywords**: www
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and related topics, but it does not directly align with their core research themes in Information Retrieval (IR), query understanding, ranking models, or user behavior modeling. The paper's focus on expectation detection in language and its application to a specific domain (medical) is not a central match for the user's research interests.
...
### 20. OpaqueToolsBench: Learning Nuances of Tool Behavior Through Interaction
...
- **Submitted**: 2026-02-16 21:26:37
- **Topic Keywords**: search
- **Reason**: The paper explores the nuances of tool behavior through interaction, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on Large Language Model agents and tool documentation is not directly aligned with the user's core research themes. The paper's relevance is further diminished by its lack of direct connection to search technologies, user behavior modeling, or e-commerce domain.
...
### 21. STAPO: Stabilizing Reinforcement Learning for LLMs by Silencing Rare Spurious Tokens
...
- **URL**: <http://arxiv.org/abs/2602.15620v2>
- **Submitted**: 2026-02-17 14:46:48
- **Topic Keywords**: rag
- **Reason**: This paper focuses on stabilizing reinforcement learning for large language models, which is somewhat related to query understanding and ranking models in information retrieval. However, the topic is more aligned with NLP and deep learning, and the specific application to large language models does not directly relate to the user's interests in e-commerce or real-time relevance optimization.
...
### 22. Can Recommender Systems Teach Themselves? A Recursive Self-Improving Framework with Fidelity Control
...
- **Submitted**: 2026-02-17 15:31:32
- **Topic Keywords**: recommend
- **Reason**: The paper discusses a self-improving framework for recommender systems, which is somewhat related to the user's interests in information retrieval and search technologies. However, the focus on recommender systems and the lack of emphasis on query understanding, ranking models, or user behavior modeling limit its relevance to the user's core research themes.
...
### 23. Eco-Amazon: Enriching E-commerce Datasets with Product Carbon Footprint for Sustainable Recommendations
...
- **Submitted**: 2026-02-17 11:30:11
- **Topic Keywords**: information retrieval, rag, ctr, retrieval, recommend, commerce, e-commerce, search
- **Reason**: This paper focuses on sustainable recommendations and environmental sustainability in e-commerce, which is somewhat related to your interests in Information Retrieval and Search technologies. However, it does not align with your core research themes of query understanding, ranking models, and user behavior modeling, and is more focused on recommender systems and data enrichment.
...
### 24. Automatic Funny Scene Extraction from Long-form Cinematic Videos

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Sibendu Paul, Haotian Jiang, Caren Chen
- **URL**: <http://arxiv.org/abs/2602.15381v1>
- **Submitted**: 2026-02-17 06:29:06
- **Topic Keywords**: ranking, rag, rank
- **Reason**: This paper focuses on automatic scene extraction and ranking from cinematic videos, which is not directly related to information retrieval, search technologies, or natural language processing. Although it involves multimodal scene localization and humor tagging, the context is limited to video content and does not align with the user's core research themes.
...
### 25. The Vision Wormhole: Latent-Space Communication in Heterogeneous Multi-Agent Systems
...
- **URL**: <http://arxiv.org/abs/2602.15382v1>
- **Submitted**: 2026-02-17 06:31:53
- **Comment**: Preprint. Work in progress
- **Topic Keywords**: pairwise, rag
- **Reason**: This paper appears to be primarily focused on multi-agent systems and large language models, with a novel framework for text-free communication. While it involves some form of 'understanding' and 'communication', it doesn't seem to align with your core research themes in information retrieval, search technologies, or natural language processing. The paper's focus on visual interfaces and heterogeneous model families doesn't seem to overlap with your interests in query understanding, ranking models, or user behavior modeling.
...
- **Submitted**: 2026-02-16 19:40:07
- **Topic Keywords**: query, rag
- **Reason**: This paper is primarily focused on protecting language models from unauthorized distillation, which is not directly related to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on language models, the context is more aligned with model security and watermarking, rather than query understanding, ranking models, or user behavior modeling.
...
### 27. Perspectives - Interactive Document Clustering in the Discourse Analysis Tool Suite

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Tim Fischer, Chris Biemann
- **URL**: <http://arxiv.org/abs/2602.15540v1>
- **Submitted**: 2026-02-17 12:44:05
- **Topic Keywords**: rag, search
- **Reason**: This paper appears to be focused on document clustering and discourse analysis in the Digital Humanities domain, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some NLP techniques, the context and application seem to be quite different from the user's areas of focus.
...
### 28. Enhancing Building Semantics Preservation in AI Model Training with Large Language Model Encodings

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Suhyung Jang, Ghang Lee, Jaekun Lee, Hyunjun Lee
- **URL**: <http://arxiv.org/abs/2602.15791v1>
- **Submitted**: 2026-02-17 18:26:36
- **Comment**: 42nd International Symposium on Automation and Robotics in Construction (ISARC 2025)
- **Topic Keywords**: rag
- **Reason**: This paper focuses on leveraging large language model embeddings for semantic comprehension in the AECO industry, which is unrelated to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 29. Recursive Concept Evolution for Compositional Reasoning in Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Sarim Chaudhry
- **URL**: <http://arxiv.org/abs/2602.15725v1>
- **Submitted**: 2026-02-17 17:01:42
- **Topic Keywords**: rank, search
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on Large Language Models and compositional reasoning. While it involves deep semantic understanding, the context is more aligned with AI and machine learning rather than information retrieval or search technologies.
...
### 30. Revisiting Northrop Frye's Four Myths Theory with Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Edirlei Soares de Lima, Marco A. Casanova, Antonio L. Furtado
- **URL**: <http://arxiv.org/abs/2602.15678v1>
- **Submitted**: 2026-02-17 16:02:52
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on literary criticism and computational narratology using Large Language Models. While it involves NLP, the application is limited to a specific domain (literary analysis) and does not align with your core research themes.
...
### 31. RUVA: Personalized Transparent On-Device Graph Reasoning
...
- **URL**: <http://arxiv.org/abs/2602.15553v1>
- **Submitted**: 2026-02-17 12:59:03
- **Topic Keywords**: retrieval
- **Reason**: This paper focuses on Personal AI and Graph Reasoning, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the concept of 'black box' retrieval, it does not address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to the user's research.
...
### 32. ExpertWeaver: Unlocking the Inherent MoE in Dense LLMs with GLU Activation Patterns

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Ziyu Zhao, Tong Zhu, Zhi Zhang, Tiantian Fan, Jinluan Yang, Kun Kuang, Zhongyu Wei, Fei Wu, Yu Cheng
- **URL**: <http://arxiv.org/abs/2602.15521v1>
- **Submitted**: 2026-02-17 11:50:58
- **Topic Keywords**: rag
- **Reason**: This paper focuses on converting dense models to sparse MoEs, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves neural networks, the topic is more aligned with model architecture and efficiency, rather than query understanding, ranking models, or user behavior modeling.
...
### 33. Fine-Refine: Iterative Fine-grained Refinement for Mitigating Dialogue Hallucination

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Xiangyan Chen, Yujian Gan, Matthew Purver
- **URL**: <http://arxiv.org/abs/2602.15509v1>
- **Submitted**: 2026-02-17 11:33:23
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to Information Retrieval or Search technologies, but rather focuses on dialogue systems and large language models. While it involves natural language processing, the topic is more specific to conversational AI and factuality assessment, which is not a central match to the user's research interests.
...
### 34. Far Out: Evaluating Language Models on Slang in Australian and Indian English

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Deniz Kaya Dilsiz, Dipankar Srirag, Aditya Joshi
- **URL**: <http://arxiv.org/abs/2602.15373v2>
- **Submitted**: 2026-02-17 05:59:20
- **Comment**: Accepted as a paper at 13th VarDial workshop at EACL 2026
- **Topic Keywords**: rag
- **Reason**: This paper focuses on evaluating language models' performance on slang in specific language varieties, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves language models, the context is limited to slang awareness and does not align with the user's focus on query understanding, ranking models, and user behavior modeling.
...
### 35. LLM-to-Speech: A Synthetic Data Pipeline for Training Dialectal Text-to-Speech Models

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Ahmed Khaled Khamis, Hesham Ali
- **URL**: <http://arxiv.org/abs/2602.15675v1>
- **Submitted**: 2026-02-17 15:58:27
- **Comment**: 8 pages, 2 figures, EACL26
- **Topic Keywords**: search
- **Reason**: This paper focuses on Text-to-Speech (TTS) models for Arabic dialects, which is outside the user's primary research interests in Information Retrieval and Search technologies. Although it involves Natural Language Processing, the specific application and domain are not directly related to the user's core themes.
...
### 36. LuxMT Technical Report

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Nils Rehlinger
- **URL**: <http://arxiv.org/abs/2602.15506v1>
- **Submitted**: 2026-02-17 11:26:32
- **Comment**: preprint
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves machine translation and NLP, its focus on a specific language pair and translation quality estimation does not align with your core themes of query understanding, ranking models, and user behavior modeling.
...
### 37. Binge Watch: Reproducible Multimodal Benchmarks Datasets for Large-Scale Movie Recommendation on MovieLens-10M and 20M
...
- **Submitted**: 2026-02-17 11:22:20
- **Topic Keywords**: recommend
- **Reason**: This paper focuses on multimodal recommender systems and large-scale movie recommendation datasets, which is somewhat related to your interests in Information Retrieval and Search technologies. However, it does not align with your primary focus on query understanding, ranking models, and user behavior modeling, nor does it involve deep semantic understanding or real-time relevance optimization.
...
### 38. TAROT: Test-driven and Capability-adaptive Curriculum Reinforcement Fine-tuning for Code Generation with Large Language Models
...
- **Submitted**: 2026-02-17 09:29:18
- **Comment**: The first three authors contributed equally to this work; listing order is random
- **Topic Keywords**: search
- **Reason**: This paper focuses on code generation with Large Language Models, which is outside your primary research interests in Information Retrieval and Search technologies. While it involves reinforcement learning, the context is not directly related to your areas of expertise in query understanding, ranking models, or user behavior modeling.
...
### 39. Measuring Social Integration Through Participation: Categorizing Organizations and Leisure Activities in the Displaced Karelians Interview Archive using LLMs
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on categorizing historical archives using large language models, which is not directly related to information retrieval, search technologies, or natural language processing in the context of query understanding, ranking models, or user behavior modeling. While it involves NLP, the application and scope are quite different from the user's primary research interests.
...
### 40. da Costa and Tarski meet Goguen and Carnap: a novel approach for ontological heterogeneity based on consequence systems

- **LLM Score**: 1
- **Keyword Score**: 1
- **Authors**: Gabriel Rocha
- **URL**: <http://arxiv.org/abs/2602.15158v1>
- **Submitted**: 2026-02-16 19:58:35
- **Comment**: 22 pages, 5 figures, 1 table
- **Topic Keywords**: search
- **Reason**: This paper appears to be focused on ontological heterogeneity and consequence systems, which is unrelated to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing.
