...
### 1. Scaling and Stabilizing Large-Scale Embedding-Based Retrieval
...
- **URL**: <http://arxiv.org/abs/2607.10096v1>
- **Submitted**: 2026-07-11 03:24:26
- **Topic Keywords**: retrieval, commerce, e-commerce, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of e-commerce search. The authors address challenges in embedding-based retrieval, such as training-inference gaps and model evolution, which aligns with your focus on query understanding, ranking models, and real-time relevance optimization. The proposed pipeline and techniques, including Hybrid Hard Negative Mining and Legacy-Aware Distillation, demonstrate a deep semantic understanding and optimization of retrieval systems.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Dense Retrieval for E-commerce Search at Walmart
- **Aim**: Deploy a dual‑encoder search system from research to a live 200 M‑item catalog while preserving legacy knowledge and delivering measurable business lift.
- **Rationale**: Training‑inference mismatch and model‑capacity evolution risk threaten performance; legacy models hold valuable domain knowledge that must be retained when upgrading to higher‑capacity backbones.
- **Ground**: Hybrid Hard‑Negative Mining (online cross‑batch sampling + offline cross‑encoder + metadata guardrails) expands the negative space without extra memory, and Legacy‑Aware Warm‑Start Distillation transfers implicit knowledge from the legacy DistilBERT to a GTE‑base student via KL‑divergence before full fine‑tuning.
- **Experiment**: Training on 4 M queries (≈900 M pairs) with 8×A100 GPUs, 40 epochs, cross‑batch negatives (global batch 576). Empirical gains: +4.00 % EM Recall@20 and +2.01 % EM Precision@20 over baseline; online A/B shows +7.34 % NDCG@5 and +0.50 % revenue lift.
- **Takeaway**: Hybrid hard‑negative mining aligns training with massive inference catalog; legacy‑aware distillation preserves domain expertise during model evolution; combined approach yields significant offline and online performance gains, proving dense retrieval production‑ready at e‑commerce scale.
...
- **Submitted**: 2026-07-12 04:04:59
- **Comment**: 12 pages, 7 figures, 6 tables
- **Topic Keywords**: information retrieval, queries, ranking, rerank, pointwise, relevance, retrieval, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of query understanding and ranking models. The proposed framework, TALRanker, leverages Large Language Models and addresses the accuracy-efficiency dilemma in reranking, which aligns with your focus on real-time relevance optimization. While the paper's primary focus is on LLMs, it touches on the broader topic of information retrieval, making it a useful read for your research.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: TALRanker: Cost‑Aware, Tool‑Augmented Retrieval Reranker
- **Aim**: Create a point‑wise relevance scorer that balances internal LLM knowledge and external evidence to minimize hallucinations while respecting latency budgets.
- **Rationale**: Closed‑book point‑wise rerankers hallucinate on complex or long‑tail queries; external knowledge can ground reasoning but indiscriminate tool use inflates latency. No existing framework decides when to rely on internal knowledge versus fetch evidence under a cost constraint.
- **Ground**: Agentic MDP formulation with state (prompt+evidence), actions (<answer>, <tool_call>, <fallback>), policy πθ generating reasoning trajectories, and a cost‑aware asymmetric reward (GRPO) that penalizes hallucinations and tool calls. Training uses a warm‑up hybrid loss (BCE + KL) followed by RL with GRPO. Backbone: Qwen‑3 family; tool: Perplexity Pro Search; inference loop with vLLM on H100 GPUs.
- **Experiment**: Benchmarked on TREC‑DL19/20, BEIR (9 datasets), and BRIGHT (1,384 multi‑hop queries). Metrics: NDCG, hallucination rate, latency. Ablations on λ (tool‑call penalty) and γ (confidence threshold) show trade‑offs; λ=0.02 and γ=0.2–0.3 yield best accuracy. Compared to baseline GRMs and naive tool‑augmented rerankers, TALRanker improves NDCG by 5–7% while keeping average latency within the specified budget.
- **Takeaway**: An agentic MDP with a cost‑aware reward can effectively decide when to invoke external tools, reducing hallucinations and improving ranking accuracy without excessive latency, making it suitable for enterprise search pipelines.
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of semantic data analysis and real-time relevance optimization. The use of GraphRAG and FAIR principles aligns with your focus on deep semantic understanding and query understanding. However, the paper's primary focus on biomedical data and knowledge graphs is somewhat specific, limiting its generalizability to your broader interests.
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in areas that require deep semantic understanding and real-time relevance optimization. The focus on human feedback and user engagement aligns with your interests in query understanding and user behavior modeling. The use of LLMs and RAG systems also relates to your work in NLP and related topics.
...
### 5. The Effect of Multi-Lingual and Keyword Adversarial Injection on LLM Relevance Judgment

- **LLM Score**: 8
- **Keyword Score**: 13
- **Authors**: Nguyen Khoi Vo, Duy Duong Tuong, Oleg Zendel, Mark Sanderson
- **URL**: <http://arxiv.org/abs/2607.10080v1>
- **Submitted**: 2026-07-11 02:13:16
- **Topic Keywords**: information retrieval, query, relevance, retrieval, trec
- **Reason**: This paper is highly relevant to Information Retrieval, particularly in the context of Large Language Models (LLMs) used for relevance evaluation. The focus on adversarial manipulation and robustness of LLMs aligns with query understanding and ranking models, which are core research themes. However, the specific domain of multilingual settings and LLMs as judges is somewhat distinct from the user's primary focus on e-commerce and real-time relevance optimization.

#### Abstract
> Large language models (LLMs) are increasingly being used as automated judges
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding and ranking models. The focus on multimodal representation models and user behavior modeling aligns with your expertise in e-commerce search, and the use of deep semantic understanding and real-time relevance optimization is also in line with your interests.
...
### 7. GRASP: GRanularity-Aware Search Policy for Agentic RAG

- **LLM Score**: 8
- **Keyword Score**: 11
- **Authors**: Varun Gandhi, Jaewook Lee, Shantanu Todmal, Franck Dernoncourt, Ryan Rossi, Zichao Wang, Andrew Lan
- **URL**: <http://arxiv.org/abs/2607.10463v1>
- **Submitted**: 2026-07-11 19:58:07
- **Topic Keywords**: semantic search, queries, rag, retrieval, search
- **Reason**: This paper aligns well with your interests in Information Retrieval, particularly in query understanding, ranking models, and user behavior modeling. The focus on retrieval-augmented generation, semantic search, and context granularity is relevant to your research. However, the specific application to agentic RAG and reinforcement learning may not be a central match, hence the score of 8.
...
### 8. Prompt Generation Technical Report
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of generative retrieval and query understanding. The focus on decoupling feature-processing logic from model architecture aligns with your interest in ranking models and real-time relevance optimization. However, the paper's primary focus on a specific industrial application (Taobao Search) and technical solution (Prompt Generation framework) prevents it from being a central match.
...
### 9. SVD-RAG: Efficient Tree-Organized Retrieval-Augmented Generation via Singular Value Decomposition

- **LLM Score**: 8
- **Keyword Score**: 8
- **Authors**: Zhihui Sun
- **URL**: <http://arxiv.org/abs/2607.10316v1>
- **Submitted**: 2026-07-11 13:43:23
- **Topic Keywords**: queries, rag, retrieval, search
- **Reason**: This paper presents a novel approach to Retrieval-Augmented Generation (RAG) using Singular Value Decomposition (SVD) for efficient extractive summarization in hierarchical RAG. Although it's primarily focused on NLP applications, the use of SVD for semantic representation and the emphasis on efficiency and scalability align with the user's interests in Information Retrieval and Search technologies. The paper's focus on real-time relevance optimization and its comparison with existing LLM-based methods also resonate with the user's research themes.
...
### 10. Multilingual Semantic Retrieval for Apple Music Search

- **LLM Score**: 8
- **Keyword Score**: 7
- **Authors**: Vishalaksh Aggarwal, Kevin Sebastian, Vivek Kanojiya, Leo Le, Nick Tucey, Santosh Shankar
- **URL**: <http://arxiv.org/abs/2607.10239v1>
- **Submitted**: 2026-07-11 10:00:48
- **Topic Keywords**: queries, retrieval, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of multilingual search and semantic retrieval. The focus on improving recall on misspelled, transliterated, and cross-lingual queries aligns with your interest in query understanding and ranking models. The use of a Siamese bi-encoder and curriculum-scheduled multi-objective training also demonstrates a deep understanding of the underlying technologies.
...
### 11. MC-RAG System: A Structure-Driven RAG System for Multi-Constraint Queries

- **LLM Score**: 8
- **Keyword Score**: 7
- **Authors**: Xiao Zhang, Yang Wan, Yi Li, Miao Xie, Chunli Lv
- **URL**: <http://arxiv.org/abs/2607.10151v1>
- **Submitted**: 2026-07-11 06:20:38
- **Topic Keywords**: queries, rag, retrieval
- **Reason**: This paper presents a novel approach to Retrieval-Augmented Generation (RAG) systems, addressing complex multi-constraint queries. Although primarily focused on question answering, the use of knowledge graphs and structural embeddings aligns with the user's interests in query understanding and ranking models. The paper's emphasis on interpretability and explainability also resonates with the user's focus on deep semantic understanding.
...
### 12. To Answer or to Abstain: Mitigating Search-Agent Hallucinations via Abstention-Aware Reinforcement Learning
...
- **Submitted**: 2026-07-12 12:43:12
- **Topic Keywords**: query, retrieval, search
- **Reason**: This paper is highly relevant to Information Retrieval, particularly in the context of query understanding and ranking models. The focus on mitigating search-agent hallucinations via abstention-aware reinforcement learning aligns with your interests in query understanding and real-time relevance optimization. Although the paper's primary focus is on open-domain QA tasks, the techniques and ideas presented can be applied to broader IR and search technologies.
...
### 13. NGM-RAG: Neural Graph Matching based Retrieval-Augmented Generation
...
- **Reason**: This paper introduces a novel framework, NGM-RAG, that leverages graph structures for improved retrieval and answer generation in Retrieval-Augmented Generation. Although it primarily focuses on NLP and RAG, it touches on query understanding and ranking models, making it somewhat related to your core research themes in Information Retrieval. However, the specific application domain and techniques used are not directly aligned with your e-commerce background or deep semantic understanding interests.
...
### 14. Boolean queries are all you need?
...
- **Reason**: This paper explores the effectiveness of Boolean queries in search, which is related to query understanding and ranking models. Although it's not directly focused on Learning to Rank or user behavior modeling, the use of a search agent and evaluation metrics like NDCG@10 suggest relevance to information retrieval. However, the emphasis on pattern matching and simplicity may not align with the user's interest in deep semantic understanding.
...
### 15. Beyond Semantic IDs: Encoding Business-Value Ranking into Document Identifiers for Generative Retrieval
...
- **Reason**: This paper explores Generative Retrieval, a sequence-to-sequence generation task for document retrieval. While it touches on ranking models and retrieval quality, its primary focus is on document identifier design, which is somewhat related to the user's interests in Information Retrieval and ranking models. However, the paper's emphasis on e-commerce and business-value ranking is not a central match for the user's research themes.
...
### 16. An LLM-powered Agentic Recommendation System for Connected TV Content Discovery
...
- **Submitted**: 2026-07-10 21:28:43
- **Comment**: 13 pages, 3 figures
- **Topic Keywords**: ranking, rag, retrieval, recommend, personalization, rank
- **Reason**: This paper discusses a recommendation system that leverages large language models (LLMs) for Connected TV content discovery. While it touches on aspects of information retrieval and ranking models, its primary focus is on recommendation systems and LLMs, which is somewhat related to your interests in IR and NLP. However, the specific application domain and focus on recommender systems rather than IR makes it less central to your core research themes.
...
### 17. Serving the Long Tail: Training-Free LLM Candidate Generation for Vacation Rental Marketplaces

- **LLM Score**: 6
- **Keyword Score**: 9
- **Authors**: Syed Mohammed Arshad Zaidi, Eric Rincon, Shayan Hassantabar
- **URL**: <http://arxiv.org/abs/2607.09877v1>
- **Submitted**: 2026-07-10 18:10:01
- **Comment**: Accepted at TSMO 2026 workshop, co-located with KDD 2026; 9 pages
- **Topic Keywords**: queries, ranking, rag, rank
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the context of query understanding and ranking models. However, its focus on a specific application (vacation rental marketplaces) and the use of pre-trained language models for candidate generation, while interesting, does not directly align with your primary areas of focus.
...
### 18. Query-Focused Event Summarization: A Dataset and Benchmark
...
- **Reason**: The paper explores Query-Focused Summarization, which is related to query understanding and ranking models in Information Retrieval. However, it focuses on event summarization, which is somewhat tangential to the user's core research themes. The paper's emphasis on deep semantic understanding and real-time relevance optimization is a partial match, but its primary focus is on summarization rather than search technologies.
...
### 19. Trust Before Fusion: QIMG-7 and Source-Aware Resolution for Polluted Multimodal RAG

- **LLM Score**: 6
- **Keyword Score**: 4
- **Authors**: Saadeldine Eletter, Owais Aijaz, Preslav Nakov
- **URL**: <http://arxiv.org/abs/2607.10798v1>
- **Submitted**: 2026-07-12 15:13:09
- **Comment**: 23 pages, 6 figures, 23 tables. Preprint under review
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of multimodal retrieval-augmented generation (RAG) and the challenges of polluted content. However, the focus on multimodal fusion and trust resolution is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 20. Score-Only Distillation for Compact Dense Retrieval
...
- **Reason**: This paper focuses on compact dense retrieval, which is related to information retrieval, but it doesn't explicitly address query understanding, ranking models, or user behavior modeling. The paper's emphasis on compact retrievers and distillation protocols is somewhat relevant to search technologies, but it doesn't seem to be a central match for your research interests.
...
- **Topic Keywords**: retriever, ranking, rerank, recommend, rank, acl
- **Reason**: The paper focuses on recommender systems, which is a related topic to information retrieval, but it does not directly address query understanding, ranking models, or user behavior modeling. The use of deep semantic understanding and real-time relevance optimization is not a primary concern in this paper, which makes it somewhat relevant but not a central match for your research interests.
...
### 22. From Patent Expiry to Business Pathways: AI Workflows for Activating Innovation Archives

- **LLM Score**: 4
- **Keyword Score**: 10
- **Authors**: Sidney Shapiro, Mark Price
- **URL**: <http://arxiv.org/abs/2607.10179v1>
- **Submitted**: 2026-07-11 07:53:06
- **Comment**: 30 pages, Proof of concept using CIPO ST.96 data
- **Topic Keywords**: semantic search, ranking, rag, rank, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, as it involves semantic search and AI workflows for discovering and translating patent disclosures. However, the focus on patent expiry and business pathways is not directly aligned with your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 23. Cross-Architecture LLM Ensembles, Feature-Based Reranking and Retrieval-Augmented Prompting for Legal Information Processing
...
- **Reason**: The paper explores legal information processing, which is somewhat related to information retrieval and search technologies. However, the focus on legal domain and specific techniques like retrieval-augmented prompting and cross-architecture ensembling does not directly align with the user's core research interests in query understanding, ranking models, and user behavior modeling.
...
### 24. User Preference Induction with LLMs for Offline Top-N Recommendation Evaluation
...
- **Reason**: The paper focuses on recommender systems, which is a related area to information retrieval, but it does not directly address query understanding, ranking models, or user behavior modeling. The use of large language models for preference induction and relevance judgement is an interesting application of NLP, but it is not a central match to the user's core research themes.
...
### 25. Generative Chinese Statute Retrieval
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of legal information retrieval. However, it focuses on a specific domain (legal) and task (statute retrieval), which is not central to your core interests in query understanding, ranking models, and user behavior modeling. The generative approach and multi-granularity structured docid are interesting, but the paper's scope and application are not directly aligned with your primary focus on e-commerce and real-time relevance optimization.
...
### 26. From Stochastic to Stable: Rank Stability and Structural Sufficiency in AI Visibility Measurement

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Ronald Sielinski
- **URL**: <http://arxiv.org/abs/2607.10341v1>
- **Submitted**: 2026-07-11 14:50:02
- **Comment**: 31 pages, 11 figures
- **Topic Keywords**: query, ranking, rank, search
- **Reason**: This paper is somewhat related to Information Retrieval, specifically in the context of AI visibility measurement and citation analysis. However, it focuses on a specific aspect of search engines and does not directly address query understanding, ranking models, or user behavior modeling, which are core areas of interest. The paper's emphasis on structural sufficiency and rank stability is an interesting contribution, but it does not align closely with the user's primary research themes.
...
### 27. CAFE: A Compound-AI Factorial Evaluation Framework

- **LLM Score**: 4
- **Keyword Score**: 6
- **Authors**: Fabian Lukassen, Christoph Weisser, Thomas Kneib, Alexander Silbersdorff
- **URL**: <http://arxiv.org/abs/2607.10380v1>
- **Submitted**: 2026-07-11 16:17:46
- **Topic Keywords**: retriever, retrieval, search
- **Reason**: The paper introduces a framework for evaluating compound AI systems, which is somewhat related to information retrieval and search technologies. However, the focus is on evaluation rather than query understanding, ranking models, or user behavior modeling, making it less central to your research interests. The use of a retrieval-augmented question-answering pipeline is a tangential connection to your area of expertise.
...
### 28. Equal Accuracy, Unequal Evidence: Search APIs as Decision Surfaces for Tool-Using Agents

- **LLM Score**: 4
- **Keyword Score**: 6
- **Authors**: Sriram Selvam, Anneswa Ghosh
- **URL**: <http://arxiv.org/abs/2607.10198v1>
- **Submitted**: 2026-07-11 08:22:35
- **Topic Keywords**: retrieval, rank, search, acl
- **Reason**: This paper explores the concept of search APIs as decision surfaces for tool-using agents, which is somewhat related to information retrieval and search technologies. However, the focus on agent retrieval architectures and progressive disclosure is not directly aligned with the user's primary research themes of query understanding, ranking models, and user behavior modeling. While the paper touches on aspects of search performance evaluation, it does not directly relate to the user's interests in deep semantic understanding and real-time relevance optimization.
...
### 29. Faithful by Design: Evaluating and Improving LLM-Generated Clinical Trial Summaries for Multi-Stakeholder Audiences

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Robert Williams
- **URL**: <http://arxiv.org/abs/2607.09932v1>
- **Submitted**: 2026-07-10 19:29:26
- **Comment**: 8 pages, 8 figures
- **Topic Keywords**: ltr, retrieval
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and Large Language Models (LLMs), but it focuses on a specific application in clinical trial summaries, which is not directly aligned with your primary focus on Information Retrieval and Search technologies. The paper's emphasis on faithfulness and hallucination in LLMs is relevant to your interests in query understanding and ranking models, but the context is distinct from your usual e-commerce domain.
...
### 30. How Temperature Shapes Ideological Discourse in Retrieval-Augmented Generation?
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper explores the impact of ideological bias on Retrieval-Augmented Generation (RAG) models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on ideological discourse and language models is not a central match to the user's core research themes, which are more focused on e-commerce, user behavior modeling, and deep semantic understanding in real-time relevance optimization.
...
### 31. RAGU: A Multi-Step GraphRAG Engine with a Compact Domain-Adapted LLM
...
- **Topic Keywords**: rag, retrieval
- **Reason**: The paper discusses a novel GraphRAG engine called RAGU, which enhances large language models with structured knowledge. While it touches on aspects of query understanding and knowledge retrieval, the primary focus is on graph-based knowledge construction and retrieval, which is somewhat related to information retrieval but not directly aligned with the user's core research themes.
...
### 32. The Nuts and Bolts of Natural Language to SQL Translation: A Systematic Analysis of Model Pipeline Optimisation Approaches and their Interactions
...
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP), but it focuses on Natural Language to SQL translation, which is not a central match for the user's primary focus on information retrieval and query understanding. The paper's exploration of model pipeline optimization approaches and interactions is also somewhat relevant to the user's interests in ranking models and user behavior modeling, but the connection is not strong enough to warrant a higher score.
...
### 33. Eval-Pair Matrix: Answer-Paired Meta-Evaluation of LLM Judges for Grounded RAG

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Sriram Selvam, Anneswa Ghosh
- **URL**: <http://arxiv.org/abs/2607.10626v1>
- **Submitted**: 2026-07-12 07:39:45
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is somewhat related to information retrieval, specifically in the context of retrieval-augmented generation (RAG) and the evaluation of language models. However, its focus on meta-evaluation and the behavior of LLM judges is not directly aligned with the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 34. Tokenizing Numerical and Embedding Features for LLM RecSys
...
- **Submitted**: 2026-07-10 22:36:13
- **Topic Keywords**: retrieval, recommend, recsys
- **Reason**: The paper explores the integration of numerical and embedding features into Large Language Models (LLMs) for recommender systems, which is somewhat related to the user's interests in Information Retrieval and Search technologies. However, the focus on recommender systems and LLMs is not the user's primary area of interest, and the paper does not explicitly address query understanding, ranking models, or user behavior modeling.
...
### 35. Remembering Distinct Items, Not Tokens: A Learnable Dirichlet-Process Cache Between State-Space Models and Attention

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Siddharth Pal, Viktoria Rojkova
- **URL**: <http://arxiv.org/abs/2607.09889v1>
- **Submitted**: 2026-07-10 18:28:21
- **Comment**: 16 pages, 9 figures. Companion paper on event-log applications forthcoming
- **Topic Keywords**: query, recommend
- **Reason**: This paper explores a novel caching mechanism for sequence models, which is somewhat related to information retrieval and ranking models, but its focus on associative recall and caching strategies is not directly aligned with the user's core research themes. While it involves deep recurrent backbones and learnable mechanisms, the context is more aligned with NLP and sequence modeling rather than search technologies or user behavior modeling.
...
### 36. ZoRRO: A Zero-Weight Personalized Recommender System for Scalable News Recommendation
...
- **Reason**: This paper focuses on recommender systems, specifically news recommendation, but lacks relevance to the user's core research themes in Information Retrieval, query understanding, ranking models, and user behavior modeling.
...
### 37. LightMem-Ego: Your AI Memory for Everyday Life
...
- **Topic Keywords**: query, queries, retrieval
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or Natural Language Processing, which are the primary areas of interest. While it involves multimodal memory and retrieval, the focus is on personal AI assistants and everyday-life assistance, which is not a central match to the user's research themes.
...
### 38. Domain-Aware Scaling Laws Uncover Data Synergy
...
- **Reason**: This paper appears to be related to machine learning and data synergy, but it does not directly align with the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on language model pretraining, the focus is on data synergy rather than query understanding, ranking models, or user behavior modeling.
...
### 39. One Token Is Enough: Fingerprinting and Verifying Large Language Models from Single-Token Output Distributions

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Tomas Bruckner
- **URL**: <http://arxiv.org/abs/2607.10252v1>
- **Submitted**: 2026-07-11 10:34:17
- **Topic Keywords**: query, queries
- **Reason**: This paper is not directly related to information retrieval, search technologies, or user behavior modeling. While it involves natural language processing, the focus is on fingerprinting and verifying large language models, which is a distinct area of research.
...
### 40. GEIS: A Generation-Evaluation-Improvement Loop of Agent Skills for Long-Form Article Generation
...
- **Reason**: This paper focuses on long-form article generation using multi-agent pipelines, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the context is more aligned with text generation and generation pipelines rather than query understanding, ranking models, or user behavior modeling.
...
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves large language models, the focus is on their accuracy in legal citation retrieval, which is a specific application rather than a core theme in your research.
...
### 42. Route, Communicate, and Reason: Gated Routing and Adaptive Depth for Efficient Multi-Agent Reasoning

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Sudipto Ghosh, Tanmoy Chakraborty
- **URL**: <http://arxiv.org/abs/2607.10836v1>
- **Submitted**: 2026-07-12 16:58:21
- **Topic Keywords**: query, rag
- **Reason**: This paper focuses on multi-agent reasoning and presents a hierarchical system for efficient reasoning, which is not directly related to information retrieval, search technologies, or natural language processing. While it involves some aspects of optimization, it is primarily concerned with multi-agent systems and does not appear to address query understanding, ranking models, or user behavior modeling, which are core areas of interest for your research.
...
### 43. Diagnosing and Mitigating Thinking Collapse in On-Policy Self-Distillation

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Keqin Peng, Chen Li, Yuanxin Ouyang, Yancheng Yuan, Liang Ding
- **URL**: <http://arxiv.org/abs/2607.10805v1>
- **Submitted**: 2026-07-12 15:24:25
- **Topic Keywords**: pointwise, rag
- **Reason**: This paper appears to be focused on addressing an optimization issue in Large Language Models, specifically a phenomenon called 'Thinking Collapse' in On-Policy Self-Distillation. While it involves deep learning and model optimization, it does not directly relate to Information Retrieval, Search technologies, or user behavior modeling, which are the core areas of your research interests.
...
### 44. STEP: Career-Path Recommendation via Temporal and Educational Trajectory Modeling
...
- **Reason**: This paper focuses on career-path recommendation and temporal/educational trajectory modeling, which is outside the primary scope of your research interests in Information Retrieval and Search technologies. While it involves some NLP techniques, its application domain is distinct from your areas of focus.
...
### 45. JobHop v2: A Large-Scale Career Trajectory Dataset from Unstructured Resumes
...
- **Topic Keywords**: rag, recommend, search
- **Reason**: This paper is not directly related to Information Retrieval or Search technologies, but rather focuses on career trajectory data and its extraction from resumes. While it involves Natural Language Processing (NLP) techniques, its primary goal is to support workforce planning and labour market analysis, which is not a central match to the user's research interests.
...
### 46. Agentic Routing: The Harness-Native Data Flywheel
...
- **Reason**: This paper focuses on routing methods for large language model agents, which is not directly related to information retrieval, search technologies, or query understanding. While it involves data and model selection, the context is more aligned with NLP and model training, rather than real-time relevance optimization or user behavior modeling.
...
### 47. UNIBROWSE: A Data-to-Agent Framework for Multimodal BrowseComp

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Xiyu Wei, Qingwei Zong, Zhuocheng Yu, Sujian Li
- **URL**: <http://arxiv.org/abs/2607.10557v1>
- **Submitted**: 2026-07-12 04:13:27
- **Comment**: 17 pages, 5 figures
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on multimodal BrowseComp tasks, which is not directly related to information retrieval, search technologies, or query understanding. While it involves data construction and agent training, the context is quite different from the user's core research themes.
...
### 48. Enjoy Your Talk: A Human-Centered Benchmark for Multi-Turn Dialogue with Decoupled User Simulation, Target Modeling, and Judging

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Jinglan Gong, Jiefan Lu, Hewei Guo, Kehan Li, Zhiyuan Han, Jihang Jiang, Wenwen Tong, Lewei Lu
- **URL**: <http://arxiv.org/abs/2607.10428v1>
- **Submitted**: 2026-07-11 18:22:49
- **Topic Keywords**: ranking, rank
- **Reason**: This paper focuses on multi-turn dialogue evaluation and benchmarking, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve language models and user behavior, the context is more centered on conversational AI and dialogue systems, making it less relevant to your primary areas of focus.
...
### 49. Adaptive Model Compression (AMC): Saliency-Driven Resource Allocation for Ultra-Low-Power Transformer Inference

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Jiayin Hu, Kai Yuan, Vanessa Hu, Xuetao Yin, Jianhua Li, Sean Suchter
- **URL**: <http://arxiv.org/abs/2607.10109v1>
- **Submitted**: 2026-07-11 04:05:26
- **Topic Keywords**: ltr, rank
- **Reason**: This paper focuses on model compression and ultra-low-power transformer inference, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the concept of resource allocation, it does not address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to the user's interests.
...
### 50. Index SLM Technical Report

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Lusheng Zhang, Shien He, Tianxing Yan, Mengran Yu, Ziang Cui, Kai Zhao, Xiaojing Liu, Tianjiao Li
- **URL**: <http://arxiv.org/abs/2607.09885v1>
- **Submitted**: 2026-07-10 18:19:47
- **Comment**: 26 pages, 10 figures
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper appears to be focused on the development of a series of open small language models, which is a topic related to Natural Language Processing (NLP). However, it does not seem to have any direct connection to Information Retrieval (IR), query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
