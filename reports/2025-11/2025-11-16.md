# Daily Papers Report - 2025-11-16
...
- **Topic Keywords**: ranking, relevance, user behavior, click, ctr, click-through rate, retrieval, commerce, e-commerce, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of e-commerce search advertising. The focus on multimodal representation learning, ranking, and click-through rate prediction aligns with your expertise. However, the emphasis on the e-commerce domain is a limiting factor, preventing a perfect match.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: MOON framework for multimodal representation learning in e‑commerce search advertising
- **Aim**: Achieve sustainable, large‑scale improvements in click‑through‑rate (CTR) and downstream search metrics by integrating multimodal embeddings into the entire Taobao search‑advertising pipeline
- **Rationale**: Multimodal signals (images, text, behavior) are under‑exploited; a systematic framework with an exchange‑rate metric is needed to quantify how intermediate gains translate to downstream performance and to guide optimization
- **Ground**: Five full‑scale iterations focused on data processing, training strategy, model architecture, and downstream application; deployed across retrieval, relevance, ranking, and other pipeline stages
- **Experiment**: Three‑stage training pipeline (Pretraining → Post‑training → Application); scaling‑law study varying training tokens, negative samples, and user‑behavior sequence length; evaluation on CTR‑prediction task showing +20% online CTR improvement, the largest gain in three years
- **Takeaway**: MOON demonstrates that iterative, multimodal integration guided by an exchange‑rate metric and informed by scaling laws can deliver substantial, sustainable CTR gains in real‑world e‑commerce advertising
...
### 2. GovScape: A Public Multimodal Search System for 70 Million Pages of Government PDFs
...
- **Topic Keywords**: queries, search, www
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of multimodal search systems and semantic text search. However, the focus on government PDFs and web archives is not directly aligned with your primary interests in e-commerce and real-time relevance optimization.
...
### 3. From Fact to Judgment: Investigating the Impact of Task Framing on LLM Conviction in Dialogue Systems
...
- **Reason**: This paper explores the impact of task framing on LLM conviction in dialogue systems, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on conversational judgment and dialogue systems is not a central match to your primary research interests in IR and Search technologies. The paper's connection to NLP is more relevant, but still not a strong alignment with your core themes.
...
### 4. PRSM: A Measure to Evaluate CLIP's Robustness Against Paraphrases
...
- **Reason**: The paper explores the robustness of CLIP against paraphrases, which is related to query understanding in Information Retrieval. However, the focus on multimodal models and linguistic variation is somewhat tangential to the user's primary interests in ranking models and user behavior modeling. The connection to fairness and equitable deployment is also a secondary aspect.
...
### 5. LEMUR: Large scale End-to-end MUltimodal Recommendation
...
- **Reason**: The paper focuses on multimodal recommendation systems, which is a related area to information retrieval, but it does not directly address query understanding, ranking models, or user behavior modeling. The use of end-to-end training and memory bank mechanism is innovative, but the application is primarily in the recommender systems domain, which is not the primary focus of the user's research interests.
...
### 6. When Data is the Algorithm: A Systematic Study and Curation of Preference Optimization Datasets
...
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and data-centric analysis, but it does not directly align with the user's primary focus on Information Retrieval (IR), query understanding, and ranking models. The paper's focus on preference optimization datasets and large language models is tangentially related to the user's interests in NLP, but it does not seem to have a direct connection to the user's core research themes.
...
### 7. Expert-Guided Prompting and Retrieval-Augmented Generation for Emergency Medical Service Question Answering
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of medical question answering and retrieval-augmented generation. However, the focus on medical domain and question answering is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling. The paper's use of large language models and retrieval strategies is somewhat relevant, but the application is limited to a specific domain.
...
### 8. ICX360: In-Context eXplainability 360 Toolkit
...
- **Reason**: The paper ICX360: In-Context eXplainability 360 Toolkit is somewhat related to the user's interests in Natural Language Processing (NLP) and related topics, but it does not directly align with the primary focus on Information Retrieval (IR), query understanding, ranking models, or user behavior modeling. The toolkit's focus on explaining Large Language Models is relevant to NLP, but it does not seem to be directly applicable to the user's core research themes.
...
### 9. Faithful Summarization of Consumer Health Queries: A Cross-Lingual Framework with LLMs
...
- **Reason**: The paper focuses on medical text summarization, which is somewhat related to information retrieval and natural language processing. However, the specific application domain (healthcare) and the use of large language models (LLMs) for summarization are not directly aligned with the user's core research themes in query understanding, ranking models, and user behavior modeling.
...
### 10. SRLF: An Agent-Driven Set-Wise Reflective Learning Framework for Sequential Recommendation
...
- **Reason**: The paper explores recommender systems and sequential recommendation, which are related to the user's interests in Information Retrieval and Search technologies. However, the focus on recommender systems and sequential recommendation is somewhat tangential to the user's primary interest in query understanding, ranking models, and user behavior modeling. The use of LLMs and contextual understanding is a relevant aspect, but it's not a central match for the user's research themes.
...
### 11. Align$^3$GR: Unified Multi-Level Alignment for LLM-based Generative Recommendation
...
- **Reason**: The paper explores recommender systems, specifically a novel framework for LLM-based generative recommendation. While it touches on aspects of user behavior modeling and semantic understanding, its primary focus is on recommendation, which is somewhat related to the user's interests in Information Retrieval and Search technologies. However, the paper's emphasis on generative recommendation and large-scale deployment in an industrial platform is not a central match for the user's research themes.
...
### 12. KGQuest: Template-Driven QA Generation from Knowledge Graphs with LLM-Based Refinement
...
- **Topic Keywords**: rag
- **Reason**: The paper discusses a method for generating questions and answers from knowledge graphs, which is related to information retrieval and natural language processing. However, the focus is on question generation and answer refinement, rather than search or ranking models, making it somewhat relevant but not a central match for your research interests.
...
### 13. Correcting Mean Bias in Text Embeddings: A Refined Renormalization with Training-Free Improvements on MMTEB
...
- **Reason**: The paper focuses on text embeddings and their bias correction, which is somewhat related to the user's interests in Information Retrieval and Natural Language Processing. However, the specific context of text embeddings and their applications does not directly align with the user's primary focus on query understanding, ranking models, and user behavior modeling.
...
### 14. M-DAIGT: A Shared Task on Multi-Domain Detection of AI-Generated Text
...
- **Reason**: The paper is somewhat related to information retrieval, but it focuses on detecting AI-generated text rather than query understanding, ranking models, or user behavior modeling. While it involves text analysis, the primary goal is not relevance optimization or deep semantic understanding. It touches on NLP, but the context is not directly applicable to the user's core research themes.
...
### 15. A Multifaceted Analysis of Negative Bias in Large Language Models through the Lens of Parametric Knowledge
...
- **Topic Keywords**: search
- **Reason**: This paper is somewhat related to your interests in Natural Language Processing (NLP) and large language models, but it focuses on a specific issue (negative bias) rather than query understanding, ranking models, or user behavior modeling. While it touches on the importance of context and prompting, it doesn't directly align with your core research themes in Information Retrieval.
...
### 16. LLM-as-a-Grader: Practical Insights from Large Language Model for Short-Answer and Report Evaluation
...
- **Topic Keywords**: search
- **Reason**: This paper explores the application of Large Language Models (LLMs) in educational tasks, specifically grading short-answer quizzes and project reports. While it touches on the use of LLMs, which is related to your interest in Natural Language Processing, it does not directly align with your core research themes in Information Retrieval, query understanding, or ranking models. The paper's focus on educational assessment and grading systems is somewhat relevant, but not central to your research interests.
...
### 17. Leveraging Parameter Space Symmetries for Reasoning Skill Transfer in LLMs
...
- **Reason**: This paper is loosely relevant to your research interests as it deals with Large Language Models (LLMs) and their parameter space alignment, which is a topic tangentially related to your work on query understanding and ranking models. However, the focus on LLMs and reasoning skill transfer is not directly aligned with your primary interests in Information Retrieval and Search technologies.
...
- **Reason**: This paper focuses on Large Language Models and their alignment with human preferences, using a novel inference-time alignment framework. While it touches on the idea of fine-grained control, which is related to query understanding and ranking models, the primary focus is on NLP and not directly on information retrieval or search technologies. The paper's relevance to your research interests is somewhat limited.
...
### 19. Unlocking Advanced Graph Machine Learning Insights through Knowledge Completion on Neo4j Graph Database
...
- **Reason**: This paper focuses on Graph Machine Learning and Knowledge Completion on Neo4j Graph Database, which is not directly related to Information Retrieval, Search technologies, or Natural Language Processing. While it involves data analysis, it does not address query understanding, ranking models, or user behavior modeling, making it less relevant to your core research interests.
...
- **Topic Keywords**: query, rag
- **Reason**: This paper focuses on Large Language Model (LLM) inference and Multi-Agent Debate, which is not directly related to Information Retrieval or Search technologies. While it involves NLP, the context and application are distinct from the user's core research themes.
...
### 21. Multimodal Peer Review Simulation with Actionable To-Do Recommendations for Community-Aware Manuscript Revisions
...
- **Reason**: This paper focuses on academic peer review simulation and multimodal LLMs, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the context is academic peer review, which is not a primary focus of the user's research. The paper's emphasis on multimodal LLMs and community-aware peer review simulation does not align with the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 22. LaoBench: A Large-Scale Multidimensional Lao Benchmark for Large Language Models
...
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or user behavior modeling, which are the core areas of your research interests. While it involves Natural Language Processing and large language models, its focus on low-resource languages and benchmarking is not aligned with your primary research themes.
...
### 23. Enhancing Group Recommendation using Soft Impute Singular Value Decomposition
...
- **Reason**: This paper focuses on group recommender systems, which is a related but distinct area from information retrieval and search technologies. While it involves data mining and matrix completion, it does not directly address query understanding, ranking models, or user behavior modeling, which are core aspects of your research interests.
...
### 24. Sabiá: Um Chatbot de Inteligência Artificial Generativa para Suporte no Dia a Dia do Ensino Superior
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper appears to focus on developing a chatbot for academic information access using Generative Artificial Intelligence and Retrieval-Augmented Generation. While it involves NLP and AI, it doesn't seem to be directly related to Information Retrieval, Search technologies, or query understanding, which are core areas of your research interests.
...
### 25. NOVA: An Agentic Framework for Automated Histopathology Analysis and Discovery
...
- **Reason**: This paper is not relevant to your research interests as it focuses on automated histopathology analysis and discovery, which is outside the scope of information retrieval, search technologies, and natural language processing. Although it involves computational problem solving, the context is specific to biomedical analysis and does not align with your primary focus on deep semantic understanding and real-time relevance optimization in the e-commerce or general search domains.
...
### 26. CLARITY: Contextual Linguistic Adaptation and Accent Retrieval for Dual-Bias Mitigation in Text-to-Speech Generation
...
- **Reason**: This paper focuses on text-to-speech generation and bias mitigation, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing, particularly in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 27. CardioEmbed: Domain-Specialized Text Embeddings for Clinical Cardiology
...
- **Topic Keywords**: retrieval, search
- **Reason**: This paper focuses on biomedical text embeddings for clinical cardiology, which is outside the user's primary research interests in Information Retrieval and Search technologies. Although it involves text embeddings and retrieval tasks, the domain is not aligned with the user's core research themes.
...
### 28. From Proof to Program: Characterizing Tool-Induced Reasoning Hallucinations in Large Language Models
...
- **Reason**: This paper appears to be unrelated to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus on Large Language Models and tool-augmented reasoning in mathematical problems does not align with your core themes of query understanding, ranking models, and user behavior modeling.
...
### 29. MedPath: Multi-Domain Cross-Vocabulary Hierarchical Paths for Biomedical Entity Linking
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on biomedical entity linking and named entity recognition, which is not directly related to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the specific domain and application are quite different from the user's core areas of focus.
...
### 30. TEDxTN: A Three-way Speech Translation Corpus for Code-Switched Tunisian Arabic - English
...
- **Reason**: This paper focuses on speech translation and code-switched Tunisian Arabic-English, which is outside the user's primary research interests in Information Retrieval and Search technologies. While it involves Natural Language Processing, it does not align with the user's specific areas of interest such as query understanding, ranking models, or user behavior modeling.
...
### 31. Proactive Hearing Assistants that Isolate Egocentric Conversations
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval and Search technologies, nor does it involve query understanding, ranking models, or user behavior modeling. While it does involve real-time processing and some aspects of audio processing, the focus on hearing assistants and conversational dynamics is not aligned with your primary interests in IR and NLP.
...
### 32. Language-Aided State Estimation
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on state estimation using natural language data, which is not directly related to Information Retrieval, Search technologies, or query understanding. While it involves Natural Language Processing, it's more focused on a specific application (state estimation) rather than the broader topics of interest.
...
### 33. Adverbs Revisited: Enhancing WordNet Coverage of Adverbs with a Supersense Taxonomy
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on enhancing WordNet's coverage of adverbs with a supersense taxonomy, which is a linguistics-based approach to NLP. While it touches on NLP applications, it doesn't directly relate to information retrieval, search technologies, or query understanding, making it less relevant to your core research interests.
...
### 34. Multi-agent Undercover Gaming: Hallucination Removal via Counterfactual Test for Multimodal Reasoning
...
- **Comment**: Accepted by AAAI 2026
- **Topic Keywords**: acl
- **Reason**: This paper appears to be unrelated to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus on multimodal reasoning in large language models and social deduction games does not align with your primary areas of interest.
...
### 35. Speech-Aware Long Context Pruning and Integration for Contextualized Automatic Speech Recognition
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or query understanding. While it involves deep learning and semantic understanding, the focus is on Automatic Speech Recognition, which is outside the user's primary research interests.
...
### 36. Enhancing Meme Emotion Understanding with Multi-Level Modality Enhancement and Dual-Stage Modal Fusion
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Meme Emotion Understanding, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve multimodal fusion and deep semantic understanding, the context and application are quite different from your areas of focus.
...
### 37. S2D-ALIGN: Shallow-to-Deep Auxiliary Learning for Anatomically-Grounded Radiology Report Generation
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text generation and multimodal alignment, the focus is on radiology report generation, which is not a central theme in your research background.
...
### 38. DiscoX: Benchmarking Discourse-Level Translation task in Expert Domains
...
- **Comment**: 36 pages
- **Topic Keywords**: rag
- **Reason**: This paper focuses on discourse-level translation in expert domains, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves machine translation and LLMs, the primary focus is on evaluation and benchmarking, rather than query understanding, ranking models, or user behavior modeling.
...
### 39. The Map of Misbelief: Tracing Intrinsic and Extrinsic Hallucinations Through Attention Patterns
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Large Language Models and hallucinations, which is not directly related to your core research themes in Information Retrieval and Search technologies. While it touches on attention patterns, it's more relevant to NLP and model uncertainty, which is a tangential interest.
...
### 40. MajinBook: An open catalogue of digital world literature with likes
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be related to data mining and NLP, but its focus on creating a digital catalogue of literature and evaluating linkage strategies does not align with your primary research interests in Information Retrieval, query understanding, and ranking models.
...
### 41. LAET: A Layer-wise Adaptive Ensemble Tuning Framework for Pretrained Language Models
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on pre-trained language models and their applications in the financial industry, but it does not directly relate to information retrieval, search technologies, or query understanding, which are core areas of your research interests.
...
### 42. SQuaD: The Software Quality Dataset
...
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus on software quality metrics and dataset creation for software analytics does not align with your primary areas of interest.
...
### 43. Evaluating Large Language Models on Rare Disease Diagnosis: A Case Study using House M.D
...
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval and Search technologies, nor does it involve query understanding, ranking models, or user behavior modeling. While it touches on Natural Language Processing, the focus is on large language models for rare disease diagnosis, which is not a central area of interest for you.
