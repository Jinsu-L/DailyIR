# Daily Papers Report - 2025-11-30

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. MCP vs RAG vs NLWeb vs HTML: A Comparison of the Effectiveness and Efficiency of Different Agent Interfaces to the Web (Technical Report)
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of query understanding and ranking models, as it compares different interfaces for large language model agents to interact with websites. The focus on evaluating the effectiveness and efficiency of these interfaces is also aligned with your interests in user behavior modeling and real-time relevance optimization. However, the paper's primary focus on e-commerce and web agents is somewhat tangential to your broader interests in NLP and data mining.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Comparison of web agent architectures for e-commerce task execution
- **Aim**: Evaluate effectiveness and efficiency of HTML browsing, RAG, MCP, and NLWeb architectures in executing e-commerce tasks
- **Rationale**: To identify optimal architectures for complex web interactions, balancing task performance with implementation feasibility
- **Ground**: Testbed evaluation using a dedicated e-commerce task environment, comparing four architectures: HTML browsing (baseline), RAG, MCP, and NLWeb
- **Experiment**: RAG, MCP, and NLWeb outperformed HTML-browsing agents by 9 F1 points on average. API-based systems (MCP, NLWeb) showed superior effectiveness but required more development effort. RAG demonstrated 2–5× fewer tokens and shorter execution times compared to HTML-browsing agents
- **Takeaway**: API-based architectures (MCP, NLWeb) and RAG offer significant performance advantages over HTML browsing for structured tasks, though requiring additional implementation resources. RAG provides a viable middle-ground solution leveraging crawled web content
...
### 2. ShoppingComp: Are LLMs Really Ready for Your Shopping Cart?
...
- **Topic Keywords**: retrieval, recommend, shopping, commerce, e-commerce, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the e-commerce domain, as it presents a benchmark for evaluating LLM-powered shopping agents. The focus on precise product retrieval, expert-level report generation, and safety critical decision making aligns with your interests in query understanding, ranking models, and user behavior modeling. However, the primary focus on LLMs and their limitations might not be a central match to your core research themes.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: A dual framework for product evaluation and Large Language Model (LLM) judging, combining structured data collection with benchmarking of AI decision-making.
- **Aim**: To establish a reliable system for evaluating products and benchmarking LLMs in judging tasks, ensuring alignment with human judgments and safety-critical scenarios.
- **Rationale**: The need for structured frameworks to evaluate products and AI decision-making arises from the complexity of real-world scenarios, where reliability, safety, and human alignment are critical.
- **Ground**: The framework includes three tools (Search, Link Summary, Link Reader) for product evaluation, four LLM judge prompts (Scenario Coverage, Rationale Validity, Safety-Critical, Reliability Ablations), and comparisons of models (Gemini-2.5-Pro, GPT-4o, DeepSeek-v3.1) using metrics like Selection Accuracy SoP.
- **Experiment**: Results from Table 5 show Gemini-2.5-Pro outperforms GPT-4o and DeepSeek-v3.1 in human-LLM agreement (75.6% vs. 73% and 68%) and exhibits the lowest self-preference bias. Sharma et al. (2025) corroborate these findings, but methodological gaps in ablation studies limit reproducibility.
- **Takeaway**: Gemini-2.5-Pro demonstrates superior reliability and human alignment in LLM judging, but transparency in evaluating AI systems remains a critical challenge. Structured frameworks are essential for benchmarking AI in complex, safety-critical contexts.
...
### 3. MegaChat: A Synthetic Persian Q&A Dataset for High-Quality Sales Chatbot Evaluation
...
- **Topic Keywords**: query, ranking, rerank, rag, retrieval, shopping, commerce, e-commerce, rank
- **Reason**: This paper is relevant to your research interests in Information Retrieval, particularly in the context of conversational AI and chatbots. The focus on query understanding, ranking models, and user behavior modeling is evident in the evaluation of retrieval-augmented generation models and the proposed agentic architecture. However, the specific domain of e-commerce in Persian language may not be a central match to your broader interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Evaluation of a multi-agent architecture for generating Persian question-answer (Q&A) pairs in Telegram-based e-commerce chatbots
- **Aim**: To assess the effectiveness of a multi-agent system in generating high-quality Q&A pairs for sales chatbots, outperforming traditional RAG models and introducing the MegaChat dataset.
- **Rationale**: The lack of Persian-specific datasets for e-commerce chatbots and the need for cost-effective solutions for SMEs in Telegram-based markets. The study addresses gaps in multilingual conversational AI and commercial domain benchmarking.
- **Ground**: The evaluation framework uses a permutation function to define ground truth labels ($𝐺𝑇(𝑞) = 𝜎(𝑞)[1] = 𝑎𝜋(1)$) and ranks answers as a string (e.g., '𝑎𝜋(1), 𝑎𝜋(2), 𝑎𝜋(3), 𝑎𝜋(4)'). The system is compared against three RAG models (GPT-4.1, GPT-4o, GPT-4-turbo) across five Telegram shopping channels.
- **Experiment**: The multi-agent system achieved superior performance in 4 out of 5 channels, generating 8, 22, 18, and 15 best-ranked responses. Key methods include persona-driven analysis, multi-query retrieval, and a GPT-5.1 evaluation layer for quality assurance.
- **Takeaway**: The proposed architecture outperforms RAG models without complex fine-tuning, introducing **MegaChat**, the first synthetic Persian Q&A dataset. This reduces data acquisition costs for SMEs and advances research in multilingual conversational AI. The 18 cited references contextualize the work within social media marketing, AI chatbot trends, and multilingual dataset development.
...
### 4. Do LLM-judges Align with Human Relevance in Cranfield-style Recommender Evaluation?
...
- **Topic Keywords**: ranking, relevance, retrieval, recommend, rank
- **Reason**: The paper explores the use of Large Language Models (LLMs) as automatic judges for evaluating recommender systems, which aligns with your interests in Information Retrieval and Search technologies. However, the focus on recommender systems and the use of Cranfield-style evaluation methodology is somewhat related to your interests, but not a central match. The paper's emphasis on scalability and practical applications is also relevant.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: LLM-judge: A Large Language Model-Based Framework for Bias-Aware Evaluation of Recommender Systems
- **Aim**: To address biases in traditional recommender system evaluation metrics and provide a scalable, automated alternative using Large Language Models (LLMs) for industrial deployment.
- **Rationale**: Traditional metrics like precision/recall exhibit popularity and exposure biases, while human annotation is costly and limited in scale. LLM-judge aims to mitigate these issues by leveraging LLMs' ability to evaluate recommendations based on diverse, context-aware criteria.
- **Ground**: The research builds on 41+ references spanning LLM-based relevance judgment ([12],[23]), offline evaluation strategies ([5],[18]), bias mitigation in recommenders ([30]), and contextualized evaluation benchmarks ([28]). It also references evolving test collections ([22]) and data splitting strategies ([29]).
- **Experiment**: Experiments demonstrated LLM-judge's effectiveness in reducing popularity bias, evaluating novel objectives (e.g., interest diversity), and complementing traditional metrics. Scalability was validated through industrial-scale testing, showing compatibility with A/B testing and human review prioritization.
- **Takeaway**: LLM-judge offers a promising framework for scalable, bias-aware evaluation but requires further research on human-LLM alignment, bias analysis in LLMs, and hybrid human-in-the-loop systems to ensure accountability and fairness.
...
### 5. ORION: Teaching Language Models to Reason Efficiently in the Language of Thought
...
- **Topic Keywords**: ltr
- **Reason**: The paper discusses a novel approach to reasoning in language models, which shares some similarities with query understanding and ranking models in Information Retrieval. However, the focus on symbolic, compositional mental language and efficient reasoning may not be directly applicable to IR, but it does demonstrate advancements in deep semantic understanding and real-time relevance optimization, which aligns with your broader interests in NLP and related topics.
...
### 6. Ambiguity Awareness Optimization: Towards Semantic Disambiguation for Direct Preference Optimization
...
- **Reason**: The paper is somewhat related to information retrieval, specifically addressing a challenge in Direct Preference Optimization (DPO), a method used in search technologies. However, the focus is on semantic disambiguation and preference optimization, which, while relevant to IR, does not directly align with the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 7. Every Token Counts: Generalizing 16M Ultra-Long Context in Large Language Models
...
- **Reason**: This paper explores ultra-long context modeling in large language models, leveraging a novel attention mechanism called Hierarchical Sparse Attention. While it touches on aspects of deep semantic understanding, its primary focus is on improving the performance of language models, which is somewhat related to information retrieval, but not directly aligned with your core research themes.
...
- **Topic Keywords**: retriever, rag, acl
- **Reason**: The paper explores the interpretability of language models, which is related to query understanding and ranking models in Information Retrieval. However, the focus on text generation and knowledge discovery is somewhat tangential to the user's primary interests in search technologies and user behavior modeling.
...
### 9. Modeling Romanized Hindi and Bengali: Dataset Creation and Multilingual LLM Integration
...
- **Reason**: This paper is somewhat related to your interests in Natural Language Processing (NLP) and Information Retrieval (IR), but it focuses on transliteration techniques and multilingual models, which is not a central match to your primary focus on query understanding, ranking models, and user behavior modeling.
...
### 10. ReAG: Reasoning-Augmented Generation for Knowledge-based Visual Question Answering
...
- **Reason**: This paper focuses on Visual Question Answering and Knowledge-based VQA, which is somewhat related to information retrieval, but the specific application and multimodal approach are not directly aligned with the user's core research themes in IR and search technologies. The use of multimodal large language models and knowledge retrieval is an interesting aspect, but the primary focus on VQA and KB-VQA limits its relevance to the user's interests.
...
### 11. Listwise Preference Optimization with Element-wise Confusions for Aspect Sentiment Quad Prediction
...
- **Topic Keywords**: listwise, rag
- **Reason**: The paper is somewhat relevant to Information Retrieval, particularly in the area of query understanding and ranking models, as it involves predicting a structured quadruple with sentiment elements. However, the focus on aspect sentiment quad prediction and natural language rationale generation is not directly aligned with the user's primary research interests in IR and search technologies. The paper's use of listwise preference optimization and element-wise confusions is also somewhat related to the user's interests in ranking models and user behavior modeling.
...
### 12. Tourism Question Answer System in Indian Language using Domain-Adapted Foundation Models
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and query understanding, but it focuses on a specific domain (tourism in Indian languages) and does not directly address your core areas of interest in Information Retrieval (IR) and Search technologies. The use of foundation models and fine-tuning techniques is relevant, but the application is not central to your research themes.
...
### 13. Masked Diffusion for Generative Recommendation
...
- **Reason**: The paper explores generative recommendation with semantic IDs, leveraging masked diffusion to improve performance over traditional autoregressive modeling. While it touches on NLP and related topics, the focus is on recommendation systems, which aligns with your broader interests but is not a central match to your primary focus on information retrieval and search technologies.
...
### 14. Training-Free Loosely Speculative Decoding: Accepting Semantically Correct Drafts Beyond Exact Match
...
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and deep semantic understanding, but it primarily focuses on improving the efficiency of language models rather than query understanding, ranking models, or user behavior modeling. While it explores a novel method for speculative decoding, its relevance to your core research themes is limited.
...
### 15. Decoding the Past: Explainable Machine Learning Models for Dating Historical Texts
...
- **Reason**: This paper focuses on temporal text classification and dating historical texts using machine learning models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves feature-engineered models and ranking capabilities, the context and application are distinct from your areas of focus.
...
### 16. RAG System for Supporting Japanese Litigation Procedures: Faithful Response Generation Complying with Legal Norms
...
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves Retrieval-Augmented Generation (RAG) and LLM systems, the focus is on a specific application in Japanese litigation procedures, which is not aligned with your primary research themes.
...
### 17. Scaling HuBERT for African Languages: From Base to Large and XL
...
- **Reason**: This paper is not relevant to your research interests as it focuses on multilingual speech processing and self-supervised learning for African languages, which is outside your primary areas of interest in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 18. Transformer-Driven Triple Fusion Framework for Enhanced Multimodal Author Intent Classification in Low-Resource Bangla
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on author intent classification in Bangla social media posts using multimodal data, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves transformer-based language models and fusion strategies, the context and application are quite different from your areas of focus.
...
### 19. BanglaSentNet: An Explainable Hybrid Deep Learning Framework for Multi-Aspect Sentiment Analysis with Cross-Domain Transfer Learning
...
- **Reason**: This paper focuses on multi-aspect sentiment analysis in the Bangla language, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it involves deep learning and ensemble learning, the context is specific to sentiment analysis and e-commerce in a low-resource language, making it less relevant to the user's interests.
...
### 20. Multi-chain Graph Refinement and Selection for Reliable Reasoning in Large Language Models
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on Large Language Models and their reasoning capabilities, proposing a novel framework for reliable reasoning. While it involves graph-based exploration mechanisms, it does not directly relate to information retrieval, search technologies, or query understanding, which are core areas of your research interests.
...
### 21. Bharat Scene Text: A Novel Comprehensive Dataset and Benchmark for Indian Language Scene Text Understanding
...
- **Topic Keywords**: commerce, e-commerce, search
- **Reason**: This paper focuses on scene text recognition in Indian languages, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. Although it mentions applications in search and e-commerce, the primary focus is on text recognition rather than query understanding, ranking models, or user behavior modeling.
...
### 22. Language-conditioned world model improves policy generalization by reading environmental descriptions
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on improving policy generalization in a model-based reinforcement learning approach, using a language-conditioned world model. While it involves language understanding and modeling, the context is primarily in robotics and human-agent interaction, which is not directly related to your core research themes in Information Retrieval and Search technologies.
...
### 23. FedAU2: Attribute Unlearning for User-Level Federated Recommender Systems with Adaptive and Robust Adversarial Training
...
- **Topic Keywords**: rag, recommend
- **Reason**: This paper focuses on federated recommender systems and attribute unlearning, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it touches on user behavior modeling, the context is recommender systems rather than search technologies, and the emphasis is on privacy and adversarial training rather than query understanding or ranking models.
...
### 24. ThetaEvolve: Test-time Learning on Open Problems
...
- **Topic Keywords**: rag
- **Reason**: This paper appears to be unrelated to Information Retrieval, Search technologies, query understanding, ranking models, or user behavior modeling. It focuses on test-time learning and mathematical discovery using large language models, which is outside the scope of your research interests.
...
### 25. Pooling Attention: Evaluating Pretrained Transformer Embeddings for Deception Classification
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on deception classification using pre-trained transformer embeddings, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing, particularly in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 26. Visual Puns from Idioms: An Iterative LLM-T2IM-MLLM Framework
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on visual puns generation and evaluation using a multimodal framework, which is not directly related to information retrieval, search technologies, or user behavior modeling. Although it involves natural language processing and text-to-image models, the application domain is not aligned with the user's core research themes.
...
### 27. Artwork Interpretation with Vision Language Models: A Case Study on Emotions and Emotion Symbols
...
- **Topic Keywords**: ctr
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval and Search technologies. The focus on vision language models for artwork interpretation and emotion detection is more aligned with NLP and computer vision, but lacks relevance to query understanding, ranking models, and user behavior modeling.
...
### 28. Is Passive Expertise-Based Personalization Enough? A Case Study in AI-Assisted Test-Taking
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval and Search technologies, as it focuses on expertise-based personalization in AI-assisted test-taking, which is more aligned with recommender systems and user experience. While it touches on user behavior modeling, it does not address query understanding, ranking models, or real-time relevance optimization, making it only loosely relevant to the user's interests.
...
### 29. Two-Stage Distributionally Robust Optimization Framework for Secure Communications in Aerial-RIS Systems
...
- **Comment**: 5 pages
- **Topic Keywords**: ctr
- **Reason**: This paper appears to be unrelated to the user's research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, Natural Language Processing, and data mining. The paper focuses on secure communications in Aerial-RIS Systems, which is outside the user's areas of interest.
