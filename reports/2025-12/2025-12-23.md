...
- **Topic**: Improving alignment between human and GPT-based evaluations of text summaries using a 'hint' strategy and uncertainty-based features.
- **Aim**: Enhance correlation between GPT and human ratings for relevance and coherence dimensions while identifying key features influencing evaluation outcomes.
- **Rationale**: Current evaluation methods for Natural Language Generators (NLGs) rely on reference-based metrics (e.g., BLEU, ROUGE) and heuristic-based approaches, which lack mathematical grounding and fail to align with human judgment standards.
- **Ground**: The study builds on prior work in human-centric evaluation protocols, LLM-as-a-judge techniques (e.g., Fusion-eval, Self-rationalization), and explainable frameworks (e.g., InstructScore). It critiques limitations of traditional metrics and references advancements like BERTScore, MoverScore, and BARTScore.
- **Experiment**: A 'hint' strategy was implemented via chain-of-thought prompting with a dedicated 'Hint' section in prompts. Uncertainty-based features (conditional perplexity, textual complexity, readability) were analyzed for their impact on evaluation outcomes.
- **Takeaway**: The 'hint' strategy significantly improves GPT-human alignment in relevance and coherence. Uncertainty-based features drive evaluation outcomes, but future research must prioritize mathematically grounded metrics to address current limitations in NLG evaluation.
...
- **Reason**: This paper is somewhat related to information retrieval, but its focus on uncertainty quantification in language models and retrieval-augmented generation is not directly aligned with your core research themes of query understanding, ranking models, and user behavior modeling. While it touches on the e-commerce domain indirectly through its application to multi-hop QA benchmarks, it does not appear to be a central match for your research interests.
...
### 5. Event Extraction in Large Language Model
...
- **Reason**: This paper focuses on event extraction, a related topic to information retrieval, but it primarily deals with natural language processing and large language models. While it touches on the idea of structurally reliable systems, it doesn't directly address query understanding, ranking models, or user behavior modeling, which are core areas of interest.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: Limitations and advancements in Event Extraction (EE) using Large Language Models (LLMs) and multimodal LLMs, focusing on structured reliability and system integration.
- **Aim**: To address LLM-based EE challenges (e.g., hallucinations, fragile temporal linking) by evolving EE into a structurally reliable, agent-ready perception and memory layer for open-world systems.
- **Rationale**: Current LLM-based EE pipelines struggle with hallucinations, weak temporal/causal linking, and limited context window knowledge management, hindering their reliability for complex, real-world applications.
- **Ground**: The survey traces EE method evolution from rule-based/neural models to instruction-driven/generative frameworks, emphasizing event schemas, slot constraints, graph-based RAG, and event stores for grounding, verification, and memory extension.
- **Experiment**: Not explicitly detailed; the focus is on surveying existing methods, datasets, and evaluation metrics, alongside proposing architectural shifts (e.g., event-centric structures, cross-lingual adaptations).
- **Takeaway**: Future EE systems should prioritize cognitive scaffolding via structured interfaces, relation-aware retrieval, and updatable memory to transition from static extraction to dynamic, agent-centric perception layers in open-world scenarios.
...
### 6. From Retrieval to Reasoning: A Framework for Cyber Threat Intelligence NER with Explicit and Adaptive Instructions
...
### 7. Auto-Prompting with Retrieval Guidance for Frame Detection in Logistics
...
### 8. Exploring Zero-Shot ACSA with Unified Meaning Representation in Chain-of-Thought Prompting
...
### 9. Bottom-up Policy Optimization: Your Language Model Policy Secretly Contains Internal Policies
...
### 10. MAGIC: Achieving Superior Model Merging via Magnitude Calibration
...
- **Topic Keywords**: search
- **Reason**: The paper discusses model merging and calibration, which is somewhat related to information retrieval and search technologies. However, the focus is on deep learning and model optimization, which is not a central match to the user's core research themes. The paper's relevance to the user's interests is limited to the potential application of model merging in search technologies, but the connection is not explicitly made.
...
### 11. Affordance RAG: Hierarchical Multimodal Retrieval with Affordance-Aware Embodied Memory for Mobile Manipulation
...
### 12. A Large-Language-Model Framework for Automated Humanitarian Situation Reporting
...
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves large language models and text analysis, its focus on humanitarian situation reporting and automated document transformation is outside the user's primary areas of interest.
...
- **Reason**: This paper is not relevant to your research interests as it focuses on benchmarking autonomous mobile agents and does not address topics such as query understanding, ranking models, or user behavior modeling in the context of information retrieval or search technologies.
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on low-resource language code generation and does not mention query understanding, ranking models, or user behavior modeling.
...
- **Reason**: This paper appears to be focused on drama script continuation evaluation, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves language models, the context and application are quite different from the user's interests.
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on speech representation learning and phonetic properties, which is not directly related to Information Retrieval, Search technologies, or Natural Language Processing in the context of query understanding, ranking models, or user behavior modeling.
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Large Language Models (LLMs) and their interactions, but it does not directly relate to Information Retrieval, Search technologies, or query understanding, which are core areas of your research interests. While it touches on natural language processing, the context is quite different from your background in e-commerce and IR.
...
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing, as it focuses on diacritic restoration for low-resource indigenous languages. While it involves NLP tasks, the specific area of study is not aligned with your primary focus on deep semantic understanding and real-time relevance optimization.
...
- **Reason**: This paper is not relevant to your research interests as it focuses on bias in language models and stigma mitigation, which is outside your primary areas of interest in Information Retrieval, Search technologies, and Natural Language Processing.
...
- **Topic Keywords**: rag
- **Reason**: This paper is unrelated to Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or the e-commerce domain. It focuses on Automatic Speech Recognition (ASR) for the Bambara language, which is outside the user's primary research interests.
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be unrelated to Information Retrieval, Search technologies, or any of your core research themes. It focuses on a theoretical model of generative linguistics, specifically the Merge operation in syntax, and its dynamical properties. The paper's abstract does not mention any concepts or techniques relevant to your research interests.
