...
- **Submitted**: 2026-07-15 02:52:22
- **Topic Keywords**: ranking, click, ctr, click-through rate, cvr, recommend, commerce, e-commerce, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the area of ranking models and user behavior modeling. The focus on scaling unified feature and sequence modeling for e-commerce search aligns with your background in the e-commerce domain and interest in real-time relevance optimization. However, the emphasis on generative search and unified feature modeling is not a central match to your primary focus on deep semantic understanding.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: TMallGS – A Scalable, Decoupled Transformer for E‑Commerce Ranking
- **Aim**: Develop a memory‑efficient, scalable CTR prediction model that handles heterogeneous e‑commerce features (short queries, long‑tail user histories, item attributes, global context) while preserving fine‑grained matching signals and ensuring stable deep training.
- **Rationale**: Existing DLRM‑style models suffer from memory‑bound sparse embeddings and shallow MLPs, leading to low GPU MFU and poor scaling. Unified transformer backbones treat all modalities identically, causing heterogeneity gaps, signal dilution, and gradient vanishing. TMallGS decouples semantic interactions from explicit signals, models bias explicitly, and introduces feature calibration to reduce FLOPs and noise, thereby overcoming these limitations.
- **Ground**: 1) Two‑Stage Feature Calibration: Field‑wise Saliency Reweighting (FSR) and Distribution‑Calibrated Projection (DCP) produce dense, calibrated embeddings with ~33 % fewer FLOPs. 2) Unified Heterogeneous Sequence: request‑centric token layout with per‑field QKV projections, visibility masks (context‑dominant, permutation‑invariant), and noise‑adaptive gated attention. 3) Late‑Stage Modulation: FiLM Late Fusion re‑injects high‑frequency signals; Bias Net corrects re‑transformation bias. 4) Training: Error‑Aware Progressive Training with auxiliary heads and adaptive curriculum stabilizes deep learning and focuses on hard samples.
- **Experiment**: Evaluated on the KDD 2026 e‑commerce CTR dataset (≈ 1 500 user actions per session). TMallGS achieved a 3.2 % relative AUC gain over the best DLRM baseline, 2.7 % over the best unified transformer (OneTrans), and reduced training time per epoch by 28 % due to lower FLOPs and higher MFU. Scaling experiments up to 8× GPU clusters maintained linear speedup, demonstrating efficient distributed training.
- **Takeaway**: Decoupling semantic and explicit interactions, coupled with lightweight feature calibration and late‑stage FiLM fusion, yields a transformer‑based CTR model that is both memory‑efficient and highly accurate. TMallGS sets a new state‑of‑the‑art for large‑scale e‑commerce ranking while enabling stable, scalable training across heterogeneous modalities.
...
### 2. Where Does the Noise Come From? A Variance-Components Decomposition of Non-Determinism in LLM Brand Answers

- **LLM Score**: 8
- **Keyword Score**: 12
- **Authors**: Dmitrij Żatuchin
- **URL**: <http://arxiv.org/abs/2607.13304v1>
- **Submitted**: 2026-07-14 22:12:52
- **Comment**: 18 pages, 6 tables, 3 code listings
- **Topic Keywords**: query, ranking, rag, retrieval, recommend, rank
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of large language models (LLMs) and query understanding. The study's focus on variance decomposition and reliability in LLM responses aligns with your interests in ranking models and user behavior modeling. Although the paper's primary focus is on LLMs, its implications for query understanding and ranking models make it a useful contribution to the field.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Reproducibility of LLM‑Generated Brand‑Reputation Scores
- **Aim**: Quantify the stochastic instability of commercial LLM APIs when generating brand‑reputation metrics and decompose its sources using Generalizability Theory (GT).
- **Rationale**: LLM outputs vary across prompts, languages, and models, threatening the reliability of academic research and commercial decisions that rely on sentiment, ranking, and recommendation scores derived from these models.
- **Ground**: Primary study: 20 Central‑Eastern‑European brands, 8 languages, 3 models (GPT‑5.2, Gemini 3 Flash, Perplexity), 15 prompts (including 5‑repeat stability probes). 12,960 target responses, 99.8 % usable. GT facets: query language (26.5 % variance), within‑prompt resampling (34.8 %), brand‑in‑context interaction (29.6 %), brand‑by‑language interaction (8.6 %). Brand‑by‑model and brand‑by‑prompt interactions negligible. Secondary tier: 3 brands, 8 languages, 3 models, 8 prompts for cross‑market validation.
- **Experiment**: Crossed random‑effects design with REML estimation. Stability probes repeated 5× per brand‑language‑model to assess within‑prompt variance. Decision‑study mapping reliability (ρ²) against query budget, exploring diversification across languages/models versus repetition. Exhaustive search identifies least‑cost designs achieving target reliability.
- **Takeaway**: Query language is the dominant source of variance; within‑prompt resampling and brand‑in‑context interaction are major contributors in stability tests. Diversifying languages and models reduces relative‑error variance far more efficiently than repeating prompts. Brand‑ranking reliability remains low even with full crossed design, but GT‑guided allocation offers a practical, cost‑effective sampling strategy for reproducible LLM‑derived brand metrics.
...
### 3. Personalizing Incremental Video Search with Hybrid Text and ID Embeddings

- **LLM Score**: 7
- **Keyword Score**: 21
- **Authors**: Vivek Kanojiya, Vishalaksh Aggarwal, Daeho Baek, Lyndon Kennedy, Xuetao Yin
- **URL**: <http://arxiv.org/abs/2607.13493v1>
- **Submitted**: 2026-07-15 06:42:50
- **Comment**: Accepted to the Industry Track of the 20th ACM Conference on Recommender Systems (RecSys 2026)
- **Topic Keywords**: queries, ranking, pairwise, relevance, rag, click, conversion rate, personalization, rank, search
- **Reason**: This paper explores personalization in video search, combining text and ID embeddings for ranking. While not directly focused on query understanding or ranking models, it does involve user behavior modeling and real-time relevance optimization, making it somewhat related to your research interests in Information Retrieval and Search technologies.
...
- **URL**: <http://arxiv.org/abs/2607.14157v1>
- **Submitted**: 2026-07-14 20:05:20
- **Comment**: Submitted to ACM TOI
- **Topic Keywords**: query, ranking, rerank, retrieval, rank
- **Reason**: This paper explores multi-domain retrieval, specifically addressing the issue of contamination in mixed-domain corpora. While it touches on ranking and risk control, which are related to information retrieval, the focus is more on the domain consistency aspect and conformal risk guarantees, which is somewhat tangential to the user's core research themes of query understanding, ranking models, and user behavior modeling.
...
### 5. Long-term User Engagement Optimization through Model-agnostic Downstream Rewards Learning
...
- **Submitted**: 2026-07-15 16:17:58
- **Comment**: Recsys 2026
- **Topic Keywords**: ranking, user action, recommend, rank, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the context of recommender systems. However, it focuses more on long-term user engagement optimization and retention, which is a related but distinct area from your primary focus on query understanding, ranking models, and user behavior modeling. The paper's emphasis on model-agnostic downstream rewards learning and its application to large-scale recommendation systems is also somewhat relevant to your interests in NLP and data mining.
...
### 6. DeepStress: Stress-Testing Deep Search Agents

- **LLM Score**: 6
- **Keyword Score**: 6
- **Authors**: Ismael Rousseau, Geraldine Damnati, Frederic Bechet
- **URL**: <http://arxiv.org/abs/2607.13920v1>
- **Submitted**: 2026-07-15 14:59:29
- **Comment**: 9 pages preprint
- **Topic Keywords**: relevance, retrieval, search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the area of search agents and robustness to poor-quality evidence. However, it does not directly focus on query understanding, ranking models, or user behavior modeling, which are your core areas of interest.
...
- **URL**: <http://arxiv.org/abs/2607.14035v1>
- **Submitted**: 2026-07-15 17:03:08
...
- **Topic Keywords**: ranking, rerank, relevance, rag, user behavior, retrieval, rank, search, emnlp
- **Reason**: This paper is somewhat related to information retrieval, but its focus on generative engines and optimization for visibility in search results is not a central match to your research interests in query understanding, ranking models, and user behavior modeling. While it touches on some related topics like relevance and context position, the paper's scope and methodology are distinct from your primary areas of focus.
...
### 8. Finding the Right Tables and Columns: A Benchmark and Corpus-Adaptive Embeddings for SQL Schema Retrieval

- **LLM Score**: 4
- **Keyword Score**: 10
- **Authors**: Qingcheng Zeng, Puxuan Yu, Aman Mehta, Fuheng Zhao, Rajhans Samdani
- **URL**: <http://arxiv.org/abs/2607.13311v1>
- **Submitted**: 2026-07-14 22:31:55
- **Topic Keywords**: retriever, queries, rag, retrieval
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the context of schema retrieval and corpus-adaptive embeddings. However, it focuses on SQL schema retrieval, which is a specific application area that may not be directly aligned with your core research themes. The paper's emphasis on deep semantic understanding and real-time relevance optimization is relevant, but the topic is not central to your research interests.
...
- **URL**: <http://arxiv.org/abs/2607.14161v1>
- **Submitted**: 2026-07-14 23:02:27
- **Comment**: Accepted at KDD '26: The 32nd ACM SIGKDD Conference on Knowledge Discovery and Data Mining
- **Topic Keywords**: rag, retrieval, recommend, shopping, commerce, e-commerce
- **Reason**: The paper is somewhat related to your research interests in Information Retrieval and Search technologies, but it focuses on a specific application in e-commerce and causal decision-making, which is not a central match to your core themes. While it involves ranking models and user behavior modeling, the context is more aligned with recommender systems and deep causal learning, which is only loosely relevant to your primary focus on query understanding and real-time relevance optimization.
...
### 10. Cluster with Auctions for Vector Search

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Swann Bessa, Pierre Fernandez, Gergely Szilvasy, Matthijs Douze, Hervé Jégou
- **URL**: <http://arxiv.org/abs/2607.13728v1>
- **Submitted**: 2026-07-15 11:42:07
- **Comment**: 10 pages, 6 figures. Under review at NeurIPS 2026
- **Topic Keywords**: query, queries, search
- **Reason**: This paper introduces a novel approach to vector search, focusing on optimizing search performance for query distributions. While it touches on indexing and partitioning, which are related to information retrieval, it does not explicitly address query understanding, ranking models, or user behavior modeling. The paper's emphasis on scalability and throughput suggests a strong connection to search technologies, but its primary focus is on improving search efficiency rather than deep semantic understanding.
...
### 11. Analogical Deep Research: Retrieving and Integrating Historical Analogies for Foresight Analysis

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Yongqiang Chen, Guangyi Chen, Yuewen Sun, Kun Zhang
- **URL**: <http://arxiv.org/abs/2607.13602v1>
- **Submitted**: 2026-07-15 08:49:00
- **Comment**: Ongoing project
- **Topic Keywords**: rag, search, acl
- **Reason**: This paper is somewhat related to your interests in Information Retrieval and Natural Language Processing, as it involves leveraging historical analogies for foresight analysis. However, the focus on causal reasoning and analogical research is not directly aligned with your primary research themes of query understanding, ranking models, and user behavior modeling. While the paper's use of Large Language Models is relevant, the application to foresight analysis and historical analogies is not a central match to your research interests.
...
### 12. Can We Steer the Black-Box? Towards Controllability-Centric Evaluation of Recommender Systems with Collaborative Agents

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Jiwen Zhou, Xiang Liu, Mingming Li, Pengbo Mo, Jiao Dai, Honglei Lv, Jizhong Han, Songlin Hu
- **URL**: <http://arxiv.org/abs/2607.13418v2>
- **Submitted**: 2026-07-15 03:37:06
- **Topic Keywords**: ctr, recommend, search
- **Reason**: The paper is somewhat related to the user's interests in Information Retrieval and Recommender Systems, but it focuses on controllability and steerability in recommender systems, which is not the primary focus of the user's research. The paper also involves collaborative agents and multi-agent frameworks, which is not directly related to the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 13. Can an Old Dog Be Taught New Tricks? Taking LLMs Beyond Sentence Level Translation

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Alaina Brandt
- **URL**: <http://arxiv.org/abs/2607.14040v1>
- **Submitted**: 2026-07-15 17:10:24
- **Comment**: Accepted for publication in HCI International 2026, Late Breaking Papers Proceedings, Springer LNCS
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your interests in Natural Language Processing (NLP) and related topics, but it does not directly align with your core research themes in Information Retrieval (IR), query understanding, ranking models, or user behavior modeling. The focus on machine translation and reformulation is an interesting application of NLP, but it does not seem to be a central match for your research interests.
...
### 14. Graded Entity-Familiarity Readouts in Language Models: Polish Adaptation, Cross-Language Robustness, and Refusal Steering

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Grzegorz Brzezinka
- **URL**: <http://arxiv.org/abs/2607.13568v1>
- **Submitted**: 2026-07-15 08:06:59
- **Comment**: 16 pages, 8 figures, 5 tables. Code and data: https://github.com/agentGreg/bielik-entity-familiarity
- **Topic Keywords**: rag
- **Reason**: This paper explores the concept of entity familiarity in language models, which is related to query understanding in Information Retrieval. However, the focus on language models and entity familiarity is somewhat tangential to the user's primary interests in ranking models and user behavior modeling. The connection to NLP is relevant, but the specific application to language models is not central to the user's research themes.
...
### 15. Meta-Learning Preferences for Multilingual LLM Alignment

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Jiaying Lin, Seongho Son, Nam Phuong Tran, Long Tran-thanh, Ilija Bogunovic, Debmalya Mandal
- **URL**: <http://arxiv.org/abs/2607.13315v1>
- **Submitted**: 2026-07-14 22:38:39
- **Topic Keywords**: rag
- **Reason**: This paper explores meta-learning for multilingual large language model alignment, leveraging preference data from other languages to adapt to a target language. While it touches on aspects of preference optimization, it primarily focuses on language model alignment and transfer learning, which is somewhat related to information retrieval but not directly aligned with your core research themes.
...
### 16. MyAG: A Graph-Based Framework for Designing and Analyzing Composable LLM Agent Systems

- **LLM Score**: 4
- **Keyword Score**: 1
- **Authors**: Zhisong Zhang
- **URL**: <http://arxiv.org/abs/2607.13474v1>
- **Submitted**: 2026-07-15 06:03:52
- **Topic Keywords**: search
- **Reason**: The paper discusses a graph-based framework for designing and analyzing composable LLM agent systems. While it touches on the use of LLMs, which are related to NLP, the primary focus is on agent systems and their composition, which is not directly related to the user's core research themes in IR and search technologies. However, the paper's use of graph-based abstractions and workflow graphs may be of interest to the user's broader background in data mining and related topics.
...
### 17. Memory as a Controlled Process: Learned Adaptive Memory Management for LLM Agents
...
- **Submitted**: 2026-07-15 08:32:40
- **Topic Keywords**: queries, retrieval
- **Reason**: This paper is loosely relevant to your research interests in Information Retrieval and Natural Language Processing, as it involves memory management for Large Language Model (LLM) agents. However, the focus is on memory management rather than query understanding, ranking models, or user behavior modeling, which are your core areas of interest.
...
### 18. Cost-Pragmatic Quality Gating and Selection-Fusion Multi-Model Combiners for BioASQ Phases A+ and B

- **LLM Score**: 2
- **Keyword Score**: 7
- **Authors**: Dima Galat, Marian-Andrei Rizoiu
- **URL**: <http://arxiv.org/abs/2607.13551v1>
- **Submitted**: 2026-07-15 07:56:33
- **Comment**: CLEF2026, BioASQ2026
- **Topic Keywords**: rag, retrieval, rank, acl
- **Reason**: This paper appears to be focused on a specific task (BioASQ) and uses techniques such as language models and quality gates, but it does not align with the user's core research themes in Information Retrieval, particularly query understanding, ranking models, and user behavior modeling.
...
### 19. Measuring What the Crawler Sees: Discovery Curves, Core Persistence, and Shell Dynamics in Longitudinal Web Crawls

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Michael Paris, Hande Celikkanat, Luca Foppiano
- **URL**: <http://arxiv.org/abs/2607.13636v1>
- **Submitted**: 2026-07-15 09:29:24
- **Comment**: 16 pages, 4 figures, web metrics
- **Topic Keywords**: pairwise, rag, rank
- **Reason**: This paper appears to be unrelated to your primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus on web crawls, longitudinal web crawls, and urn models does not align with your core themes of query understanding, ranking models, and user behavior modeling.
...
### 20. RAGthoven at SemEval-2026 Task 1: A Multi-Stage Pipeline Walks Into a Benchmark and Barely Clears the Bar

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Marek Šuppa, Viktória Ondrejová, Lucia Ganajová, Gregor Karetka, Daniel Skala
- **URL**: <http://arxiv.org/abs/2607.13189v1>
- **Submitted**: 2026-07-14 18:39:33
- **Comment**: SemEval-2026 Task 1
- **Topic Keywords**: rag, retrieval, rank
- **Reason**: This paper appears to be focused on humor generation using a multi-stage large language model pipeline, which is unrelated to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve language models, the context and application are distinct from the user's areas of focus.
...
### 21. Evaluation Ability Does Not Imply Optimization Utility: LLM-as-a-Judge Signals in Closed-Loop Table Recognition

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Donghwan Kim
- **URL**: <http://arxiv.org/abs/2607.13347v1>
- **Submitted**: 2026-07-15 00:14:31
- **Comment**: 31 pages, 9 figures, appendix included
- **Topic Keywords**: ranking, rank
- **Reason**: This paper appears to be focused on the evaluation of Large Language Models (LLMs) in a specific task (table recognition), but it does not align with the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. The paper's focus on LLMs as evaluators and the evaluation of their ability does not seem to be directly related to query understanding, ranking models, or user behavior modeling, which are key areas of interest for the user.
...
### 22. Adaptive Filtering of the KV Cache: Diagnosing and Correcting Structural-Role Bias in LLM Inference

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Soumil Mandal
- **URL**: <http://arxiv.org/abs/2607.13205v1>
- **Submitted**: 2026-07-14 18:55:20
- **Comment**: 6 pages, 2 figures, 5 tables
- **Topic Keywords**: ranking, rank
- **Reason**: This paper appears to be primarily focused on optimizing the performance of Large Language Models (LLMs) through cache management, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on aspects of ranking and filtering, the context is specific to LLM inference and does not align with the user's focus on query understanding, user behavior modeling, and real-time relevance optimization.
...
### 23. Gauge-Invariant, Parameter-Insensitive Regularization for Potential Recovery from Flow on Directed Graphs

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Mohammad Forouhesh
- **URL**: <http://arxiv.org/abs/2607.13609v1>
- **Submitted**: 2026-07-15 08:56:57
- **Comment**: 17 pages, 6 figures, submitted to LoG 2026
- **Topic Keywords**: click, rank
- **Reason**: This paper appears to be primarily focused on a mathematical problem in graph theory and its application to graph neural networks. While it mentions 'clickstream corpora', the context suggests a different type of analysis than what is typically associated with information retrieval and search technologies. The paper's emphasis on gauge-invariant regularization and parameter-insensitivity does not seem to align with the user's core research themes in IR, NLP, and related topics.
...
- **URL**: <http://arxiv.org/abs/2607.13477v1>
- **Submitted**: 2026-07-15 06:10:53
- **Topic Keywords**: pairwise
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, or user behavior modeling. The paper focuses on auditing protocol-level shortcuts in Large Audio-Language Models for speech evaluation, which is outside your primary areas of interest.
...
### 25. The Refusal Residue: When Probes Catch Alignment Faking and When They Don't

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Aman Mehta
- **URL**: <http://arxiv.org/abs/2607.13346v1>
- **Submitted**: 2026-07-15 00:11:42
- **Comment**: Accepted to the Mechanistic Interpretability Workshop at ICML 2026. 12 pages, 4 figures
- **Topic Keywords**: query
- **Reason**: This paper appears to be unrelated to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The topic of alignment faking and detection in language models is more relevant to the field of Artificial Intelligence and Machine Learning, specifically in the area of model evaluation and trustworthiness.
...
- **Submitted**: 2026-07-15 17:21:43
- **Topic Keywords**: retrieval
- **Reason**: This paper focuses on evaluating Large Language Models (LLMs) using a novel approach called Hindcast, which aims to close the leaks in traditional backtesting methods. While it touches on the topic of model evaluation, it doesn't directly relate to information retrieval, search technologies, or user behavior modeling, which are the core areas of your research interests.
...
### 27. Do Agent Optimizers Compound? A Continual-Learning Evaluation on Terminal-Bench 2.0

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Wenxiao Wang, Priyatham Kattakinda, Soheil Feizi
- **URL**: <http://arxiv.org/abs/2607.14004v1>
- **Submitted**: 2026-07-15 16:36:04
- **Comment**: Technical Report by RELAI (relai.ai)
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or Natural Language Processing. The paper focuses on agent optimizers and continual learning, which is outside your primary areas of interest.
...
### 28. SPyCE: Skill-Policy Co-evolution for Multimodal Agents

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Ru Zhang, Weijie Qiu
- **URL**: <http://arxiv.org/abs/2607.13854v1>
- **Submitted**: 2026-07-15 14:01:48
- **Topic Keywords**: retrieval
- **Reason**: This paper appears to be primarily focused on reinforcement learning for multimodal agents, which is not a central match to your research interests in Information Retrieval and Search technologies. While it does involve some form of optimization, the context and methodology are quite different from your areas of expertise.
...
### 29. The Test Oracle Problem in Synthetic LLM-as-Judge Corpora: Disappearance, Distortion and a Validation Protocol

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Serkan Ballı
- **URL**: <http://arxiv.org/abs/2607.13707v1>
- **Submitted**: 2026-07-15 11:12:54
- **Comment**: 23 pages, 1 figure, 3 tables
- **Topic Keywords**: acl
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. It focuses on the test oracle problem in synthetic LLM-as-judge corpora, which is a specific issue in the field of Natural Language Processing, but not a central match to your core research themes.
...
### 30. Live Gurbani Tracking: A Benchmark and Reference System for Captioning Sikh Kirtan

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Karanbir Singh
- **URL**: <http://arxiv.org/abs/2607.13457v1>
- **Submitted**: 2026-07-15 05:33:44
...
- **Topic Keywords**: acl
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on a specific task of live captioning of Sikh Kirtan and does not address query understanding, ranking models, or user behavior modeling.
...
### 31. GFlowRL: Scaling Distribution-Matching RL to Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Xiaodong Liu, Michael Xu, Jack W. Stokes, Paul Smolensky, Doug Burger, Jianfeng Gao
- **URL**: <http://arxiv.org/abs/2607.13394v1>
- **Submitted**: 2026-07-15 02:43:16
- **Comment**: 31 pages, 8 figures, 17 tables
- **Topic Keywords**: rag
- **Reason**: This paper focuses on reinforcement learning for large language models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on deep semantic understanding, the context is specific to reinforcement learning and large language models, making it somewhat tangential to your primary focus.
...
### 32. Classifying daily activities needs posture, reconstructing them needs motion

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Arefeh Farahmandi, Gunnar Blohm
- **URL**: <http://arxiv.org/abs/2607.13216v1>
- **Submitted**: 2026-07-14 19:21:32
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The paper's focus on movement classification and reconstruction using visual input is outside your primary areas of interest.
...
### 33. Do LLMs Need Architectural Changes for Simultaneous Speech Translation? A Prefix-to-Prefix Data Driven Approach

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Junkun Chen, Jian Xue, Ming Tang, Abdel Heba, Hoda Gholami, Ruchao Fan, Jinyu Li
- **URL**: <http://arxiv.org/abs/2607.13158v1>
- **Submitted**: 2026-07-14 18:07:18
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing, as it focuses on Simultaneous Speech Translation and its application to decoder-only LLM systems.
...
### 34. High-Order Question Generation in a Multilingual Educational Context

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Suna-Şeyma Uçar, Itziar Aldabe, Nora Aranberri, Orphée De Clercq
- **URL**: <http://arxiv.org/abs/2607.13901v1>
- **Submitted**: 2026-07-15 14:42:08
- **Comment**: This paper was accepted at the 15th edition of the Language Resources and Evaluation Conference (LREC 2026)
- **Topic Keywords**: search
- **Reason**: This paper focuses on question generation using large language models in a multilingual educational context, which is not directly related to your research interests in Information Retrieval, Search technologies, and Natural Language Processing, particularly in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 35. DevicesWorld: Benchmarking Cross-Device Agents in Heterogeneous Environments
...
- **Submitted**: 2026-07-15 05:53:27
- **Comment**: https://github.com/AgenticOrgLab/DevicesWorld
- **Topic Keywords**: search
- **Reason**: This paper focuses on cross-device agents and benchmarking in heterogeneous environments, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves natural-language user goals, the primary focus is on multi-device collaboration, which is not a central theme in your research.
...
### 36. When Rubrics Change: Cross-Rubric Generalization for Critical Thinking Essay Scoring
...
- **URL**: <http://arxiv.org/abs/2607.13433v1>
- **Submitted**: 2026-07-15 04:34:14
- **Comment**: Published in AI for Education Day at SIGKDD 2026
- **Topic Keywords**: search
- **Reason**: This paper focuses on automated essay scoring and cross-rubric generalization, which is unrelated to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves a Large Language Model (LLM), the context and application are distinct from the user's areas of focus.
...
### 37. Discrete Diffusion Models: A Unified Framework from Tokenization to Generation
...
- **Submitted**: 2026-07-15 04:23:22
- **Topic Keywords**: search
- **Reason**: This paper appears to be primarily focused on discrete denoising diffusion models for discrete data, which is not directly related to information retrieval, search technologies, or query understanding. While it touches on the concept of tokenization, it is more relevant to natural language processing and generation tasks, which, although related, are not the primary focus of the user's research interests.
...
### 38. Where Should RL Post-Training Compute Go? Model Size, Search, Learning, and Feedback

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Patrick Wilhelm, Odej Kao
- **URL**: <http://arxiv.org/abs/2607.13389v1>
- **Submitted**: 2026-07-15 02:34:35
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it touches on reinforcement learning, it's primarily focused on the computational aspects of post-training adaptation in robot-learning pipelines, which is not a central match to your interests.
...
### 39. A POS Tier Is the Key to Automated Annotation for Low-Resource Language Documentation: Neural Interlinear Glossing for Irabu, a Southern Ryukyuan Language

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Michinori Shimoji
- **URL**: <http://arxiv.org/abs/2607.13372v1>
- **Submitted**: 2026-07-15 01:33:25
...
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus is on language documentation, annotation, and field linguistics, which are unrelated to your core themes.
...
### 40. MemoHarness: Agent Harnesses That Learn from Experience

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Yue Huang, Wenjie Wang, Han Bao, Yuchen Ma, Xiaonan Luo, Yi Nian, Haomin Zhuang, Zheyuan Liu, Yue Zhao, Xiangliang Zhang
- **URL**: <http://arxiv.org/abs/2607.14159v1>
- **Submitted**: 2026-07-14 21:22:18
- **Topic Keywords**: search
- **Reason**: This paper appears to be primarily focused on harnessing large language models (LLMs) for agent behavior, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves optimization and adaptation, the context and scope are distinct from the user's areas of focus.
...
### 41. GSM-Plus-BN: A Perturbation-Based Benchmark for Bangla Mathematical Reasoning in Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Bidyarthi Paul, Nahida Jannat Mayouree, Md. Asif Karim, Sagar Chandra Nath, Swastika Kundu
- **URL**: <http://arxiv.org/abs/2607.13248v1>
- **Submitted**: 2026-07-14 20:23:27
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, and Natural Language Processing, as it focuses on Bangla mathematical reasoning in Large Language Models and does not address query understanding, ranking models, or user behavior modeling.
...
### 42. Text2Sign: A Single-GPU Diffusion Baseline for Text-to-Sign Language Video Generation

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Ruize Xia
- **URL**: <http://arxiv.org/abs/2607.13164v1>
- **Submitted**: 2026-07-14 18:15:32
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. It focuses on text-to-sign language video generation using a diffusion model, which is outside your areas of expertise.
