# Daily Papers Report - 2025-11-24

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. Path-Constrained Retrieval: A Structural Approach to Reliable LLM Agent Reasoning Through Graph-Scoped Semantic Search
...
- **Comment**: 10 pages
- **Topic Keywords**: semantic search, relevance, rag, retrieval, rank, search
- **Reason**: This paper aligns well with your research interests in Information Retrieval, particularly in query understanding and ranking models. The focus on structural consistency and semantic search in the context of Large Language Model agents is highly relevant to your work. The evaluation of the proposed method on a benchmark dataset further supports its relevance to your research.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Path‑Constrained Retrieval (PCR) for Retrieval‑Augmented Generation
- **Aim**: Guarantee that every retrieved node lies on a directed path from a query‑derived anchor, thereby enforcing structural consistency while maintaining semantic relevance.
- **Rationale**: Conventional RAG methods treat all candidate passages equally, often retrieving nodes disconnected from the query’s anchor and producing incoherent evidence for multi‑hop reasoning. PCR restricts the search space to the reachable subgraph, ensuring that all returned nodes are structurally related to the anchor and thus more coherent for downstream LLM reasoning.
- **Ground**: PCR is defined on a directed knowledge graph (G=(V,E)) with node embeddings. Given a query (q) and an anchor node (a) (the most semantically similar node to (q)), PCR selects (Rsubseteq V) such that each (vin R) is reachable from (a) and maximizes cosine similarity to (q). The algorithm performs a BFS from (a) (optionally depth‑limited), embeds (q) with the same model as nodes, and conducts a constrained FAISS search over the reachable set. A hybrid score combines vector similarity and BM25. The PathRAG‑6 benchmark consists of six synthetic subgraphs (Tech, Legal, Bio, Microservices, Citations, Medical) with 30 nodes each, 120 queries, and metrics including Relevance@k, Structural Consistency, Multi‑hop Consistency, and Graph Distance Penalty.
- **Experiment**: PCR was evaluated against vector, BM25, and hybrid baselines. Key results: Structural Consistency 1.00 (vs 0.32–0.24 for baselines); Relevance@10 0.70 (vs 0.78–0.80 for baselines); average graph distance penalty 0.16 (vs ~0.75). Domain‑wise, PCR achieved perfect consistency across all six domains; relevance was perfect in Tech, zero in Legal, and 0.5 in others. Statistical tests showed PCR vs hybrid (p=0.017), Cohen’s d = –0.46. Ablation on depth showed depth‑1 slightly reduced relevance but maintained consistency. Latency averaged 42.3 ms, with reachability computation 2.1 ms. Implementation used OpenAI `text-embedding-3-small`, FAISS, NetworkX, and BM25 with (k_1=1.5, b=0.75).
- **Takeaway**: PCR provides a lightweight, principled way to enforce structural consistency in RAG, achieving perfect consistency and competitive relevance with minimal latency overhead. It is robust across domains, scales to larger KGs, and offers clear avenues for future work such as adaptive depth, learned anchor selection, and integration into end‑to‑end multi‑hop reasoning pipelines.
...
### 2. Rethinking Retrieval: From Traditional Retrieval Augmented Generation to Agentic and Non-Vector Reasoning Systems in the Financial Domain for Large Language Models
...
- **Topic Keywords**: ranking, rerank, pairwise, rag, retrieval augmented generation, retrieval, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of query understanding and ranking models. The focus on Retrieval-Augmented Generation (RAG) and its application to financial domain is somewhat niche, but the exploration of vector-based and non-vector RAG architectures aligns with your interests in deep semantic understanding and real-time relevance optimization.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Retrieval-Augmented Generation for Financial Question‑Answering over SEC Filings
- **Aim**: Develop an end‑to‑end benchmark that systematically compares vector‑based agentic RAG with hierarchical node‑based retrieval, evaluates cross‑encoder reranking and small‑to‑big chunk expansion, and measures retrieval accuracy, answer quality, latency, and cost.
- **Rationale**: Existing studies evaluate retrieval or generation in isolation, leaving a gap in systematic comparison of vector‑based RAG versus non‑vector structured pipelines and the isolated impact of enhancement techniques. Accurate, low‑latency, low‑cost QA over long, domain‑specific regulatory filings is critical for financial decision‑making.
- **Ground**: SEC filings (10‑K, 10‑Q, 8‑K) are lengthy, domain‑specific documents. Prior work (Setty et al., 2024; Michel et al., 2025; Wang et al., 2025) lacks joint evaluation of retrieval and generation. Retrieval architectures include vector‑based agentic RAG (semantic + BM25, LLM query formulation) and hierarchical node‑based reasoning (GPT‑4o generated node trees). Enhancements such as cross‑encoder reranking (Cohere) and small‑to‑big retrieval expand context while preserving precision. A 1,200‑filing corpus and 150 annotated questions provide a balanced benchmark. Evaluation metrics cover MRR, Recall@5, answer quality via LLM‑judge, latency, and cost.
- **Experiment**: The benchmark partitions questions for architecture comparison, reranking, and chunk expansion. Vector‑based agentic RAG uses 512‑token chunks, OpenAI embeddings, Azure AI Search, and an LLM agent for query formulation. Hierarchical node‑based reasoning traverses GPT‑4o‑generated node trees. Cross‑encoder reranking fine‑grains relevance after initial retrieval; parameter sweep over (k_initial,k_final) identifies (10,5) as optimal. Small‑to‑big retrieval expands retrieved chunks by two neighbors asynchronously. Metrics are computed using Azure/OpenAI APIs, LLM‑judge, profiling tools, and cost calculators. Results show vector‑based RAG outperforms node‑based on MRR and Recall@5; reranking and chunk expansion further improve answer quality with minimal latency and cost increases.
- **Takeaway**: Vector‑based agentic RAG augmented with cross‑encoder reranking and small‑to‑big retrieval delivers the highest retrieval precision and answer quality while keeping latency and cost comparable to non‑vector baselines. Hierarchical node‑based reasoning remains a viable low‑cost alternative when embedding costs or latency are critical. The benchmark framework offers a reproducible foundation for future research on RAG in regulated domains.
...
### 3. Principled Context Engineering for RAG: Statistical Guarantees via Conformal Prediction
...
- **Reason**: This paper is highly relevant to Information Retrieval, specifically in the context of Retrieval-Augmented Generation (RAG), which aligns with your interests in query understanding and ranking models. The focus on context engineering and statistical guarantees via conformal prediction also resonates with your background in e-commerce and interest in real-time relevance optimization.
...
### 4. Toward an AI-Native Internet: Rethinking the Web Architecture for Semantic Retrieval
...
- **Reason**: This paper aligns with your interests in Information Retrieval, particularly in the context of semantic retrieval and AI-driven systems. The focus on rethinking web architecture for AI-native retrieval is relevant to your work on query understanding and ranking models. However, the emphasis on Generative AI Search and LLMs is not a primary focus of your research, which is why we score it an 8.
...
### 5. Agent-as-a-Graph: Knowledge Graph-Based Tool and Agent Retrieval for LLM Multi-Agent Systems
...
- **Topic Keywords**: retriever, queries, ranking, rerank, retrieval augmented generation, retrieval, rank, search
- **Reason**: The paper discusses a knowledge graph-based retrieval approach for Large Language Model Multi-Agent Systems, which is somewhat related to information retrieval and search technologies. However, the focus on multi-agent systems and knowledge graph retrieval is not directly aligned with the user's core research themes in query understanding, ranking models, and user behavior modeling. The paper's relevance is mostly due to the use of retrieval and ranking techniques, but the context is quite different from the user's primary interests.
...
### 6. Table Comprehension in Building Codes using Vision Language Models and Domain-Specific Fine-Tuning
...
- **Reason**: This paper explores the application of Vision Language Models (VLMs) for extracting information from tabular data in building codes. While it touches on information retrieval and natural language processing, the primary focus is on fine-tuning VLMs for understanding complex structured data, which is somewhat related to the user's interests in query understanding and ranking models, but not a central match.
...
### 7. Save, Revisit, Retain: A Scalable Framework for Enhancing User Retention in Large-Scale Recommender Systems
...
- **Reason**: The paper focuses on recommender systems and user retention, which is somewhat related to your interests in Information Retrieval and Search technologies. However, it does not directly address query understanding, ranking models, or user behavior modeling, which are core areas of your research. The paper's emphasis on recommender systems and scalability is also somewhat tangential to your broader interests in NLP, data mining, and deep semantic understanding.
...
### 8. Multi-Agent Collaborative Filtering: Orchestrating Users and Items for Agentic Recommendations
...
- **Reason**: The paper explores recommender systems, which is a related topic to information retrieval. However, the focus on large language model agents and multi-agent collaboration is not directly aligned with the user's primary interests in query understanding, ranking models, and user behavior modeling. The connection to information retrieval is somewhat loose, primarily through the use of collaborative filtering, which is a technique also used in IR.
...
### 9. From Archives to Decisions: Multi-Agent Pharmaceutical Co-Scientist for Traceable Drug Discovery and Reverse Translation
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, particularly in the context of semantic retrieval and cross-document linking. However, the focus on pharmaceutical research and reverse translation is not directly aligned with your core themes of query understanding, ranking models, and user behavior modeling. The paper's emphasis on real-world applications and expert evaluation is also not a central match for your interests.
...
### 10. Token-Controlled Re-ranking for Sequential Recommendation via LLMs
...
- **Reason**: The paper explores recommender systems, specifically using Large Language Models (LLMs) for re-ranking, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the focus on recommender systems and sequential recommendations is not a central match to your primary research themes, which include query understanding, ranking models, and user behavior modeling.
...
### 11. LLM Reasoning for Cold-Start Item Recommendation
...
- **Reason**: The paper explores recommender systems, specifically addressing the cold-start problem, which is somewhat related to the user's interests in Information Retrieval and Search technologies. However, the focus on recommender systems and the lack of emphasis on query understanding, ranking models, and user behavior modeling limit its relevance. The use of Large Language Models (LLMs) is an interesting aspect, but it is not directly connected to the user's core research themes.
...
### 12. A Multimodal Conversational Agent for Tabular Data Analysis
...
- **Reason**: This paper explores the application of large language models in a conversational agent for data analysis, which is somewhat related to information retrieval and search technologies. However, the focus on multimodal conversational agents and data analysis is not a central match to your core research themes in query understanding, ranking models, and user behavior modeling.
...
### 13. Time Matters: Enhancing Sequential Recommendations with Time-Guided Graph Neural ODEs
...
- **Reason**: The paper focuses on sequential recommendations and graph neural networks, which are somewhat related to information retrieval and search technologies. However, the emphasis on sequential recommendations and time-guided graph neural ODEs is more aligned with recommender systems, which is a secondary interest of the user. The paper does not directly address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to the user's core research themes.
...
### 14. Large Language Model Enhanced Graph Invariant Contrastive Learning for Out-of-Distribution Recommendation
...
- **Reason**: This paper focuses on graph recommender systems and out-of-distribution recommendation, which is somewhat related to information retrieval and search technologies. However, the primary emphasis on causal learning and large language models in graph neural networks does not directly align with the user's core research themes in query understanding, ranking models, and user behavior modeling.
...
### 15. Democratic Recommendation with User and Item Representatives Produced by Graph Condensation
...
- **Topic Keywords**: rag, recommend
- **Reason**: The paper discusses graph-based recommendation systems and proposes a framework called DemoRec that leverages graph condensation to generate user and item representatives. While it touches on some aspects of information retrieval, such as reducing graph size and computational complexity, it does not directly address query understanding, ranking models, or user behavior modeling, which are core areas of interest. The paper's focus on recommender systems and graph condensation makes it somewhat related but not a central match for the user's research interests.
...
### 16. Computational frame analysis revisited: On LLMs for studying news coverage
...
- **Reason**: This paper explores the application of LLMs in media frame analysis, which is somewhat related to my interests in Information Retrieval, particularly in areas requiring deep semantic understanding. However, the focus on media frame analysis and computational approaches to content analysis is not a central match to my primary research themes. The paper's relevance to my interests is limited to the use of LLMs, which is a tangential aspect of my research.
...
### 17. Extracting Interaction-Aware Monosemantic Concepts in Recommender Systems
...
- **Reason**: This paper is somewhat related to the user's interests in Information Retrieval and Recommender Systems, but it focuses more on the interpretability and controllability of recommender systems rather than query understanding, ranking models, or user behavior modeling. The paper's emphasis on monosemantic concepts and interaction-aware extraction is not directly aligned with the user's core research themes.
...
### 18. Fidelity-Aware Recommendation Explanations via Stochastic Path Integration
...
- **Reason**: The paper is somewhat related to the user's research interests in Information Retrieval and recommender systems, but it focuses more on explainability in recommendation systems rather than query understanding, ranking models, or user behavior modeling. The use of stochastic path integration for neural recommender explanations is an interesting approach, but it does not directly align with the user's primary focus on deep semantic understanding and real-time relevance optimization in IR.
...
### 19. UFO: Unfair-to-Fair Evolving Mitigates Unfairness in LLM-based Recommender Systems via Self-Play Fine-tuning
...
- **Topic Keywords**: ranking, recommend, rank
- **Reason**: This paper focuses on recommender systems and fairness in LLM-based systems, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the specific topic of recommender systems and fairness is not a central match to your core research themes, and the paper does not appear to address query understanding, ranking models, or user behavior modeling.
...
### 20. Paper2SysArch: Structure-Constrained System Architecture Generation from Scientific Papers
...
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or your core research themes. Although it involves text processing and generation, its focus is on scientific visualization and system architecture diagrams, which is not a central match to your interests.
...
### 21. PoETa v2: Toward More Robust Evaluation of Large Language Models in Portuguese
...
- **Reason**: This paper focuses on evaluating Large Language Models in the Portuguese language, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the specific context and scope are not aligned with the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 22. MindEval: Benchmarking Language Models on Multi-turn Mental Health Support
...
- **Reason**: The paper focuses on evaluating language models in mental health therapy conversations, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the context and application are quite different from your areas of focus.
...
### 23. "AGI" team at SHROOM-CAP: Data-Centric Approach to Multilingual Hallucination Detection using XLM-RoBERTa
...
- **Reason**: This paper focuses on hallucination detection in multilingual scientific text generated by Large Language Models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing, especially in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 24. Bias Is a Subspace, Not a Coordinate: A Geometric Rethinking of Post-hoc Debiasing in Vision-Language Models
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on post-hoc debiasing in Vision-Language Models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve some aspects of semantic understanding, the context is primarily in computer vision and multimodal reasoning, making it less relevant to your primary areas of focus.
...
### 25. General Agentic Memory Via Deep Research
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on memory systems for AI agents, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it mentions large language models, the context is not about query understanding, ranking models, or user behavior modeling, but rather about memory optimization for AI agents.
...
### 26. Findings of the BlackboxNLP 2025 Shared Task: Localizing Circuits and Causal Variables in Language Models
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on mechanistic interpretability of language models, which is a topic related to NLP but not directly related to the user's core research interests in Information Retrieval, Search technologies, and query understanding.
...
### 27. Measuring the Impact of Lexical Training Data Coverage on Hallucination Detection in Large Language Models
...
- **Reason**: This paper focuses on hallucination detection in large language models, which is a topic related to NLP. However, it does not directly align with the user's core research interests in Information Retrieval, Search technologies, and query understanding, ranking models, or user behavior modeling.
...
### 28. Gradient Masters at BLP-2025 Task 1: Advancing Low-Resource NLP for Bengali using Ensemble-Based Adversarial Training for Hate Speech Detection
...
- **Topic Keywords**: rag
- **Reason**: This paper is primarily focused on low-resource NLP for Bengali hate speech detection, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves ensemble-based adversarial training, it is not explicitly related to query understanding, ranking models, or user behavior modeling.
...
### 29. Leveraging Evidence-Guided LLMs to Enhance Trustworthy Depression Diagnosis
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on leveraging large language models for trustworthy depression diagnosis, which is outside the scope of information retrieval and search technologies. While it involves natural language processing, the application is specific to clinical diagnosis and does not align with the user's primary research interests in IR and search technologies.
...
### 30. When Better Teachers Don't Make Better Students: Revisiting Knowledge Distillation for CLIP Models in VQA
...
- **Reason**: This paper focuses on knowledge distillation for CLIP models in VQA, which is a specific application in the NLP domain. While it touches on multimodal tasks, it does not directly relate to information retrieval, query understanding, ranking models, or user behavior modeling, which are core areas of your research interests.
...
### 31. Point of Order: Action-Aware LLM Persona Modeling for Realistic Civic Simulation
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves fine-tuning Large Language Models, the focus is on civic simulation and speaker-attributed data, which is not a central match for the user's interests.
...
### 32. GeeSanBhava: Sentiment Tagged Sinhala Music Video Comment Data Set
...
- **Reason**: This paper appears to be primarily focused on sentiment analysis and emotion recognition in Sinhala music video comments, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it involves Natural Language Processing, it does not seem to address query understanding, ranking models, or user behavior modeling, making it only loosely relevant to the user's interests.
...
### 33. Blu-WERP (Web Extraction and Refinement Pipeline): A Scalable Pipeline for Preprocessing Large Language Model Datasets
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on large language model preprocessing, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on data quality optimization, it does not address query understanding, ranking models, or user behavior modeling, making it somewhat tangential to your research.
...
### 34. L2V-CoT: Cross-Modal Transfer of Chain-of-Thought Reasoning via Latent Intervention
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on cross-modal transfer of Chain-of-Thought reasoning between Vision-Language Models and large language models, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some aspects of deep semantic understanding, the context is primarily in the realm of multimodal reasoning and vision-language models, making it somewhat tangential to your research focus.
...
### 35. HyM-UNet: Synergizing Local Texture and Global Context via Hybrid CNN-Mamba Architecture for Medical Image Segmentation
...
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, Natural Language Processing, data mining, or recommender systems. The paper focuses on medical image segmentation using a hybrid CNN-Mamba architecture, which is outside your primary areas of interest.
...
### 36. Towards Efficient LLM-aware Heterogeneous Graph Learning
...
### 37. A superpersuasive autonomous policy debating system
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be unrelated to Information Retrieval, Search technologies, or other areas of interest, as it focuses on literary scholars' engagement with large language model interpretability research.
