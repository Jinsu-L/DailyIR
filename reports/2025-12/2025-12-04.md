# Daily Papers Report - 2025-12-04

## 🌟 Top 5 Papers with Summaries

선정된 Top 5개 논문에 대한 상세 요약입니다.

### 1. Learning to Comparison-Shop
...
- **Topic Keywords**: ranking, listwise, conversion rate, shopping, commerce, e-commerce, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding, ranking models, and user behavior modeling. The proposed Learning-to-Comparison-Shop (LTCS) System addresses the challenge of aligning search rankings with user comparison shopping behavior, which is a key area of focus for you. The paper's emphasis on deep semantic understanding and real-time relevance optimization also aligns with your interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Learning‑to‑Comparison‑Shop (LTCS): an end‑to‑end ranking architecture that jointly models pointwise relevance and setwise comparison for online marketplaces
- **Aim**: To improve booking conversion and reduce pre‑booking browsing by training a single model that learns both item‑level relevance and contextual comparison signals, thereby aligning ranking with real user shopping workflows
- **Rationale**: Conventional LTR pipelines compute independent pointwise scores even when pairwise or listwise losses are used, breaking the natural coupling between relevance and comparison. Probabilistic factorization shows the re‑ranker’s probability as a product of a pointwise score and a contextual bias, motivating a unified, co‑trained model
- **Ground**: Initial ranker: single MLP (2048→1024→512→256→64) producing booking‑probability logits and embeddings; Set‑wise re‑ranker: 30‑layer Transformer encoder (4‑head self‑attention, no positional embeddings) that processes top‑K (≤40) embeddings, concatenates each with its context embedding, and feeds into an MLP to produce re‑ranking logits. Co‑training loop optimises both stages with weighted loss λ=(1‑α)λ_initial+αλ_re‑rank, α=0.5. Trained on ~360M booking‑labelled examples on 10 NVIDIA A10G GPUs (~20 h). Inference is two‑stage: leaf nodes score all candidates; master node re‑ranks top‑K embeddings, keeping latency O(K²) within production limits
- **Experiment**: Offline NDCG: LTCS 0.6982 (std 0.0003) outperforms attn‑DIN (0.6856), attn‑DIN+ (0.6918), MO‑LTR (0.6860). Online metrics: +0.6 % CVR lift, –0.88 % pre‑booking detail‑page views (p = 0.001). Hyperparameter sensitivity: K=40 optimal, Transformer depth 30 layers, α=0.5 yields peak NDCG and stable training
- **Takeaway**: LTCS demonstrates that a single, end‑to‑end learning‑to‑rank model that jointly learns pointwise relevance and setwise comparison delivers measurable business gains (+0.6 % CVR, higher NDCG) while remaining production‑ready. The architecture leverages permutation‑invariant modeling, click‑bias mitigation, and personalized re‑ranking insights, establishing a new baseline for comparison‑driven ranking in online marketplaces
...
### 2. AR-Med: Automated Relevance Enhancement in Medical Search via LLM-Driven Information Augmentation
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in areas requiring deep semantic understanding and real-time relevance optimization. The use of Large Language Models (LLMs) for relevance enhancement in medical search aligns with your focus on query understanding and ranking models. The paper's emphasis on accuracy, reliability, and scalability also resonates with your background in e-commerce and NLP.
...
### 3. Idea-Gated Transformers: Enforcing Semantic Coherence via Differentiable Vocabulary Pruning
...
- **Reason**: This paper introduces a novel architecture for language modeling that addresses the issue of 'Topic Drift' by separating semantic planning from syntactic generation. Although it's primarily focused on language modeling, the concept of differentiable vocabulary pruning and semantic coherence is relevant to information retrieval, particularly in areas that require deep semantic understanding. The paper's emphasis on real-time relevance optimization and controllable language modeling aligns with the user's interests in search technologies and ranking models.
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, specifically in the context of Retrieval-Augmented Generation and hierarchical structure-aware indexing. However, it focuses on a specific application (question answering on complex documents) and does not directly address your core areas of interest in query understanding, ranking models, and user behavior modeling.
...
### 5. M3DR: Towards Universal Multilingual Multimodal Document Retrieval
...
- **Reason**: The paper M3DR: Towards Universal Multilingual Multimodal Document Retrieval is somewhat related to the user's research interests in Information Retrieval, particularly in the area of multimodal document retrieval. However, the focus on multilingual and multimodal aspects, while interesting, does not directly align with the user's primary focus on query understanding, ranking models, and user behavior modeling. The connection to NLP and data mining is also present, but the paper's emphasis on real-world multilingual scenarios and contrastive training is not a central match for the user's research themes.
...
- **Reason**: The paper explores the application of large language models in recommendation systems, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the focus on explainability and recommendation systems, rather than ranking models or user behavior modeling, limits its relevance. The use of LLMs also extends into NLP, which is a related topic, but not a central match for your research interests.
...
### 7. Nexus: Higher-Order Attention Mechanisms in Transformers
...
- **Topic Keywords**: query, queries, rank
- **Reason**: This paper proposes a novel architecture, Nexus, which enhances representational power in Transformers through recursive self-attention mechanisms. While it contributes to the advancement of deep learning models, it does not directly address query understanding, ranking models, or user behavior modeling in the context of information retrieval. The focus on Transformers and attention mechanisms is somewhat related to the user's interests in NLP and deep semantic understanding, but it is not a central match.
...
### 8. A Preliminary Study on the Promises and Challenges of Native Top-$k$ Sparse Attention
...
- **Reason**: The paper explores the Top-$k$ Attention mechanism, which is related to ranking models and query understanding in Information Retrieval. However, the focus is on the computational efficiency of Large Language Models, which is not a central theme in the user's research interests. The paper's relevance is somewhat tangential to the user's core research areas.
...
### 9. BERnaT: Basque Encoders for Representing Natural Textual Diversity
...
- **Topic Keywords**: ctr
- **Reason**: The paper explores linguistic diversity in language models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, it primarily focuses on Natural Language Understanding (NLU) tasks and language model robustness, which, while relevant to broader NLP topics, does not directly align with the user's core research interests in IR and search technologies.
...
### 10. Improving Alignment Between Human and Machine Codes: An Empirical Assessment of Prompt Engineering for Construct Identification in Psychology
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and query understanding, as it explores the use of large language models for text classification. However, the focus on prompt engineering for construct identification in psychology is not directly aligned with your primary research themes in Information Retrieval and Search technologies.
...
### 11. CartoMapQA: A Fundamental Benchmark Dataset Evaluating Vision-Language Models on Cartographic Map Understanding
...
- **Topic Keywords**: search, kdd
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, particularly in the context of question-answering tasks and visual-language models. However, the focus on cartographic map understanding and geospatial reasoning is not directly aligned with your primary interests in query understanding, ranking models, and user behavior modeling. While the paper touches on real-time relevance optimization, it is not a central theme.
...
- **Topic Keywords**: search
- **Reason**: The paper AlignCheck focuses on evaluating factual consistency in text, which is related to query understanding and ranking models in Information Retrieval. However, it primarily deals with Natural Language Processing (NLP) and does not directly address search technologies, user behavior modeling, or real-time relevance optimization, which are core areas of interest.
...
### 13. LLM-Generated Ads: From Personalization Parity to Persuasion Superiority
...
- **Reason**: This paper explores the effectiveness of large language models (LLMs) in generating persuasive content for advertising, which is somewhat related to information retrieval and search technologies. However, the focus on advertising and persuasion principles, while tangentially related to user behavior modeling, is not a central match to the user's core research themes in IR and NLP.
...
### 14. PERCS: Persona-Guided Controllable Biomedical Summarization Dataset
...
- **Topic Keywords**: search
- **Reason**: The paper is somewhat related to information retrieval, but its focus on biomedical summarization and persona-guided summarization is not directly aligned with the user's core research themes. While it involves natural language processing and summarization, it lacks a strong connection to query understanding, ranking models, and user behavior modeling.
...
### 15. Thinking with Programming Vision: Towards a Unified View for Thinking with Images
...
- **Reason**: This paper is not directly related to Information Retrieval or Search technologies, which are the primary focus of your research interests. While it involves multimodal large language models and robust tool-based reasoning, the context is more aligned with computer vision and programming, rather than query understanding, ranking models, or user behavior modeling.
...
### 16. Modeling Topics and Sociolinguistic Variation in Code-Switched Discourse: Insights from Spanish-English and Spanish-Guaraní
...
- **Topic Keywords**: rag, search
- **Reason**: This paper is not relevant to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves large language models, the focus is on sociolinguistic analysis and bilingual discourse, which is outside your primary areas of interest.
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on cognitive skills in reasoning models, leveraging reinforcement learning and self-distillation. While it touches on model fine-tuning and learning, it doesn't directly relate to information retrieval, search technologies, or query understanding, which are core areas of your research interests.
...
### 18. Training and Evaluation of Guideline-Based Medical Reasoning in LLMs
...
- **Topic Keywords**: ctr
- **Reason**: This paper focuses on teaching Large Language Models (LLMs) to follow medical consensus guidelines, which is outside the scope of Information Retrieval and Search technologies. While it involves NLP, the application is specific to medicine and does not align with the user's core research themes.
...
### 19. Algorithms for Boolean Matrix Factorization using Integer Programming and Heuristics
...
- **Reason**: This paper focuses on Boolean matrix factorization, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on scalability and optimization, the context is different from the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 20. AITutor-EvalKit: Exploring the Capabilities of AI Tutors
...
- **Topic Keywords**: acl
- **Reason**: This paper appears to be unrelated to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. Although it involves language technology, its focus on AI tutors and pedagogical quality evaluation does not align with the user's areas of interest.
...
### 21. Fine-grained Narrative Classification in Biased News Articles
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text analysis and classification, its focus on propaganda detection and narrative classification in biased news articles is not aligned with the user's interests in query understanding, ranking models, or user behavior modeling.
...
### 22. NAS-LoRA: Empowering Parameter-Efficient Fine-Tuning for Visual Foundation Models with Searchable Adaptation
...
- **Topic Keywords**: rank, search
- **Reason**: This paper appears to be primarily focused on visual foundation models and neural architecture search, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it does involve some form of adaptation and optimization, the context and techniques used are more specific to computer vision and not directly applicable to the user's areas of interest.
...
### 23. Tuning for TraceTarnish: Techniques, Trends, and Testing Tangible Traits
...
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests as it focuses on adversarial stylometry and text anonymization, which is outside your core areas of Information Retrieval, Search technologies, and Natural Language Processing. The paper's emphasis on stylometric features and forensic analysis does not align with your interests in query understanding, ranking models, and user behavior modeling.
...
### 24. PretrainZero: Reinforcement Active Pretraining
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to Information Retrieval or Search technologies, but rather focuses on Reinforcement Learning and Artificial General Intelligence. While it mentions pretraining and reasoning, the context is quite different from the user's core research themes.
...
### 25. SPARK: Stepwise Process-Aware Rewards for Reference-Free Reinforcement Learning
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on reinforcement learning and process reward models, which is somewhat related to information retrieval, but the topic is not central to your research interests. The paper's emphasis on reference-free reinforcement learning and its application to mathematical reasoning does not align with your primary focus on information retrieval, especially in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 26. Identifying attributions of causality in political text
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on political text analysis and causal explanation detection, which is unrelated to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. Although it involves NLP, the specific application and goals are not aligned with the user's areas of focus.
...
### 27. InvertiTune: High-Quality Data Synthesis for Cost-Effective Single-Shot Text-to-Knowledge Graph Generation
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your core research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text generation and knowledge graph construction, the focus is on data synthesis and model fine-tuning, which is not a central match to your research themes.
...
### 28. Evaluating Hydro-Science and Engineering Knowledge of Large Language Models
...
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests as it focuses on evaluating large language models in the Hydro-Science and Engineering domain, which is outside your areas of expertise in Information Retrieval, Search technologies, and Natural Language Processing.
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on Large Language Model (LLM) fingerprinting, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it touches on neural networks, the context is more aligned with model protection and intellectual property rather than query understanding, ranking models, or user behavior modeling.
...
### 30. Dual LoRA: Enhancing LoRA with Magnitude and Direction Updates
...
- **Topic Keywords**: rank
- **Reason**: This paper focuses on parameter-efficient fine-tuning of pre-trained language models, which is a topic in NLP, but it doesn't directly relate to information retrieval, query understanding, or ranking models, which are the core areas of your research interests.
...
### 31. Enhancing Job Matching: Occupation, Skill and Qualification Linking with the ESCO and EQF taxonomies
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on job matching and labor market information classification, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves language models and entity linking, the context and application are quite different from the user's areas of interest.
