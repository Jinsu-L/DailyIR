...
def _crawl_papers(config: dict, target_date: datetime.date, days_to_fetch: int, window_based: bool) -> list:
    ...
    if config.get("arxiv_crawler", {}).get("enabled", False):
        ...
        try:
            ...
            all_new_papers.extend(papers)
            logging.info(
                f"Fetched {len(papers)} papers from Arxiv for the last {days_to_fetch} day(s) ending on {target_date}.")
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DailyPapers: A CLI tool to fetch, score, and report on research papers.")
    parser.add_argument(
        "--date",
        type=str,
        default=None,
