...
### 1. AI Co-Scientist for Ranking: Discovering Novel Search Ranking Models alongside LLM-based AI Agents with Cloud Computing Access

- **LLM Score**: 8
- **Keyword Score**: 7
- **Authors**: Liwei Wu, Cho-Jui Hsieh
- **URL**: <http://arxiv.org/abs/2603.22376v1>
- **Submitted**: 2026-03-23 09:29:08
- **Comment**: Submitted to ACL for review on January 4, 2026
- **Topic Keywords**: ranking, rag, rank, search
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in ranking models and query understanding. The use of AI Co-Scientist framework to automate the search ranking research pipeline aligns with your focus on real-time relevance optimization and deep semantic understanding. However, the paper's primary focus on AI agents and software engineering might not be a central match to your interests.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: AI Co-Scientist for Large-Scale Commercial Search Ranking
- **Aim**: Develop an end-to-end automated research pipeline that discovers, implements, and evaluates novel ranking models for commercial search systems while keeping a human expert in the loop.
- **Rationale**: LLM-driven automation can produce architectures that match or surpass human-crafted ones, yielding measurable offline gains and dramatically reducing routine research effort in the highly competitive ranking domain.
- **Ground**: Prior AI‑Scientist systems (Lu et al., 2024; Gottweis et al., 2025) and ranking research (DCN, stacked sequences) provide the foundation. The paper addresses the lack of an automated pipeline for end-to-end ranking model discovery, training, and evaluation, and builds on baselines V1 (mean‑pooling + DCN), V2 (human‑engineered stacked sequences), and V3 (AI‑derived incremental innovations).
- **Experiment**: The pipeline iteratively generates hypotheses via multi‑LLM consensus, implements code, schedules GPU training, and analyzes results using a composite metric M (mean of six AUCs). The evolution from V3.0 to V3.5 shows that reducing learning rate, adding slot‑type and temporal embeddings, and employing a four‑phase adaptive LR schedule yields the best performance: V3.5 outperforms V2 by +0.083% and V1 by +0.201% in M. Offline gains translate to significant conversion lift.
- **Takeaway**: An AI Co‑Scientist can autonomously discover novel ranking techniques, achieve measurable offline improvements, and reduce routine research effort, marking the first application of such a framework in commercial search ranking.
...
### 2. Semantic Shift: the Fundamental Challenge in Text Embedding and Retrieval
...
- **Reason**: This paper is highly relevant to Information Retrieval, specifically addressing a fundamental challenge in text embedding and retrieval. The authors propose a novel concept of 'semantic shift' that can predict retrieval degradation, which aligns with the user's interest in query understanding and ranking models. While not directly focused on user behavior modeling or e-commerce, the paper's contribution to text embedding and retrieval makes it a valuable resource for the field.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: Transformer-based embedding models suffer from anisotropy and embedding collapse due to semantic shift in variable-length texts
- **Aim**: Identify the causal factor behind embedding concentration and its impact on retrieval performance, and provide a computable diagnostic metric
- **Rationale**: Prior work only described symptoms; semantic shift explains how increasing semantic diversity forces pooled vectors away from individual sentence embeddings, reducing discriminativeness
- **Ground**: Theoretical analysis shows that higher semantic diversity leads to smoother, less discriminative pooled representations; semantic shift is defined as a metric combining local semantic evolution and global dispersion
- **Experiment**: Controlled experiments across multiple corpora and embedding models demonstrate a strong correlation between semantic shift and embedding concentration, and that semantic shift predicts retrieval performance loss better than raw text length
- **Takeaway**: Semantic shift offers a unified, actionable framework for diagnosing when anisotropy and embedding collapse become detrimental to retrieval tasks
...
### 3. AgenticRec: End-to-End Tool-Integrated Policy Optimization for Ranking-Oriented Recommender Agents
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the area of ranking models and user behavior modeling. Although it focuses on recommender systems, the use of ranking-oriented approaches and optimization techniques aligns with your interests. However, the specific domain of recommender systems and the use of Large Language Models are not central to your primary focus.
...
- **Reason**: The paper discusses multi-objective re-ranking in recommender systems, which is somewhat related to the user's interests in Information Retrieval and Search technologies. However, the focus on recommender systems and multi-objective optimization is not the user's primary area of interest, and the paper does not explicitly address query understanding, ranking models, or user behavior modeling.
...
### 5. On the Challenges and Opportunities of Learned Sparse Retrieval for Code
...
- **Reason**: This paper explores learned sparse retrieval for code, which is somewhat related to information retrieval and search technologies. However, the focus on code retrieval and software engineering systems is not a central match to the user's core research themes, which are primarily in query understanding, ranking models, and user behavior modeling.
...
### 6. GoogleTrendArchive: A Year-Long Archive of Real-Time Web Search Trends Worldwide
...
- **Reason**: The paper is somewhat related to your research interests in Information Retrieval, particularly in the context of real-time search trends and query understanding. However, its focus on Google Trending Now data and collective attention dynamics, while interesting, does not directly align with your primary areas of interest in query understanding, ranking models, and user behavior modeling.
...
### 7. TagLLM: A Fine-Grained Tag Generation Approach for Note Recommendation
...
- **Reason**: The paper focuses on tag generation for note recommendation using Large Language Models, which is somewhat related to information retrieval and search technologies. However, the specific application domain and task are not directly aligned with the user's core research themes, such as query understanding, ranking models, and user behavior modeling.
...
### 8. Dual-Space Knowledge Distillation with Key-Query Matching for Large Language Models with Vocabulary Mismatch
...
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and Large Language Models, but it primarily focuses on knowledge distillation and model efficiency, which is not a central match for the user's core research themes in Information Retrieval and Search technologies.
...
### 9. Toward a Theory of Hierarchical Memory for Language Agents
...
- **Reason**: The paper explores hierarchical memory for language agents, which is somewhat related to information retrieval, particularly in the context of query understanding and ranking models. However, the focus on language agents and hierarchical memory is not directly aligned with the user's primary interests in e-commerce, deep semantic understanding, and real-time relevance optimization. The connection to NLP is more relevant, but still not a central match.
...
- **Reason**: The paper explores a recommender system, which is related to your research interests in Information Retrieval and Search technologies. However, it focuses on commercial retrieval and monetization, which is somewhat tangential to your primary interests in query understanding, ranking models, and user behavior modeling. The use of generative models and control tokens is an interesting aspect, but it doesn't directly align with your core research themes.
...
- **Reason**: This paper is somewhat related to Information Retrieval, as it involves re-framing narrative detection as a retrieval task and proposing a framework for ranking texts by alignment with a narrative. However, the focus on climate disinformation and narrative detection is not directly aligned with the user's core research themes in query understanding, ranking models, and user behavior modeling. The use of graph-based methods for community detection and narrative structure is an interesting aspect, but it is not a central match for the user's research interests.
...
### 12. Generalizable Self-Evolving Memory for Automatic Prompt Optimization
...
- **Reason**: The paper discusses automatic prompt optimization for large language models, which is somewhat related to information retrieval and search technologies. However, the focus on prompt optimization and language models is not a central match to the user's primary research interests in query understanding, ranking models, and user behavior modeling. The connection to NLP is relevant, but the paper's scope is more narrow than the user's broader interests in NLP and data mining.
...
### 13. From Static Templates to Dynamic Runtime Graphs: A Survey of Workflow Optimization for LLM Agents

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Ling Yue, Kushal Raj Bhandari, Ching-Yun Ko, Dhaval Patel, Shuxin Lin, Nianjun Zhou, Jianxi Gao, Pin-Yu Chen, Shaowu Pan
- **URL**: <http://arxiv.org/abs/2603.22386v1>
- **Submitted**: 2026-03-23 17:42:59
- **Topic Keywords**: information retrieval, retrieval
- **Reason**: This paper is somewhat related to the user's interests in Information Retrieval and Search technologies, but it focuses on workflow optimization for Large Language Model (LLM) agents, which is not a central match. The paper does touch on information retrieval, but it's not a primary focus. The user's background in e-commerce and experience with query understanding, ranking models, and user behavior modeling are not directly relevant to this paper.
...
### 14. PROMPT2BOX: Uncovering Entailment Structure among LLM Prompts
...
- **Reason**: This paper explores the limitations of vector embeddings in capturing prompt specificity and proposes a novel approach called PROMPT2BOX. While it touches on the topic of query understanding and ranking models, its primary focus is on Natural Language Processing (NLP) and the analysis of Large Language Model (LLM) weaknesses. The connection to information retrieval is indirect, making it somewhat relevant but not a central match for your research interests.
...
- **Reason**: This paper is somewhat related to the user's interests in Information Retrieval and Natural Language Processing, as it involves evaluating dialectal bias in Large Language Models. However, the focus on Bengali dialects and translation quality evaluation is not directly aligned with the user's core research themes, which are more focused on query understanding, ranking models, and user behavior modeling.
...
### 16. Silicon Bureaucracy and AI Test-Oriented Education: Contamination Sensitivity and Score Confidence in LLM Benchmarks
...
- **Reason**: The paper explores the limitations of large language model benchmarks, proposing an audit framework to analyze contamination sensitivity and score confidence. While it touches on AI and NLP, it doesn't directly relate to information retrieval, query understanding, or ranking models, which are core areas of your research interests.
...
- **URL**: <http://arxiv.org/abs/2603.22213v1>
- **Submitted**: 2026-03-23 17:11:43
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and data augmentation, but it does not directly address your core focus on Information Retrieval (IR), query understanding, or ranking models. The paper's emphasis on knowledge injection and large language models is tangentially relevant to your interests, but it does not appear to be a central match.
...
### 18. KG-Hopper: Empowering Compact Open LLMs with Knowledge Graph Reasoning via Reinforcement Learning

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Shuai Wang, Yinan Yu
- **URL**: <http://arxiv.org/abs/2603.21440v1>
- **Submitted**: 2026-03-22 23:07:29
- **Comment**: Accepted to IJCNN 2026
- **Topic Keywords**: rag
- **Reason**: The paper presents a novel framework for knowledge graph reasoning using reinforcement learning, which is somewhat related to the user's interests in Information Retrieval and Natural Language Processing. However, the focus on large language models and knowledge graph reasoning is not directly aligned with the user's primary focus on query understanding, ranking models, and user behavior modeling. The paper's relevance is somewhat enhanced by its use of reinforcement learning, which is a related topic in the broader context of search technologies.
...
### 19. Multi-Perspective LLM Annotations for Valid Analyses in Subjective Tasks

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Navya Mehrotra, Adam Visokay, Kristina Gligorić
- **URL**: <http://arxiv.org/abs/2603.21404v1>
- **Submitted**: 2026-03-22 21:18:05
- **Topic Keywords**: rag
- **Reason**: This paper focuses on improving the annotation of large language models, which is related to Natural Language Processing (NLP) and query understanding. However, it does not directly address information retrieval, ranking models, or user behavior modeling, which are core areas of your research interests.
...
- **Topic Keywords**: rank
- **Reason**: This paper explores parameter-efficient fine-tuning methods for medical text summarization, which is a specific application of Natural Language Processing (NLP). While it touches on aspects of model adaptation and fine-tuning, it is not directly related to the user's core research themes in Information Retrieval (IR), query understanding, ranking models, or user behavior modeling. The paper's focus on medical text summarization and low-rank adaptation is somewhat relevant to the user's broader interests in NLP and data mining, but it is not a central match.
...
### 21. PLR: Plackett-Luce for Reordering In-Context Learning Examples

- **LLM Score**: 4
- **Keyword Score**: 1
- **Authors**: Pawel Batorski, Paul Swoboda
- **URL**: <http://arxiv.org/abs/2603.21373v1>
- **Submitted**: 2026-03-22 19:15:59
- **Topic Keywords**: search
- **Reason**: The paper discusses in-context learning and ordering methods for adapting large language models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on language models and few-shot accuracy in classification tasks is not directly aligned with the user's primary research interests in IR and search technologies. The connection to ranking models is indirect, but the paper's emphasis on probabilistic approaches and efficient ordering methods may be of interest to the user.
...
### 22. When Documents Disagree: Measuring Institutional Variation in Transplant Guidance with Retrieval-Augmented Language Models
...
- **Reason**: This paper appears to be primarily focused on a specific domain (transplant guidance) and uses retrieval-augmented language models, but it does not align with the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. The paper's focus on a particular domain and lack of emphasis on query understanding, ranking models, or user behavior modeling make it only loosely relevant to the user's interests.
...
### 23. ADaFuSE: Adaptive Diffusion-generated Image and Text Fusion for Interactive Text-to-Image Retrieval
...
- **Reason**: This paper is primarily focused on image and text fusion for interactive text-to-image retrieval, which is not directly related to your core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve some aspects of query understanding and ranking models, the context is more aligned with computer vision and multimedia retrieval, making it somewhat tangential to your research interests.
...
### 24. SLURP-TN : Resource for Tunisian Dialect Spoken Language Understanding
...
- **Reason**: This paper is not relevant to your research interests as it focuses on Spoken Language Understanding (SLU) and Automatic Speech Recognition, which are not core areas of your research. Although it involves Natural Language Processing (NLP), the specific domain and application do not align with your interests in Information Retrieval, query understanding, and ranking models.
...
### 25. Reasoner-Executor-Synthesizer: Scalable Agentic Architecture with Static O(1) Context Window

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Ivan Dobrovolskyi
- **URL**: <http://arxiv.org/abs/2603.22367v1>
- **Submitted**: 2026-03-23 05:16:55
- **Topic Keywords**: rag, retrieval, search
- **Reason**: This paper focuses on Large Language Models (LLMs) and proposes a new architecture to address issues with context length and token cost. However, the paper's primary concerns and contributions are not directly related to Information Retrieval, Search technologies, or query understanding, which are core areas of your research interests.
...
### 26. Overview of TREC 2025 Biomedical Generative Retrieval (BioGen) Track
...
- **Reason**: This paper appears to be focused on the evaluation of large language models in the biomedical domain, specifically addressing hallucinations and confabulations. While it touches on the topic of information retrieval, it does not seem to be directly related to the user's core research themes in query understanding, ranking models, and user behavior modeling. The paper's focus on NLP and biomedical tasks is somewhat tangential to the user's interests.
...
### 27. CatRAG: Functor-Guided Structural Debiasing with Retrieval Augmentation for Fair LLMs
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on debiasing Large Language Models, which is outside your primary research interests in Information Retrieval and Search technologies. While it involves Retrieval-Augmented Generation (RAG), the context is different from your typical work in query understanding, ranking models, and user behavior modeling.
...
### 28. Ara-Best-RQ: Multi Dialectal Arabic SSL
...
- **Reason**: This paper focuses on multi-dialectal Arabic speech processing using self-supervised learning models, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 29. Politics of Questions in News: A Mixed-Methods Study of Interrogative Stances as Markers of Voice and Power
...
- **Reason**: This paper appears to be a linguistics study focused on the analysis of interrogative stances in news discourse, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on aspects of text analysis, its primary focus is on linguistic and sociological perspectives, making it only loosely relevant to the user's interests.
...
### 30. EvoIdeator: Evolving Scientific Ideas through Checklist-Grounded Reinforcement Learning
...
- **Topic Keywords**: rag, search
- **Reason**: This paper is not directly related to your core research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves Reinforcement Learning and Large Language Models, the focus is on scientific idea generation and autonomous knowledge discovery, which is not a central theme in your research.
...
### 31. TaigiSpeech: A Low-Resource Real-World Speech Intent Dataset and Preliminary Results with Scalable Data Mining In-the-Wild
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on speech intent detection in a low-resource language, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves data mining and NLP, the context is specific to speech technologies and does not align with the user's primary focus on deep semantic understanding and real-time relevance optimization in IR.
...
### 32. MemDLM: Memory-Enhanced DLM Training
...
- **Submitted**: 2026-03-23 17:39:56
- **Topic Keywords**: retrieval
- **Reason**: This paper focuses on improving the performance of Diffusion Language Models, which is a topic in Natural Language Processing. However, it does not appear to be directly related to Information Retrieval, query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
...
### 33. Dyadic: A Scalable Platform for Human-Human and Human-AI Conversation Research

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: David M. Markowitz
- **URL**: <http://arxiv.org/abs/2603.22227v1>
- **Submitted**: 2026-03-23 17:25:45
- **Topic Keywords**: search, www
- **Reason**: This paper is not directly related to Information Retrieval or Search technologies, but rather focuses on conversation research and human-AI interaction. While it involves text-based interactions, the primary focus is on studying conversations rather than developing IR or NLP models.
...
### 34. Gumbel Distillation for Parallel Text Generation

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Chi Zhang, Xixi Hu, Bo Liu, Qiang Liu
- **URL**: <http://arxiv.org/abs/2603.22216v1>
- **Submitted**: 2026-03-23 17:13:22
- **Comment**: ICLR 2026
- **Topic Keywords**: rag
- **Reason**: This paper focuses on parallel text generation and introduces a novel distillation technique, Gumbel Distillation, to improve generation quality. However, it does not align with the user's primary research interests in Information Retrieval, Search technologies, and related topics. The paper's relevance to the user's interests in NLP and data mining is also limited.
...
### 35. Enhancing Document-Level Machine Translation via Filtered Synthetic Corpora and Two-Stage LLM Adaptation

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Ireh Kim, Tesia Sker, Chanwoo Kim
- **URL**: <http://arxiv.org/abs/2603.22186v1>
- **Submitted**: 2026-03-23 16:48:22
- **Comment**: Accepted to ICASSP 2026
- **Topic Keywords**: rag
- **Reason**: This paper focuses on Machine Translation and Large Language Models, which is outside your primary research area of Information Retrieval and Search technologies. Although it involves NLP, the specific topic of document-level machine translation does not align with your interests in query understanding, ranking models, and user behavior modeling.
...
- **URL**: <http://arxiv.org/abs/2603.21875v1>
- **Submitted**: 2026-03-23 12:05:57
...
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus is on speech deepfake source verification, which is outside your primary areas of interest.
...
### 37. Probing How Scalable Table Data Enhances General Long-Context Reasoning
...
- **URL**: <http://arxiv.org/abs/2603.21719v1>
- **Submitted**: 2026-03-23 09:05:46
- **Topic Keywords**: rag
- **Reason**: This paper focuses on long-context reasoning in Large Language Models, leveraging structured table data to enhance performance. While it touches on data analysis and modeling, it does not directly relate to information retrieval, search technologies, or user behavior modeling, which are core areas of your research interests.
...
### 38. Thinking Deeper, Not Longer: Depth-Recurrent Transformers for Compositional Generalization

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Hung-Hsuan Chen
- **URL**: <http://arxiv.org/abs/2603.21676v1>
- **Submitted**: 2026-03-23 08:06:45
- **Topic Keywords**: rag
- **Reason**: This paper focuses on developing a depth-recurrent Transformer for compositional generalization, which is not directly related to information retrieval, search technologies, or user behavior modeling. While it involves deep semantic understanding, the context is more aligned with NLP and deep learning architectures, rather than real-time relevance optimization or query understanding.
...
### 39. TAMTRL: Teacher-Aligned Reward Reshaping for Multi-Turn Reinforcement Learning in Long-Context Compression

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Li Wang, Yandong Wang, Xin Yu, Kui Zhang, Tianhao Peng, Wenjun Wu
- **URL**: <http://arxiv.org/abs/2603.21663v1>
- **Submitted**: 2026-03-23 07:42:04
- **Topic Keywords**: rag
- **Reason**: This paper focuses on multi-turn reinforcement learning in long-context compression, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves large language models and processing long documents, the primary goal is to improve long-context processing, which is not a central theme in your research.
...
### 40. SynSym: A Synthetic Data Generation Framework for Psychiatric Symptom Identification

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Migyeong Kang, Jihyun Kim, Hyolim Jeon, Sunwoo Hwang, Jihyun An, Yonghoon Kim, Haewoon Kwak, Jisun An, Jinyoung Han
- **URL**: <http://arxiv.org/abs/2603.21529v1>
- **Submitted**: 2026-03-23 03:41:41
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests as it focuses on psychiatric symptom identification on social media, which is outside your primary area of interest in Information Retrieval and Search technologies. Although it involves Natural Language Processing, the context and application are not aligned with your core themes.
...
### 41. Triangulating Temporal Dynamics in Multilingual Swiss Online News

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Bros Victor, Dufraisse Evan, Popescu Adrian, Gatica-Perez Daniel
- **URL**: <http://arxiv.org/abs/2603.21519v1>
- **Submitted**: 2026-03-23 03:27:00
- **Comment**: ICWSM 2026
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The focus is on media studies, linguistic diversity, and cultural contexts, which is outside your primary areas of interest.
...
### 42. Riding Brainwaves in LLM Space: Understanding Activation Patterns Using Individual Neural Signatures

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Ajan Subramanian, Sumukh Bettadapura, Rohan Sathish
- **URL**: <http://arxiv.org/abs/2603.21847v1>
- **Submitted**: 2026-03-23 11:36:00
- **Topic Keywords**: personalization
- **Reason**: This paper explores the intersection of neural signals and language models, but it does not appear to be directly related to information retrieval, search technologies, or user behavior modeling. While it touches on deep semantic understanding, the focus is on individual neural responses rather than query understanding or ranking models.
...
- **Reason**: This paper appears to be focused on photonic accelerators and their application in long-context language model inference, which is unrelated to your primary research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 44. Instruction Set and Language for Symbolic Regression
...
- **Reason**: This paper appears to be focused on symbolic regression and representation frameworks, which is unrelated to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 45. DRTriton: Large-Scale Synthetic Data Reinforcement Learning for Triton Kernel Generation
...
- **Topic Keywords**: rag, search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or Natural Language Processing. The paper focuses on developing efficient CUDA kernels for generative AI using Large Language Models, which is outside your primary areas of interest.
...
### 46. BHDD: A Burmese Handwritten Digit Dataset

- **LLM Score**: 0
- **Keyword Score**: 1
- **Authors**: Swan Htet Aung, Hein Htet, Htoo Say Wah Khaing, Thuya Myo Nyunt
- **URL**: <http://arxiv.org/abs/2603.21966v1>
- **Submitted**: 2026-03-23 13:31:57
- **Comment**: 4 pages, 9 figures, 1 table. Dataset available at https://github.com/baseresearch/BHDD
- **Topic Keywords**: search
- **Reason**: This paper is about a handwritten digit dataset for Burmese digits, which is not related to Information Retrieval, Search technologies, or any of the user's core research themes.
