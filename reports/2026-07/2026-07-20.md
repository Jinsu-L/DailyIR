...
- **Reason**: This paper presents a listwise reranker, jina-reranker-v3.5, which aligns with your research interests in Information Retrieval, particularly in query understanding and ranking models. The paper's focus on efficiency, domain robustness, and real-time relevance optimization also resonates with your interests. The use of hybrid attention and self-distillation is a relevant technique in NLP and IR.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: jina‑reranker‑v3.5: a 0.6 B sparse‑attention list‑wise reranker
- **Aim**: Deliver enterprise‑grade reranking that closes the specialization gap with large LLM rerankers while achieving significant inference‑speed gains on NVIDIA A100 GPUs
- **Rationale**: Leverages a 3‑layer‑2‑global (3L2G) sparse‑attention schedule on a Qwen3‑0.6 B backbone, a three‑stage self‑distillation pipeline aligning a lightweight student with a full‑attention teacher, and a hybrid local‑global attention schedule to reduce per‑layer cost from (O(L^2)) to (O(L·w)). The model is trained on diverse domain corpora (legal, finance, medical, structured tables, multilingual web data) with synthetic hard‑negative generation and multi‑shard sampling to ensure robustness across structured and unstructured queries.
- **Ground**: Architecture: 28 transformer layers (17 local + 11 global), 3L2G schedule, 16 heads (8 KV + GQA), 1024‑token sliding window, 131 k max context, 1024→512→512 projector, cosine scoring. Data: structured‑data shard (high sampling weight), multilingual shard, synthetic hard negatives via typed constraints and paraphrasing. Distillation: Stage I teacher fine‑tune (full attention), Stage II student adaptation (attention‑only warm‑up + full‑parameter fine‑tune), Stage III teacher‑guided distillation with multi‑level loss (rank, score, state, embed, contrastive similarity, token dispersion). Training: FlashAttention‑2, bf16, listwise batches up to 50 docs, 30–45 hard negatives, random ID perturbations, teacher caching, domain‑dependent steps.
- **Experiment**: Benchmarks: BEIR (13 English datasets), MIRACL (18 languages), RTEB (legal, finance, programming, medicine), Struct‑IR (structured‑data shard), STARK (semi‑structured KG retrieval). Retrieval: top‑100 candidates via jina‑embeddings‑v5‑text‑small, listwise reranking with FlashAttention‑2. Baselines: jina‑reranker‑v3, Qwen3‑Reranker‑0.6 B, Qwen3‑Reranker‑4 B, mxbai‑rerank‑base‑v2, mxbai‑rerank‑large‑v2, Mixedbread AI, Zhang et al. (2025b). Metrics: nDCG@10 (BEIR, MIRACL, RTEB, Struct‑IR), Hit@1/5/MRR (STARK). Results: Student matches/exceeds teacher on all benchmarks, rivals 4 B‑parameter baselines, achieves inference‑speed gains on A100.
- **Takeaway**: A 0.6 B sparse‑attention reranker can match or surpass larger LLM rerankers while delivering faster inference and handling up to 128 k tokens. The 3L2G schedule, hybrid attention, and self‑distillation pipeline enable efficient enterprise‑style ranking across diverse domains, closing the specialization gap with minimal computational cost.
...
- **URL**: <http://arxiv.org/abs/2607.17092v1>
- **Submitted**: 2026-07-19 06:14:21
- **Topic Keywords**: ranking, pairwise, rag, user behavior, click, recommend, rank
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of ranking models and user behavior modeling. The proposed framework, UAME, addresses uncertainty and label bias in short video recommendation, which aligns with your focus on deep semantic understanding and real-time relevance optimization. While the specific domain is e-commerce, the techniques and concepts explored are transferable to other areas of IR.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Uncertainty‑Aware Multi‑Objective Ensemble Ranking (UAME) for short‑video recommendation
- **Aim**: Embed predictive uncertainty into the ranking objective to mitigate proxy‑label bias and align learned rankings with true user satisfaction.
- **Rationale**: Proxy satisfaction signals are noisy, fragmented, and contradictory; deterministic models over‑fit to consistent but biased signals. Modeling aleatoric uncertainty derived from proxy conflicts allows the system to down‑weight unreliable samples and focus learning on informative pairs, thereby reducing satisfaction‑label bias.
- **Ground**: UAME uses a multi‑task backbone with separate pxtr heads, a dual‑branch satisfaction head producing a Gaussian (hat{s}_isimmathcal{N}(mu_i,sigma_i^2)), a probabilistic pairwise ranking loss (P_{ij}=Phi((mu_i-mu_j)/sqrt{sigma_i^2+sigma_j^2})), uncertainty‑aware weighting (omega_{ij}) based on (sigma_i^2+sigma_j^2), an uncertainty regulariser to prevent variance explosion, and an auxiliary conflict loss aligning variance with pxtr disagreement. Theoretical analysis shows the weighting scheme reduces expected bias between learned and true satisfaction rankings.
- **Experiment**: Offline evaluation on a large industrial short‑video dataset shows UAME outperforms EMER and EASQ on NDCG, MAP, and a questionnaire‑based satisfaction score, with relative gains of 5.8% (PWTR vs EMER), 14.2% (PLVTR vs EASQ), and 9.2% (pCTR vs EASQ). Online A/B tests demonstrate statistically significant improvements in watch time, engagement, and revenue, and enhanced stability across traffic variations. Ablation studies confirm each component (Gaussian scoring, probabilistic pairwise loss, uncertainty weighting, regularisation, auxiliary loss) contributes positively; removing any degrades performance. UAME consistently achieves the best or tied scores on all eight core industrial metrics across both EMER and EASQ backbones.
- **Takeaway**: UAME provides a principled, uncertainty‑aware framework that directly addresses proxy‑label bias, improves alignment with true user satisfaction, achieves superior ranking performance, and offers operational benefits such as reduced latency, simplified maintenance, and valuable variance estimates for downstream decision making.
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding and ranking models. The Matryoshka Hypencoder's use of neural networks for query encoding and its efficiency trade-offs align with your focus on real-time relevance optimization. However, the paper's specific application to the Hypencoder approach is a more specialized topic within IR.
...
### 4. DeLIVeR: Decomposed Learning for Information-grounded Veracity Recognition via Reinforced Knowledge Graph Exploration
...
- **Reason**: This paper is highly relevant to Information Retrieval, particularly in the context of query understanding and evidence retrieval. The use of Knowledge Graphs and reinforced question-planning strategy aligns with your interests in deep semantic understanding and real-time relevance optimization. While the focus is on fact-checking, the techniques and methods employed are applicable to broader search and retrieval tasks.
...
### 5. VDAR-Router: Adaptive LLMs Routing via Verbalized Query Difficulty Analysis Retrieval
...
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in query understanding and ranking models. The proposed VDAR-Router framework uses difficulty analysis and retrieval-based routing, which aligns with your focus on deep semantic understanding and real-time relevance optimization. Although the paper does not explicitly mention Learning to Rank or user behavior modeling, its relevance to IR and NLP makes it a useful contribution to your field.
...
### 6. MagicSelector: Joint Optimization for Agent Tool Selection via Counterfactual Decomposition and Progressive Reranking
...
- **Reason**: This paper presents a framework for tool retrieval in agents, leveraging techniques such as counterfactual task decomposition, progressive reranking, and dynamic Top-K. While primarily focused on tool retrieval, it involves aspects of query understanding and ranking models, which align with your interests in Information Retrieval. However, the specific domain and application (agent tool selection) are somewhat distinct from your core research themes.
...
### 7. RIMS: Preference Optimization via Smoothed Multi-pair Aggregation for Small-Scale LLM Retrieval-Augmented Generation

- **LLM Score**: 7
- **Keyword Score**: 6
- **Authors**: Pei Tian, Zihan Dong, Tianci Liu, Linjun Zhang, Haoyu Wang
- **URL**: <http://arxiv.org/abs/2607.16431v1>
- **Submitted**: 2026-07-17 18:24:19
- **Topic Keywords**: rag, retrieval, acl
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, particularly in the context of retrieval-augmented generation and preference optimization. However, it focuses on language models and generation, which is a related but distinct area of study. The paper's emphasis on deep semantic understanding and real-time relevance optimization aligns with your interests, but the specific application to small-scale language models and generation is not a central match.
...
### 8. It Matters How You Say It: Exploring Rhetorical Patterns for AI-Assisted Information Evaluation
...
- **Reason**: This paper explores the role of rhetorical patterns in AI-assisted information evaluation, which is somewhat related to query understanding and user behavior modeling in Information Retrieval. While it doesn't directly address ranking models or click models, it touches on aspects of user interaction and reflection, making it somewhat relevant to your research interests.
...
### 9. Calibrating Semantic Uncertainty from Observable Language-Model Probabilities
...
- **Reason**: This paper explores the concept of semantic uncertainty in language models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on language model probabilities and their calibration to meaningful states is not directly aligned with the user's primary research interests in IR and Search technologies. The connection to NLP and data mining is more relevant, but the paper's scope is narrower than the user's broader interests.
...
### 10. Debate-on-Graph: Reliable and Adaptive Reasoning of Large Language Model on Uncertain Knowledge Graph

- **LLM Score**: 6
- **Keyword Score**: 3
- **Authors**: Peiji Yu, Xin Chen, Tianxing Wu
- **URL**: <http://arxiv.org/abs/2607.17266v1>
- **Submitted**: 2026-07-19 14:17:37
- **Comment**: 18 pages, Accepted by ECML-PKDD 2026
- **Topic Keywords**: rag, search
- **Reason**: This paper explores the use of uncertain knowledge graphs to enhance large language models for question answering tasks, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on knowledge graphs and language models is more aligned with Natural Language Processing, and the paper does not directly address user behavior modeling or real-time relevance optimization.
...
### 11. NOWJ@COLIEE 2026: Adaptive Pipelines for Legal Retrieval and Reasoning
...
- **URL**: <http://arxiv.org/abs/2607.16603v1>
- **Submitted**: 2026-07-18 02:46:06
- **Comment**: Presented at COLIEE 2026
- **Topic Keywords**: dense retrieval, query, ranking, rerank, pairwise, retrieval, rank
- **Reason**: The paper is somewhat related to Information Retrieval, specifically in the legal domain, but it doesn't align with the user's core research themes of query understanding, ranking models, and user behavior modeling. The focus on legal retrieval and reasoning is not a central match to the user's interests in e-commerce or deep semantic understanding.
...
- **Reason**: The paper FinSAgent explores a specific application of Information Retrieval in the financial domain, which is somewhat related to your interests in IR and Search technologies. However, the focus on evidence-grounded SEC filing question answering and the use of multi-agent frameworks is not directly aligned with your core research themes of query understanding, ranking models, and user behavior modeling.
...
### 13. Evidence-in-the-Loop: Trace-Driven Optimization for Customer-Service LLM Agents
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, particularly in the context of customer-service bots and large language models. However, the focus on evidence-grounded decision-making and deployment patterns in a customer-service setting is not directly aligned with your primary interests in query understanding, ranking models, and user behavior modeling.
...
- **Reason**: The paper RAMP focuses on improving CTR/CVR prediction accuracy in online advertising by addressing the challenge of limited personalized-feature availability. While it involves click models and user behavior modeling, the primary focus is on recommender systems rather than information retrieval. The paper's emphasis on robust ad recommendation and its application in privacy-constrained settings is somewhat related to the user's interests in search technologies and real-time relevance optimization, but it does not directly align with the user's core research themes in IR and NLP.
...
### 15. WHALE: A Scalable Unified Model for Recommendation with Wukong-HSTU Architecture
...
- **Submitted**: 2026-07-19 00:59:09
- **Topic Keywords**: query, ranking, user behavior, recommend, rank
- **Reason**: The paper WHALE presents a scalable unified recommendation architecture that combines non-sequence and sequence features, which is somewhat related to information retrieval and ranking models. However, the focus on recommendation systems and user behavior modeling is not a central match to your primary research interests in information retrieval, especially in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 16. Adaptive Incident Prioritization for Security Operations at Scale

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Scott Freitas, Amir Gharib, Maayan Magenheim
- **URL**: <http://arxiv.org/abs/2607.16963v1>
- **Submitted**: 2026-07-18 20:43:35
- **Topic Keywords**: query, ranking, rank, search
- **Reason**: The paper discusses Adaptive Incident Prioritization (AIP), a ranking algorithm for security incidents. While it involves ranking and query-less multi-tenant queue setting, it is primarily focused on security operations and incident prioritization, which is somewhat related to information retrieval but not directly aligned with your core research themes of query understanding, ranking models, and user behavior modeling.
...
### 17. Salience Induction against Multi-Hop RAG Agents: Threat and Defense
...
- **Reason**: The paper discusses a novel attack surface in retrieval-augmented generation systems, specifically salience induction, which can redirect reasoning without injecting false facts. While it touches on information retrieval and NLP, the focus is on a specific threat and defense, rather than query understanding, ranking models, or user behavior modeling, which are core to your research interests.
...
### 18. Is Progressive Disclosure All You Need for Long-Context Agents?
...
- **Reason**: This paper explores the concept of progressive disclosure in the context of long-document question answering, which is somewhat related to information retrieval and search technologies. However, the focus on agent skills and navigation patterns in a specific domain (question answering) is not a central match to the user's core research themes, particularly query understanding and ranking models.
...
### 19. Learning Sparse Representations of Multimodal Content for Enhanced Cold Item Recommendation

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Gregor Meehan, Johan Pauwels
- **URL**: <http://arxiv.org/abs/2607.17184v1>
- **Submitted**: 2026-07-19 10:45:57
- **Comment**: Accepted at RecSys 2026
- **Topic Keywords**: rag, retrieval, recommend
- **Reason**: The paper explores recommender systems, specifically addressing the cold-start problem through sparse representation learning. While it touches on aspects of information retrieval, such as content-based recommendation, it does not directly relate to query understanding, ranking models, or user behavior modeling, which are core areas of interest. The focus on recommender systems and sparse embeddings, although relevant to IR, is not a central match to the user's primary research themes.
...
### 20. Oracle Gap and Signal Fidelity: A Fixed-Pool Diagnostic for Test-Time Collaboration
...
- **Topic Keywords**: rag, acl
- **Reason**: This paper explores test-time collaboration in the context of Large Language Models (LLMs), which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on collaboration and verification in LLMs is not a central match to the user's primary research interests in IR and search technologies. The paper's relevance is loosely related to the user's background in the e-commerce domain, but the connection is not strong enough to warrant a higher score.
...
### 21. DS@GT ARC at eRisk 2026: Hybrid Multi-Agent LLM System with Structured Algorithmic Guidance for Conversational Depression Screening

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Victor Gong, David Guecha
- **URL**: <http://arxiv.org/abs/2607.16712v1>
- **Submitted**: 2026-07-18 09:02:19
- **Comment**: Conference and Labs of the Evaluation Forum (CLEF), 19 pages, 5 figures
- **Topic Keywords**: ranking, rank
- **Reason**: This paper is somewhat related to the user's interests in Natural Language Processing (NLP) and conversational systems, but it does not directly align with their primary focus on Information Retrieval (IR) and query understanding. The paper's emphasis on conversational depression screening and algorithmic supervision is not a central match for the user's research themes.
...
- **Reason**: This paper explores the interaction between language models and user expressions of belief, which is somewhat related to query understanding and user behavior modeling in Information Retrieval. However, the focus on linguistic variation and its impact on LLMs is more aligned with Natural Language Processing, and the paper does not directly address ranking models or real-time relevance optimization.
...
### 23. L1 Augmented Attention as an Improved Vector Similarity Metric
...
- **Reason**: This paper introduces a new similarity metric, L1 augmented attention, which combines dot product attention with L1 distance. While it has implications for Natural Language Processing (NLP) and Transformer models, it does not directly address query understanding, ranking models, or user behavior modeling, which are core areas of interest. The paper's focus on similarity computation and geometric roles may be tangentially related to information retrieval, but it is not a central match.
...
- **Reason**: The paper is somewhat related to the user's interests in Natural Language Processing (NLP) and data mining, but it primarily focuses on citation function classification, which is not a central match for the user's core research themes in Information Retrieval (IR) and Search technologies. The paper's use of large language models is relevant, but the specific application to citation function classification is not directly aligned with the user's interests.
...
### 25. DRNOISE: Benchmarking Deep Research Agents in Misleading Evidence Environments
...
- **Submitted**: 2026-07-19 15:20:40
- **Comment**: 16 pages, 2 figures, 11 tables
- **Topic Keywords**: retrieval, search
- **Reason**: The paper explores a benchmark for evaluating deep research agents in misleading evidence environments, which is somewhat related to information retrieval and query understanding. However, the focus on answer recovery and verification inertia is more aligned with NLP and data mining, rather than the user's primary interest in search technologies and ranking models. While the paper touches on the importance of active reconciliation, it does not directly address the user's core research themes.
...
### 26. SynH-Rank: Quality-Aware Code Search via Diverse Data Synthesis and Hierarchical Ranking Training

- **LLM Score**: 2
- **Keyword Score**: 14
- **Authors**: Keyu Liang, Haoye Wang, Yanfu Yan, Zhiyuan Wan, Zhongxin Liu
- **URL**: <http://arxiv.org/abs/2607.17139v1>
- **Submitted**: 2026-07-19 08:52:36
- **Topic Keywords**: queries, ranking, rerank, relevance, rank, search
- **Reason**: While this paper explores ranking models and code search, it is primarily focused on code search in the software development domain, which is not a central match to your interests in information retrieval, search technologies, and natural language processing.
...
### 27. TurboVec: A Case Study in Cost-Efficient Private Retrieval for Enterprise RAG via Codebook-Oblivious Quantization

- **LLM Score**: 2
- **Keyword Score**: 10
- **Authors**: Navnit Shukla, Kamal Pandey, Omsankar Tiwari
- **URL**: <http://arxiv.org/abs/2607.16973v1>
- **Submitted**: 2026-07-18 21:43:40
- **Topic Keywords**: query, queries, rag, retrieval
- **Reason**: This paper focuses on a specific problem in Retrieval-Augmented Generation (RAG) systems, namely private vector retrieval for enterprise applications. While it touches on indexing and quantization techniques, it does not directly relate to query understanding, ranking models, or user behavior modeling, which are core areas of interest for your research.
...
### 28. AlphaOracle: Oracle bone script decipherment via human-workflow-inspired deep learning
...
- **Reason**: This paper is not relevant to your research interests as it focuses on deciphering an ancient script using a human-workflow-inspired deep learning framework, which is outside the scope of information retrieval, search technologies, and natural language processing.
...
### 29. Team DACTYL at PAN 2026: Bayesian Data Mixing and Empirical X-risk Minimization for AI-text Detection
...
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves text detection and classification, the focus is on AI-generated text and dataset curation, which does not align with your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 30. How Jailbreak Attacks Inform Safety Alignment: A Defender-Centric, Shapley-Based Evaluation of Jailbreak Contributions

- **LLM Score**: 2
- **Keyword Score**: 7
- **Authors**: Yukai Zhou, Feiyang Lu, Xiaokai Mao, Jinfei Liu, Wenjie Wang
- **URL**: <http://arxiv.org/abs/2607.17152v1>
- **Submitted**: 2026-07-19 09:18:35
- **Topic Keywords**: queries, ranking, rank
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or Natural Language Processing. The paper focuses on evaluating jailbreak attacks on large language models from a defender-centric perspective, which is outside your primary areas of interest.
...
### 31. Adapting Embedding Models for Agent Capability Retrieval

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Tingwei Chen, Yunxiao Shi, Zhengdong Chu, Qingsong Wen, Min Xu
- **URL**: <http://arxiv.org/abs/2607.17347v1>
- **Submitted**: 2026-07-19 17:17:47
- **Comment**: Accepted for oral presentation at the AgentSearch Workshop, SIGIR 2026
- **Topic Keywords**: queries, retrieval, search
- **Reason**: This paper focuses on adapting retrieval models for agent capability retrieval in open agent marketplaces, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves retrieval models, the context is specific to agent marketplaces and does not align with the user's interests in e-commerce, query understanding, ranking models, or user behavior modeling.
...
### 32. D-NOVA: In-Storage Retrieval Accelerator via Dual-Bound 3D NAND-Optimized Similarity Search with Vector Adaptation
...
- **Reason**: This paper focuses on hardware acceleration for similarity search in storage, which is not directly related to information retrieval, query understanding, or ranking models. While it touches on vector search, it's more focused on hardware optimization and does not address the deep semantic understanding or real-time relevance optimization aspects of your research interests.
...
### 33. KyrgyzLLM-Bench: Benchmarking Kyrgyz Language Understanding
...
- **Submitted**: 2026-07-19 10:17:18
- **Comment**: Preprint; manuscript currently under consideration at a journal
- **Topic Keywords**: ranking, rank, search
- **Reason**: This paper focuses on evaluating large language models in the Kyrgyz language, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it does involve NLP, the specific context and language are not aligned with your areas of focus.
...
### 34. Fenced Citation-Context Retrieval for Case Law: Temporal Leakage and Degree Control Across Two Jurisdictions

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Yao Liu, Tien-Ping Tan, Zhilan Liu
- **URL**: <http://arxiv.org/abs/2607.17142v1>
- **Submitted**: 2026-07-19 08:57:40
- **Topic Keywords**: query, retrieval
- **Reason**: This paper focuses on legal precedent retrieval and citation context analysis, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves some aspects of ranking models, the context is highly specialized in the legal domain and does not align with the user's interests in e-commerce or real-time relevance optimization.
...
### 35. What Transfers Under Source Shift? Definitions, Examples, and Fine-Tuning for Climate Disclosure Classification
...
- **Reason**: This paper focuses on climate disclosure classification and adaptation strategies for large language models (LLMs), which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the specific application and context are quite different from the user's areas of focus.
...
### 36. ANNLib: A Development Framework for Efficient Approximate Nearest Neighbor Search
...
- **Reason**: This paper is not directly related to your core research interests in Information Retrieval, Search technologies, query understanding, ranking models, or user behavior modeling. While it touches on deep learning pipelines, it focuses on Approximate Nearest Neighbor Search, which is not a primary area of interest for you.
...
### 37. SWE-Pruner Pro: The Coder LLM Already Knows What to Prune
...
- **Reason**: This paper is not relevant to your research interests as it focuses on pruning long context for coding agents using a Large Language Model (LLM), which is outside your primary areas of interest in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 38. Benchmarking Resource-Efficient LLMs for Research Topic Ontology Generation in the Biomedical Field
...
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on applying Large Language Models (LLMs) for ontology generation in the biomedical field, which is outside your primary research interests in Information Retrieval and Search technologies. While it involves NLP, the specific application and domain are not aligned with your core themes.
...
- **Reason**: This paper focuses on optimizing large language models using a preconditioned orthogonalized LoRA optimizer, which is not directly related to information retrieval, search technologies, or user behavior modeling. While it involves deep learning and optimization techniques, its application is primarily in the NLP domain, which is a secondary interest of yours.
...
### 40. A Systematic Evaluation of Traditional Privacy Policy Analysis Tools Against LLMs

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Madhav Aryal, Sudipa Saha, Kaushal Kafle, Anshuman Chhabra, Sunil Manandhar
- **URL**: <http://arxiv.org/abs/2607.17075v1>
- **Submitted**: 2026-07-19 04:50:05
- **Topic Keywords**: rag, search
- **Reason**: The paper is not directly related to Information Retrieval, Search technologies, or user behavior modeling, which are the user's core research themes. While it involves Natural Language Processing (NLP) and deep semantic understanding, the focus is on privacy policy analysis and data compliance, which is a distinct area of research.
...
### 41. Training Continuous Chain of Thought Models: A Tale of Two Regimes

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Varun Yerram, He He, Eunsol Choi
- **URL**: <http://arxiv.org/abs/2607.16972v1>
- **Submitted**: 2026-07-18 21:32:06
- **Comment**: Accepted to AdaptFM Workshop, ICML 2026
- **Topic Keywords**: rag, search
- **Reason**: This paper focuses on developing a more efficient method for training Chain-of-Thought models, which is a topic in Natural Language Processing. However, it does not appear to be directly related to Information Retrieval, query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
...
### 42. A Method for Learning Value Systems in Generative AI

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Andrés Holgado-Sánchez, Holger Billhardt, Sascha Ossowski
- **URL**: <http://arxiv.org/abs/2607.16903v1>
- **Submitted**: 2026-07-18 17:37:50
...
- **Reason**: This paper focuses on value learning in generative AI, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves machine learning and data analysis, the context and application are quite different from the user's areas of interest.
...
### 43. JOR-Bench: Japanese Operations Research Benchmarks for Large Language Models

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Yuu Jinnai
- **URL**: <http://arxiv.org/abs/2607.16777v1>
- **Submitted**: 2026-07-18 11:24:10
- **Topic Keywords**: rag, search
- **Reason**: This paper is not directly related to Information Retrieval, Search technologies, or user behavior modeling, which are the core areas of your research interests. Although it involves Natural Language Processing (NLP) and large language models, the focus is on operations research and solver evaluation, which is not a central match to your primary research themes.
...
### 44. Dependency-Guided Code Generation: Structured Matrix Decomposition and Consistency-Guided Refinement
...
- **Submitted**: 2026-07-18 08:01:49
- **Comment**: 12 pages
- **Topic Keywords**: rag, rank
- **Reason**: This paper is not relevant to your research interests as it focuses on code generation in software engineering, which is outside your primary areas of interest in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 45. Encoding EEG Signals to Examine Human-Like Next-Word Prediction Behaviour in Language Models

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Boi Mai Quach, Binh T. Nguyen, Cathal Gurrin, Graham Healy
- **URL**: <http://arxiv.org/abs/2607.16549v1>
- **Submitted**: 2026-07-17 23:11:16
- **Topic Keywords**: ctr, search
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it touches on language models, it focuses on cognitive signals and brain responses, which is not a central theme in your work.
...
### 46. Committed Before Reasoning: Behavioral Reproduction and Preliminary Activation-Level Evidence of Answer Pre-Commitment in an Open-Weight LLM

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Heejin Jo
- **URL**: <http://arxiv.org/abs/2607.16451v1>
- **Submitted**: 2026-07-17 18:49:15
- **Comment**: 8 pages. Code, data, and all reported statistics: https://github.com/JO-HEEJIN/interview_mate/tree/docs/car-wash-repro/car_wash/paper_4
- **Topic Keywords**: recommend, acl
- **Reason**: This paper appears to be related to Natural Language Processing (NLP), specifically focusing on the behavior of Large Language Models (LLMs). However, it does not seem to be directly related to Information Retrieval (IR), query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
...
### 47. Do Language Models Dream of Binding Molecules? Benchmarking LLMs under Spatial Constraints
...
- **Topic Keywords**: rag
- **Reason**: This paper is not relevant to your research interests as it focuses on the application of Language Models (LLMs) in molecular design and 3D spatial environments, which is outside your core areas of Information Retrieval, Search technologies, and Natural Language Processing.
...
### 48. Remote Awareness of Seafloor Images Collected by AUVs over Low-Bandwidth Communication Links
...
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, Natural Language Processing, data mining, or recommender systems. The focus is on real-time processing and transmission of AUV imagery over low-bandwidth communication links, which is outside your areas of expertise.
...
### 49. C$^2$KV: Compressed and Composable KV Cache Reuse for Efficient LLM Inference
...
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper focuses on optimizing large language model inference using a KV cache reuse framework, which is unrelated to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 50. VEHBench: A Stage-Local Diagnostic Benchmark for LLM-Assisted Vibration Energy Harvester Design
...
- **Topic Keywords**: search, acl
- **Reason**: This paper appears to be unrelated to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The focus is on vibration energy harvesters and engineering workflows, which does not align with your areas of expertise.
