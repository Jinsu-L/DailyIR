...
### 1. Explaining When PRF Fails: Participatory Auditing for Selective Query Expansion

- **LLM Score**: 8
- **Keyword Score**: 25
- **Authors**: Zeyan Liang, Graham McDonald, Iadh Ounis
- **URL**: <http://arxiv.org/abs/2607.12098v1>
- **Submitted**: 2026-07-13 19:14:21
...
- **Topic Keywords**: query, queries, ranking, rerank, relevance, relevance feedback, rag, retrieval, rank, trec
- **Reason**: This paper is highly relevant to Information Retrieval, specifically addressing the limitations of Pseudo-Relevance Feedback (PRF) and proposing a novel framework for explainability. The focus on query understanding and user behavior modeling aligns with your research interests, although the domain is not explicitly e-commerce. The use of deep learning-based rerankers is also a relevant aspect of the paper.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: Pseudo‑relevance feedback transparency
- **Aim**: Transform PRF from a black‑box booster into a selective, user‑grounded, traceable component via a two‑stage audit‑then‑automate pipeline
- **Rationale**: PRF routinely improves aggregate metrics yet induces query drift; existing selective PRF relies on opaque QPP derived from the same ranking statistics, perpetuating the explainability gap. A user‑centric audit can expose per‑query benefits and harms, providing a principled basis for selective application and regulatory compliance.
- **Ground**: A participatory audit with 108 participants on 43 TREC DL 2019 queries using Team Draft Interleaving to create an unbiased document pool. Users interacted with a 3×3 grid UI, clicking and selecting passages to generate high‑confidence preference signals. Findings: 20.9% of queries benefited from PRF, 25.6% were harmed, 53.5% were neutral. Reverting harmful queries to baseline yielded a 68.5% relative improvement in engagement, nearly twice the 37.0% gain from applying PRF only to beneficial queries. Difference‑in‑differences analysis and statistical tests (p=0.032, Cohen’s d=1.9) confirmed the superiority of selective PRF.
- **Experiment**: Stage 1: Conducted the audit to collect user‑derived preference signals. Stage 2: Trained a large‑language‑model reranker (LLM) to rank the same interleaved document pool, using its ranking as a proxy for system preference. The LLM’s ranking provides traceable explanations (documents and passages highlighted). Deployment is always‑on, eliminating opaque expansion switches. The pipeline was validated via difference‑in‑differences and statistical significance tests.
- **Takeaway**: The audit‑then‑automate framework delivers a transparent, selective PRF system that avoids query drift, achieves double the engagement benefit of benefit‑only PRF, and offers process‑level explainability traceable from user signals to LLM output. It satisfies emerging regulatory transparency demands and generalizes to other opaque IR components.
...
### 2. QUBO-Optimized Evidence Selection for Retrieval-Augmented Question Answering with Unconventional Solvers

- **LLM Score**: 8
- **Keyword Score**: 11
- **Authors**: Rahul Singh, Madhav Vadlamani
- **URL**: <http://arxiv.org/abs/2607.12334v1>
- **Submitted**: 2026-07-14 04:29:07
- **Topic Keywords**: ranking, relevance, rag, retrieval, rank
- **Reason**: This paper is highly relevant to your research interests in Information Retrieval, particularly in the context of query understanding and ranking models. The use of Quadratic Unconstrained Binary Optimization (QUBO) for evidence selection in Retrieval-Augmented Question Answering (RAG) is an innovative approach that aligns with your focus on deep semantic understanding and real-time relevance optimization.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: QUBO‑Based Context‑Selection for Retrieval‑Augmented Question Answering
- **Aim**: Construct a compact, non‑redundant set of passages that jointly satisfies every information‑requirement of a multi‑hop query, improving answer accuracy while keeping context short for efficient generation.
- **Rationale**: Traditional retrieve‑then‑rerank keeps redundant or irrelevant evidence, missing necessary facts and allowing distractors. Casting evidence selection as a set‑level combinatorial optimization simultaneously rewards relevance, full coverage, complementarity, and penalizes redundancy, length, and unsupported passages. This binary energy‑minimization can be expressed as a QUBO, enabling efficient solution on quantum annealers, Ising machines, or quantum‑inspired solvers.
- **Ground**: Problem setting, motivation for set‑selection, detailed QUBO formulation (variables, linear/quadratic terms, constraints), modular RAG pipeline (query decomposition, candidate retrieval, scoring, QUBO optimization, generation), experimental setup (HotpotQA, baselines, metrics, solver), configuration sweep (C0–C12), results, ablation insights, contributions, future directions, related work.
- **Experiment**: HotpotQA multi‑hop benchmark; baselines: BM25, top‑k, MMR, hybrid lexical‑semantic ranking, greedy coverage, random, LLM‑based selectors (SetR‑style). Metrics: Exact Match, token‑level F1, requirement coverage, redundancy, compactness (#selected passages). Solver: simulated annealing for hyper‑parameter sweep across configurations C0–C12. Key findings: C1 (weak budget penalty) achieved EM≈0.58, F1≈0.70, 100 % coverage with 3–6 passages, outperforming all non‑LLM baselines and matching or surpassing LLM selectors.
- **Takeaway**: The QUBO formulation provides a hardware‑friendly, requirement‑aware evidence selector that yields state‑of‑the‑art answer accuracy with a compact evidence set. It decouples selection from generation, enabling modular tuning and low‑latency, energy‑efficient deployment on quantum/Ising hardware. Future work includes scaling to larger benchmarks, learning QUBO coefficients, and evaluating on specialized Ising/QUBO devices.
...
### 3. Comparing Semantic Navigation in Humans and Large Language Models using Natural Language Processing

- **LLM Score**: 8
- **Keyword Score**: 6
- **Authors**: Gabriel Paris-Colombo, Rodrigo M. Cabral-Carvalho, Felipe D. Toro-Hernández
- **URL**: <http://arxiv.org/abs/2607.12195v1>
- **Submitted**: 2026-07-13 22:38:58
- **Comment**: Cogsci paper 2026
- **Topic Keywords**: semantic search, retrieval, search
- **Reason**: This paper explores semantic search dynamics in humans and large language models, which aligns with your interests in query understanding and ranking models. Although the focus is on NLP and not directly on IR or e-commerce, the study's emphasis on semantic understanding and real-time relevance optimization makes it relevant to your broader research themes.
...
- **Submitted**: 2026-07-14 06:12:26
- **Topic Keywords**: rag, retrieval, recommend
- **Reason**: This paper is highly relevant to Information Retrieval, specifically addressing the challenge of scaling up retrieval with heterogeneous content unification. The proposed MESH framework mitigates the Scaling Bias of Heterogeneity, leading to improved scaling behavior and significant performance gains in online evaluations. While not directly focused on query understanding or ranking models, the work has implications for real-time relevance optimization and large-scale retrieval systems.
...
- **URL**: <http://arxiv.org/abs/2607.12621v1>
- **Submitted**: 2026-07-14 10:58:12
- **Topic Keywords**: ranking, rerank, retrieval, rank
- **Reason**: This paper explores a novel approach to Composed Image Retrieval (CIR) using Vision-Free techniques, which aligns with your interests in Information Retrieval and Search technologies. Although it focuses on image retrieval, the use of deep semantic understanding and reranking models is relevant to your research. However, the specific domain of image retrieval and the use of LLMs for reranking, while related, do not perfectly match your primary focus on query understanding and ranking models in the e-commerce domain.
...
- **Submitted**: 2026-07-14 03:33:00
- **Comment**: 24 pages, 4 figures, 18 tables. Accepted at the Conference on Language Modeling (COLM) 2026
- **Topic Keywords**: rag, retrieval
- **Reason**: This paper is relevant to your research interests in Information Retrieval, particularly in the context of question answering and retrieval-augmented generation. However, the focus on data lakes and enterprise knowledge graphs is somewhat specific and may not directly align with your primary interests in e-commerce and deep semantic understanding. Nevertheless, the paper's emphasis on robust discovery and cross-file composition mechanisms may be of interest to you.
...
- **URL**: <http://arxiv.org/abs/2607.12831v1>
- **Submitted**: 2026-07-14 14:48:53
- **Topic Keywords**: retrieval
- **Reason**: This paper explores the concept of suppressing parametric recall in language models, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on language models and evidence-grounded reasoning is not a central match to your primary research interests in IR and Search technologies. The paper's relevance to your interests is somewhat tangential, but it may be useful for exploring the intersection of NLP and IR.
...
### 8. SlimPer: Make Personalization Model Slim and Smart
...
- **Submitted**: 2026-07-14 02:31:32
- **Topic Keywords**: queries, ranking, relevance, rag, recommend, personalization, rank
- **Reason**: The paper discusses a personalized ranking model, SlimPer, which is relevant to information retrieval and search technologies. However, the focus is on recommendation systems, which is somewhat related to your interests, but not a central match. The use of transformer-style architectures and attention mechanism is also relevant to NLP, but the application is primarily in recommender systems.
...
### 9. Cheaper is Better: A Discount-Aware Network for Conversion Rate Prediction in E-commerce Recommendation System

- **LLM Score**: 4
- **Keyword Score**: 11
- **Authors**: Ruocong Tang, Yang Huang, Xing Fang, Chenyi Yan, Chuike Sun, Jing Wang
- **URL**: <http://arxiv.org/abs/2607.12578v1>
- **Submitted**: 2026-07-14 09:49:17
- **Topic Keywords**: click, ctr, cvr, conversion rate, recommend, commerce, e-commerce
- **Reason**: The paper is somewhat related to the user's interests in Information Retrieval and e-commerce, but it focuses on conversion rate prediction and recommender systems, which is not the user's primary focus. The use of deep learning models and real-time relevance optimization is relevant, but the paper's emphasis on discount rates and conversion rates is more aligned with recommender systems than information retrieval.
...
- **URL**: <http://arxiv.org/abs/2607.12252v2>
- **Submitted**: 2026-07-14 01:31:20
- **Topic Keywords**: query, queries, ranking, rank, search
- **Reason**: This paper is somewhat related to information retrieval, but it focuses on a specific domain (financial reports) and uses a unique approach (deep research benchmark) that doesn't align with the user's core research themes. While it involves query understanding and ranking models, the context is not directly applicable to the user's interests in e-commerce or general search technologies.
...
### 11. What Would You Click? Personalized Video Thumbnail Generation with Preference-aware Highlight Retrieval

- **LLM Score**: 4
- **Keyword Score**: 8
- **Authors**: Zhiyu He, Zecheng Zhao, Tong Chen, Zi Huang, Yiqun Liu, Min Zhang
- **URL**: <http://arxiv.org/abs/2607.12882v1>
- **Submitted**: 2026-07-14 15:30:09
- **Topic Keywords**: retriever, click, retrieval, personalization
- **Reason**: This paper is somewhat related to your interests in Information Retrieval and Search technologies, as it involves personalized video thumbnail generation with preference-aware highlight retrieval. However, it is not directly related to your core research themes of query understanding, ranking models, and user behavior modeling. The paper's focus on visual content and user preferences is somewhat tangential to your broader interests in NLP, data mining, and related topics.
...
### 12. SHEAF: Self-profiled Hardness Estimation from Answer-set Flux for Predicting Query Hardness in Graph-based ANN Search

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Dongfang Zhao
- **URL**: <http://arxiv.org/abs/2607.12229v1>
- **Submitted**: 2026-07-14 00:20:13
- **Topic Keywords**: query, queries, search
- **Reason**: This paper is somewhat related to information retrieval, specifically in the context of graph-based approximate nearest neighbor search. However, it focuses on query hardness estimation and prediction, which, while relevant to search technologies, is not a central match to your primary research interests in query understanding, ranking models, and user behavior modeling.
...
### 13. Cost-Governed RAG: Unified Per-Tenant Cost Attribution Across Retrieval and Generation in Multi-Tenant LLM Systems

- **LLM Score**: 4
- **Keyword Score**: 7
- **Authors**: Navnit Shukla
- **URL**: <http://arxiv.org/abs/2607.12188v1>
- **Submitted**: 2026-07-13 22:16:58
- **Topic Keywords**: query, rag, retrieval
- **Reason**: The paper discusses a novel architecture for cost attribution in multi-tenant LLM systems, which is somewhat related to information retrieval and search technologies. However, the focus is more on cost governance and system optimization rather than query understanding, ranking models, or user behavior modeling, which are core areas of interest. The paper does touch on retrieval and generation, but the context is more on system architecture and cost attribution rather than deep semantic understanding or real-time relevance optimization.
...
### 14. RecRec: Latent Interests Recursive Reasoning for Sequential Recommendation
...
- **URL**: <http://arxiv.org/abs/2607.12945v1>
- **Submitted**: 2026-07-14 16:28:28
- **Comment**: Accepted at RecSys 2026, 10 pages
- **Topic Keywords**: rag, user behavior, recommend
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Search technologies, as it explores sequential recommendation and latent reasoning. However, the focus on recommender systems and sequential recommendation is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling. The paper's use of deep supervision and latent interests is somewhat relevant to your background in NLP and data mining, but it does not align with your core research themes.
...
### 15. Learning to Forget: Satiation-Aware Long-Sequence Transducers for Mitigating Post-Purchase Redundancy

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Yipin Dai, Ruocong Tang, Xing Fang, Yang Huang, Jing Wang, Zhentao Song, He Guo
- **URL**: <http://arxiv.org/abs/2607.12714v1>
- **Submitted**: 2026-07-14 12:40:32
- **Topic Keywords**: click, recommend, commerce, e-commerce
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of recommender systems. However, its focus on sequential recommendation models and post-purchase redundancy in e-commerce scenarios is not a central match to your primary interests in query understanding, ranking models, and user behavior modeling.
...
### 16. On-Device Deep Research at 4B: Exposure Bounds Faithfulness, Retrieval Bounds Coverage

- **LLM Score**: 4
- **Keyword Score**: 5
- **Authors**: Vinay Kumar Chaganti
- **URL**: <http://arxiv.org/abs/2607.12257v1>
- **Submitted**: 2026-07-14 01:57:39
- **Comment**: 13 pages, 2 figures, appendix
- **Topic Keywords**: rag, retrieval, search
- **Reason**: This paper explores the relationship between exposure and faithfulness in on-device research agents, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on citation faithfulness and coverage in a specific context (on-device research) is not directly aligned with the user's core research themes.
...
### 17. Fin-Analyst at FinMMEval 2026 Task 3: A Live Hybrid Trading Agent with LLM Specialists and Rule-Based Signals

- **LLM Score**: 4
- **Keyword Score**: 4
- **Authors**: Mohotarema Rashid, Lingzi Hong, Junhua Ding, K. S. M. Tozammel Hossain
- **URL**: <http://arxiv.org/abs/2607.12233v1>
- **Submitted**: 2026-07-14 00:27:07
- **Comment**: 14 pages, 7 tables, 1 figure. CLEF 2026 FinMMEval Task 3 Working Notes
- **Topic Keywords**: ranking, rank
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, as it involves the use of Large Language Models (LLMs) for trading and analysis. However, the focus on financial markets and trading agents is not a central match to your primary research themes, which are more focused on search technologies, query understanding, and user behavior modeling.
...
### 18. Can Induced Emotion Bias LLM Behaviors in Sequential Decision Making?
...
- **Submitted**: 2026-07-14 11:09:51
- **Topic Keywords**: rag, search
- **Reason**: This paper explores the effects of induced emotion on Large Language Model (LLM) decision-making, which is somewhat related to my interests in query understanding and user behavior modeling. However, the focus on LLMs and their decision-making dynamics is not directly aligned with my core research themes in Information Retrieval and Search technologies. The paper's relevance is also limited by its focus on sequential decision-making and affective modulation, which are not central to my research interests.
...
- **Submitted**: 2026-07-14 15:30:37
- **Comment**: Accepted to MLHC 2026
- **Topic Keywords**: acl
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, particularly in the context of query understanding and deep semantic understanding. However, the focus on medical conversations and misconception correction in large language models is not directly aligned with your primary research themes. While the paper touches on aspects of model behavior and evaluation, it does not specifically address ranking models, user behavior modeling, or real-time relevance optimization.
...
### 20. The Sound of Absence: Audio-Language Embedding Models Struggle with Negation

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Chun-Yi Kuan, Hung-yi Lee
- **URL**: <http://arxiv.org/abs/2607.12290v1>
- **Submitted**: 2026-07-14 02:55:00
- **Comment**: Manuscript in progress
- **Topic Keywords**: retrieval
- **Reason**: This paper explores the limitations of audio-language embedding models in handling negation, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on audio-language models and negation in captions is not directly aligned with the user's core research themes. The paper's relevance to the user's interests is limited, but it may provide some insights into the challenges of deep semantic understanding in specific domains.
...
### 21. Code-MUE: Measuring Code LLMs' Uncertainty through Execution-based Semantic Interaction Graphs

- **LLM Score**: 4
- **Keyword Score**: 2
- **Authors**: Xiaoning Ren, Yinxing Xue, Lei Ma, Yuheng Huang
- **URL**: <http://arxiv.org/abs/2607.12273v1>
- **Submitted**: 2026-07-14 02:23:17
- **Comment**: To appear at The ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA) 2026
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and deep semantic understanding, but its focus on code-based Large Language Models (LLMs) and uncertainty estimation is not directly aligned with your primary focus on Information Retrieval (IR) and search technologies. While the paper explores a novel approach to uncertainty measurement, it does not appear to be directly applicable to your areas of interest.
...
### 22. LLM Judges Can Be Too Generous When There Is No Reference Answer

- **LLM Score**: 4
- **Keyword Score**: 1
- **Authors**: Chalamalasetti Kranti, Sowmya Vajjala
- **URL**: <http://arxiv.org/abs/2607.12885v1>
- **Submitted**: 2026-07-14 15:31:33
- **Comment**: Preprint
- **Topic Keywords**: search
- **Reason**: This paper explores the reliability of LLM judges in evaluating open-ended model responses without a reference answer, which is somewhat related to query understanding and ranking models in Information Retrieval. However, the focus on LLM judges and their calibration is not directly aligned with the user's core research themes, and the paper's methodology and results are more relevant to NLP and evaluation metrics rather than IR and search technologies.
...
- **Submitted**: 2026-07-14 14:01:29
- **Comment**: Accepted by ACM MM2026
- **Topic Keywords**: search
- **Reason**: This paper explores multimodal emotion recognition and proposes a lightweight framework, which is somewhat related to information retrieval and search technologies. However, the focus on multimodal emotion language models and knowledge distillation is not directly aligned with the user's core research themes in query understanding, ranking models, and user behavior modeling.
...
### 24. Where Reasoning Matters: Rethinking Latent Reasoning in Semantic ID-based Generative Recommendation

- **LLM Score**: 4
- **Keyword Score**: 1
- **Authors**: Shangxin Yang, Min Gao, Zongwei Wang, Junliang Yu
- **URL**: <http://arxiv.org/abs/2607.12425v1>
- **Submitted**: 2026-07-14 06:55:12
- **Comment**: 12 pages, 7 figures, 5 tables
- **Topic Keywords**: recommend
- **Reason**: The paper explores a recommendation system using semantic ID-based generative recommendation, which is somewhat related to information retrieval, but it focuses on recommender systems rather than search technologies. The use of latent reasoning and information-gain budget allocation is an interesting aspect, but it's not directly aligned with the user's core research themes in IR and search technologies.
...
### 25. ViHoRec: A Quality-Controlled Vietnamese Hotel Recommendation Dataset and Cold-Start Benchmark

- **LLM Score**: 3
- **Keyword Score**: 2
- **Authors**: Minh Hoang Nguyen
- **URL**: <http://arxiv.org/abs/2607.12946v1>
- **Submitted**: 2026-07-14 16:28:40
- **Topic Keywords**: recommend, search
- **Reason**: This paper introduces a Vietnamese hotel recommendation dataset and benchmark, which is somewhat related to information retrieval and recommender systems. However, it focuses on the construction and evaluation of a specific dataset, rather than query understanding, ranking models, or user behavior modeling, which are core areas of interest for the user.
...
### 26. WikiSTAR: A System for Shedding Light on the Hidden History of Scientific Wikipedia Articles

- **LLM Score**: 3
- **Keyword Score**: 1
- **Authors**: Omer Ehrlich, Nitzan Barzilay, Rona Aviram, Tom Hope
- **URL**: <http://arxiv.org/abs/2607.12441v2>
- **Submitted**: 2026-07-14 07:15:38
- **Topic Keywords**: search
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, but it focuses on a specific application (Wikipedia revision history analysis) and does not directly address your core themes of query understanding, ranking models, or user behavior modeling.
...
### 27. Not Only NTP: Extending Training Signal Coverage for Generative Recommendation

- **LLM Score**: 2
- **Keyword Score**: 9
- **Authors**: Changhao Li, Shuli Wang, Junwei Yin, Senjie Kou, Yinqiu Huang, Chi Wang, Yinhua Zhu, Haitao Wang, Xingxing Wang
- **URL**: <http://arxiv.org/abs/2607.12277v1>
- **Submitted**: 2026-07-14 02:28:23
- **Topic Keywords**: ranking, rag, ctr, recommend, rank
- **Reason**: This paper focuses on Generative Recommendation and Temporal Contrastive Learning, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the paper's primary focus on recommender systems and lack of emphasis on deep semantic understanding and real-time relevance optimization makes it less relevant to your core research themes.
...
### 28. The One-Word Census: Answer-Choice Conformity Across 44 Language Models

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Tapan Parikh
- **URL**: <http://arxiv.org/abs/2607.12796v1>
- **Submitted**: 2026-07-14 14:12:05
- **Comment**: 19 pages, 5 figures. Data, prompts, and code: https://github.com/tap2k/modelun
- **Topic Keywords**: ranking, rag, rank
- **Reason**: This paper appears to be related to Natural Language Processing (NLP) and language models, but it does not align with your core research themes in Information Retrieval (IR) and Search technologies, particularly query understanding, ranking models, and user behavior modeling.
...
### 29. A JoLT for the KV Cache: Near-Lossless KV Cache Compression via Joint Tucker and JL-Residual Allocation for LLMs

- **LLM Score**: 2
- **Keyword Score**: 6
- **Authors**: Rahul Krishnan, Volker Schulz
- **URL**: <http://arxiv.org/abs/2607.12550v1>
- **Submitted**: 2026-07-14 09:23:20
- **Comment**: 11 pages, 1 figure
- **Topic Keywords**: query, retrieval, rank
- **Reason**: This paper focuses on optimizing the key-value cache for transformer inference, which is not directly related to information retrieval, search technologies, or query understanding. While it involves deep semantic understanding, the context is specific to large language models and cache optimization, which is not a central match to your research interests.
...
- **URL**: <http://arxiv.org/abs/2607.12341v1>
- **Submitted**: 2026-07-14 04:41:09
- **Topic Keywords**: query, rag
- **Reason**: This paper focuses on Text-to-SQL and column-level access control, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it involves semantic understanding, the context is specific to SQL and access control, making it less relevant to the user's interests.
...
### 31. The Capacity of Thought: Benchmarking Llama 3.2 in Semantic fMRI Neural Language Decoding and Improving the Huth Encoding-Model Baseline

- **LLM Score**: 2
- **Keyword Score**: 5
- **Authors**: Milos Suvakovic, Dom Marhoefer, Glenn Grant-Richards, Aidan Pinero
- **URL**: <http://arxiv.org/abs/2607.12079v1>
- **Submitted**: 2026-07-13 18:56:17
...
- **Reason**: This paper is not relevant to your research interests as it focuses on neural language decoding from fMRI signals and the evaluation of a specific language model's capacity, which does not align with your core research themes in Information Retrieval and Search technologies.
...
### 32. Do AI Agents Know When a Task Is Simple? Toward Complexity-Aware Reasoning and Execution

- **LLM Score**: 2
- **Keyword Score**: 4
- **Authors**: Junjie Yin, Xinyu Feng
- **URL**: <http://arxiv.org/abs/2607.13034v1>
- **Submitted**: 2026-07-14 17:59:31
...
- **Reason**: This paper focuses on task-aware execution and complexity estimation in AI agents, which is not directly related to your core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the idea of optimizing effort and scope, it's more aligned with the broader field of Artificial Intelligence and Engineering Grounded AI, rather than your specific areas of focus.
...
### 33. Token Reduction Is Not Cost Reduction

- **LLM Score**: 2
- **Keyword Score**: 3
- **Authors**: Sarel Weinberger, Amir Hozez
- **URL**: <http://arxiv.org/abs/2607.12161v2>
- **Submitted**: 2026-07-13 21:10:22
- **Topic Keywords**: retrieval, rank
- **Reason**: This paper appears to be focused on evaluating the effectiveness of context-reduction systems in API-based coding agents, primarily through the lens of cost reduction. While it touches on the concept of 'retrieval rankers', it does not seem to be directly related to information retrieval, query understanding, or ranking models, which are core areas of your research interests.

#### Abstract
> Context-reduction layers for API-based coding agents, including command-output compressors, retrieval rankers, and API-boundary proxies, are commonly evaluated by how much context or tool output they remove. We ask a different question: which interventions actually reduce end-to-end billed cost while preserving task success?
...
- **Submitted**: 2026-07-14 15:33:44
- **Topic Keywords**: retrieval
- **Reason**: This paper focuses on long-term memory in conversational AI, introducing a benchmark called MemOps. While it touches on the idea of dynamic interactions, it does not directly relate to information retrieval, search technologies, or query understanding, which are core areas of your research interests.
...
### 35. KnowAct-GUIClaw: Know Deeply, Act Perfectly, Personal GUI Assistant with Self-Evolving Memory and Skill
...
- **URL**: <http://arxiv.org/abs/2607.12625v2>
- **Submitted**: 2026-07-14 11:04:01
- **Comment**: 29 pages, 9 figures
- **Topic Keywords**: rag
- **Reason**: This paper appears to be focused on developing a personal GUI assistant framework, addressing issues in cross-platform interaction and self-improvement. While it involves some form of 'knowing' and 'acting', the primary focus is on task automation and GUI manipulation, which seems to be unrelated to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 36. Entropy in Semantic Memory Navigation in Blind and Sighted Individuals: The Effect of Visual Experience

- **LLM Score**: 2
- **Keyword Score**: 2
- **Authors**: Felipe D. Toro-Hernández, Rodrigo Lagos, Sergio E. Chaigneau
- **URL**: <http://arxiv.org/abs/2607.12185v1>
- **Submitted**: 2026-07-13 22:05:36
- **Comment**: Cogsci paper 2026
- **Topic Keywords**: retrieval
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. The study focuses on embodied accounts of semantic memory and the effect of visual experience on navigation patterns in blind and sighted individuals, which is outside your primary areas of interest.
...
### 37. Can LLMs Write Reliable Rubrics? A Meta-Evaluation for Experiment Reproduction
...
- **Submitted**: 2026-07-14 14:52:42
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves LLMs, the focus is on rubric-based evaluation and experiment reproduction, which is not a central match to your core research themes.
...
### 38. Accelerating Masked Diffusion Large Language Models: A Survey of Efficient Inference Techniques

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Daehoon Gwak, Minhyung Lee, Junwoo Park, Jaegul Choo
- **URL**: <http://arxiv.org/abs/2607.12829v1>
- **Submitted**: 2026-07-14 14:48:06
- **Comment**: Accepted at IJCAI-ECAI 2026 (Survey Track)
- **Topic Keywords**: search
- **Reason**: This paper focuses on accelerating large language models, which is a topic in Natural Language Processing (NLP), but it does not relate to Information Retrieval (IR), query understanding, ranking models, or user behavior modeling, which are the core areas of your research interests.
...
- **Submitted**: 2026-07-14 14:02:50
- **Topic Keywords**: search
- **Reason**: This paper focuses on the development of self-improving language models, specifically addressing the issue of evaluation metrics in such systems. While it touches on the idea of 'evaluation' and 'metrics', it does not directly relate to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing, particularly in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 40. A Learning-Rate-Gated Failure of GRPO in a Small Language and Vision-Language Model Web Agent: A Controlled Null and Its Mechanism

- **LLM Score**: 2
- **Keyword Score**: 1
- **Authors**: Chengguang Gan, Zhixi Cai, Yunhao Liang, Hanjun Wei, Shiwen Ni, Qinghao Zhang
- **URL**: <http://arxiv.org/abs/2607.12640v1>
- **Submitted**: 2026-07-14 11:17:35
- **Topic Keywords**: rank
- **Reason**: This paper appears to be primarily focused on reinforcement learning and the Group Relative Policy Optimization (GRPO) algorithm, which is not directly related to the user's core research themes in Information Retrieval and Search technologies. While it does involve a web agent and some aspects of language understanding, the paper's emphasis on reinforcement learning and model optimization does not align closely with the user's interests in query understanding, ranking models, and user behavior modeling.
...
### 41. Beyond Binary Detection: A Multi-Dimensional Taxonomy of Cancer Misinformation on Reddit
...
- **URL**: <http://arxiv.org/abs/2607.12383v1>
- **Submitted**: 2026-07-14 05:55:44
- **Topic Keywords**: search
- **Reason**: This paper is not directly related to the user's core research themes in Information Retrieval, Search technologies, or Natural Language Processing. While it involves large language models and online discussions, its focus on cancer misinformation on Reddit is outside the user's primary areas of interest.
...
### 42. Evaluating Health Misinformation in Low-Resource Languages: Integrating Small Language Models with a Culturally-Sensitive Responsible NLP Framework (Bangla as a Case Study)
...
- **Submitted**: 2026-07-14 04:34:05
- **Comment**: 39 pages
- **Topic Keywords**: search
- **Reason**: This paper focuses on evaluating health misinformation in low-resource languages, specifically using Bangla as a case study. While it involves NLP and AI, the topic is not directly related to information retrieval, search technologies, or user behavior modeling, which are the user's core research interests.
...
### 43. Agentic systems for breast cancer treatment recommendations
...
- **URL**: <http://arxiv.org/abs/2607.12051v1>
- **Submitted**: 2026-07-13 18:09:50
- **Comment**: Under peer review. Source code available at: https://github.com/GRUPOMED4U/breast_cancer_agents_paper
- **Topic Keywords**: recommend
- **Reason**: This paper is not relevant to your research interests as it focuses on clinical decision support and breast cancer treatment recommendations, which is outside your primary areas of interest in Information Retrieval and Search technologies, particularly in the e-commerce domain. Although it involves large language models, the context is not aligned with your expertise in query understanding, ranking models, and user behavior modeling. The paper's focus on clinical applications and oncology treatment planning is not a central match to your research themes.
...
### 44. An Empirical Analysis of Continual Learning for Heterogeneous Medical Visual Question Answering
...
- **Submitted**: 2026-07-13 18:08:26
- **Topic Keywords**: rank
- **Reason**: This paper is not relevant to your research interests as it focuses on medical visual question answering and continual learning in a specific domain, with no apparent connection to information retrieval, search technologies, or natural language processing.
...
### 45. Self-Improvements in Modern Agentic Systems: A Survey
...
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, query understanding, ranking models, user behavior modeling, or Natural Language Processing. The focus is on self-improving autonomous agents and adaptive systems, which is outside your primary areas of interest.
