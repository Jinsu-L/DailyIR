...
- **Reason**: The paper focuses on query and document expansion using large language models, directly addressing query understanding and semantic alignment—core aspects of your IR interests. It demonstrates improvements on dense retrieval benchmarks, indicating practical relevance to ranking models and real‑time relevance optimization.

#### T.A.R.G.E.T. Summary (from Full Text)
- **Topic**: TCDE (Topic-Centric Dual Expansion) for Information Retrieval
- **Aim**: Address semantic misalignment in information retrieval through a training-free framework leveraging large language models (LLMs) to enhance query-document alignment.
- **Rationale**: Existing retrieval systems struggle with semantic gaps between queries and documents. TCDE introduces a dual expansion strategy to improve lexical and semantic alignment without requiring additional training data or models.
- **Ground**: The framework combines two components: (1) **Topic-Centric Query Expansion (TQE)**, which generates pseudo-documents for sub-topics in queries using LLMs, and (2) **Topic-Centric Document Expansion (TDE)**, which extracts core topic sentences from documents via LLMs. Both methods enrich input representations for retrieval.
- **Experiment**: Evaluated on TREC Deep Learning, BEIR, and SciFact datasets using metrics like NDCG@10, MAP, and P@10. Baselines include BM25 and multilingual-e5-base. Results show consistent improvements across sparse and dense retrieval paradigms.
- **Takeaway**: TCDE achieves significant gains (e.g., 10.2% MAP increase for BM25, 2.8% NDCG@10 gain on SciFact) while maintaining training-free operation. It demonstrates robustness across retrieval models and datasets, offering a scalable solution for semantic alignment.
...
- **Reason**: This paper explores Retrieval-Augmented Generation (RAG) systems, which is a relevant topic in Information Retrieval. The focus on long-context understanding and global semantic representation aligns with your interests in query understanding and ranking models. However, the specific application to text summarization and generation diverges slightly from your core focus on search technologies and user behavior modeling.
...
### 3. The Mental World of Large Language Models in Recommendation: A Benchmark on Association, Personalization, and Knowledgeability
...
- **Reason**: This paper explores the application of Large Language Models (LLMs) in recommendation systems, which is somewhat related to your interests in Information Retrieval and Search technologies. However, the focus on recommendation systems and the mental world of LLMs is not a central match to your primary research themes, but it does touch on related topics such as query understanding and ranking models.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: Evaluating the performance of large language models (LLMs) in recommendation systems (RecSys) using the LRWorld benchmark.
- **Aim**: To identify limitations of LLMs in RecSys and propose a structured benchmark (LRWorld) to assess their capabilities across recommendation tasks.
- **Rationale**: LLMs and RecSys operate on fundamentally different principles (language knowledge vs. personalized behavior), creating a semantic gap. A systematic evaluation framework is needed to quantify LLM performance in recommendation contexts.
- **Ground**: LRWorld is built from 38K high-quality samples and 23M tokens across public datasets, categorizing LLMs into three scales (association, personalization, knowledgeability) with ten factors and 31 measurable tasks.
- **Experiment**: Experiments on multiple LLMs revealed struggles with deep neural embeddings but strong performance in item-item associations, entity taxonomies, and multimodal reasoning. Performance variability across factors highlights model limitations in personalization and robustness.
- **Takeaway**: LLMs show promise in specific recommendation tasks but lack consistency across factors. Further research is needed to address semantic gaps, model scalability, and position bias to improve their applicability in RecSys.
...
### 4. Confidence-Credibility Aware Weighted Ensembles of Small LLMs Outperform Large LLMs in Emotion Detection
...
- **Reason**: This paper is somewhat related to your research interests in Natural Language Processing (NLP) and information retrieval, particularly in the context of emotion detection and ensemble methods. However, it does not directly address your core areas of focus, such as query understanding, ranking models, or user behavior modeling.
...
### 5. AutoMetrics: Approximate Human Judgements with Automatically Generated Evaluators
...
- **Reason**: The paper presents a framework for synthesizing evaluation metrics, which is somewhat related to information retrieval and ranking models. However, the focus is more on evaluation metrics and human feedback rather than query understanding, ranking models, or user behavior modeling, which are core areas of interest. The connection to NLP and LLM applications is relevant, but not a central match.

#### T.A.R.G.E.T. Summary (from Abstract)
- **Topic**: AutoMetrics: A Framework for Evaluating User-Facing AI Applications Under Low-Data Constraints
- **Aim**: To develop a framework that combines curated metrics and lightweight human feedback to improve evaluation accuracy of AI applications with minimal data requirements.
- **Rationale**: Existing evaluation methods for AI applications either require extensive human annotations or lack correlation with human judgment. AutoMetrics addresses this gap by leveraging low-data human feedback and interpretable metrics.
- **Ground**: The framework integrates MetricBank (48 evaluation metrics) with LLM-as-a-Judge criteria, optimized via regression to maximize alignment with human ratings.
- **Experiment**: AutoMetrics demonstrated 33.4% improvement in Kendall correlation with human ratings over LLM-as-a-Judge across 5 tasks, using fewer than 100 feedback points. It also achieved proxy reward equivalence to verifiable rewards.
- **Takeaway**: AutoMetrics provides a scalable, low-data solution for evaluating AI systems, outperforming existing methods while enabling adaptive evaluation through its open-source toolkit and MetricBank.
...
### 6. When F1 Fails: Granularity-Aware Evaluation for Dialogue Topic Segmentation
...
- **Reason**: This paper is somewhat related to the user's interests in Information Retrieval, specifically in the context of dialogue topic segmentation and conversational systems. However, it focuses more on evaluation metrics and annotation granularity, which, while relevant to IR, is not a central match to the user's core research themes.
...
- **Reason**: This paper explores search engine behavior in Wikipedia and Grokipedia, which is somewhat related to information retrieval and search technologies. However, the focus on encyclopedic platforms and unexpected search outcomes is not directly aligned with the user's core research themes, particularly query understanding, ranking models, and user behavior modeling.
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval, particularly in the context of marketing and product personalisation. However, the focus on agentic messaging and its behavioural effects in a financial service application is not a central match to your core research themes. The paper touches on some IR methods, but the application and scope are limited.
...
### 9. A Solver-in-the-Loop Framework for Improving LLMs on Answer Set Programming for Logic Puzzle Solving
...
- **Reason**: The paper is somewhat related to the user's interests in Natural Language Processing (NLP) and deep semantic understanding, but it primarily focuses on Large Language Models (LLMs) and code generation for domain-specific languages, which is not a central match for the user's core research themes in Information Retrieval and Search technologies.
...
### 10. Knowledge Distillation with Structured Chain-of-Thought for Text-to-SQL
...
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, particularly in the area of query understanding and text-to-SQL systems. However, the focus on knowledge distillation and structured chain-of-thought for text-to-SQL is not directly aligned with your primary interests in search technologies, ranking models, and user behavior modeling.
...
### 11. CIFE: Code Instruction-Following Evaluation
...
- **Topic Keywords**: rag
- **Reason**: This paper is somewhat related to your research interests in Information Retrieval and Natural Language Processing, as it involves large language models and code generation. However, the focus on code instruction-following evaluation and developer intent is not directly aligned with your core themes of query understanding, ranking models, and user behavior modeling.
...
- **Topic Keywords**: retrieval
- **Reason**: While the paper presents a unified AI framework for mapping actors, topics, and arguments in public debates, it primarily focuses on media intelligence and stakeholder analysis, which is somewhat related to information retrieval and NLP. However, the paper's emphasis on descriptive analytics and argument extraction does not directly align with the user's core research themes in query understanding, ranking models, and user behavior modeling. The paper's domain is also more focused on public debates and media intelligence rather than e-commerce or real-time relevance optimization.
...
- **Reason**: The paper focuses on recommender systems, which is a related area to information retrieval, but it's more centered on the e-commerce domain and doesn't directly address query understanding, ranking models, or user behavior modeling. While it does involve deep semantic understanding and real-time relevance optimization, the context is more aligned with recommender systems than traditional information retrieval.
...
### 14. DEER: A Comprehensive and Reliable Benchmark for Deep-Research Expert Reports
...
- **Reason**: The paper introduces a benchmark for evaluating expert-level deep research reports, which is somewhat related to information retrieval and search technologies. However, the focus is on evaluating the quality of reports generated by large language models, rather than query understanding, ranking models, or user behavior modeling. While it touches on aspects of semantic understanding, it is not directly aligned with the user's core research themes.
...
- **Topic Keywords**: search
- **Reason**: The paper focuses on NLP, which is a related field to your research interests. However, it primarily deals with question generation and answer evaluation, which is more aligned with NLP and education rather than information retrieval and search technologies. While it touches on the application of NLP, it doesn't directly address your core research themes.
...
### 16. Diversity Recommendation via Causal Deconfounding of Co-purchase Relations and Counterfactual Exposure
...
- **Reason**: This paper focuses on recommender systems, specifically addressing diversity in recommendations through causal deconfounding of co-purchase relations and counterfactual exposure. While it touches on item relationships and user behavior, it does not directly relate to the user's core research themes in Information Retrieval, query understanding, ranking models, or real-time relevance optimization.
...
### 17. Enhancing Long Document Long Form Summarisation with Self-Planning
...
- **Reason**: The paper focuses on long document summarization, which is not a core area of interest in Information Retrieval or Search technologies. While it involves Natural Language Processing, the primary focus is on summarization rather than query understanding, ranking models, or user behavior modeling.
...
### 18. Data Augmentation Supporting a Conversational Agent Designed for Smoking Cessation Support Groups
...
- **Topic Keywords**: relevance, rag
- **Reason**: While this paper touches on Natural Language Processing (NLP) and conversational agents, it does not appear to be directly related to Information Retrieval (IR), query understanding, ranking models, or user behavior modeling, which are core areas of your research interests.
...
- **Reason**: This paper focuses on Medical Entity Recognition (MedER) in the Bangla language, which is not directly related to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves NLP, the specific task and language are not aligned with the user's core research themes.
...
- **Reason**: This paper focuses on collaborative filtering and partition-aware similarity refinement, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on recommender systems, it's more about scalability and reproducibility in collaborative filtering, rather than deep semantic understanding and real-time relevance optimization.
...
- **Topic Keywords**: ctr
- **Reason**: This paper focuses on translation of user-generated content, which is related to Natural Language Processing (NLP), but it does not directly align with the user's core research themes in Information Retrieval (IR), query understanding, ranking models, or user behavior modeling.
...
- **Topic Keywords**: rag
- **Reason**: This paper is not directly related to your core research themes in Information Retrieval, Search technologies, or Natural Language Processing, as it focuses on automatic speech recognition and named entity recognition. While it involves attention-based models and prediction, the context and application are distinct from your areas of interest.
...
- **Topic Keywords**: acl
- **Reason**: This paper is not directly related to your research interests in Information Retrieval, Search technologies, or Natural Language Processing. While it involves Large Language Models, the focus is on software engineering and code generation, which is outside your primary areas of interest.
...
### 25. UCoder: Unsupervised Code Generation by Internal Probing of Large Language Models
...
- **Topic Keywords**: rag
- **Reason**: This paper focuses on unsupervised code generation using internal probing of large language models, which is not directly related to information retrieval, search technologies, or user behavior modeling. While it touches on NLP, it's more specific to code generation and doesn't align with the user's primary research interests in deep semantic understanding and real-time relevance optimization.
...
### 26. AdvJudge-Zero: Binary Decision Flips in LLM-as-a-Judge via Adversarial Control Tokens
...
- **Reason**: This paper appears to be primarily focused on the security and robustness of Large Language Models (LLMs) in post-training pipelines, which is not directly related to the user's core research themes in Information Retrieval, Search technologies, and Natural Language Processing. While it touches on the use of LLMs, the context and methodology are centered around adversarial control tokens and reward hacking, which is not a central match for the user's interests.
...
### 27. Task Schema and Binding: A Double Dissociation Study of In-Context Learning
...
- **Topic Keywords**: retrieval
- **Reason**: This paper is not relevant to your research interests in Information Retrieval, Search technologies, and Natural Language Processing. The study focuses on in-context learning mechanisms in neural networks, which is a topic in the broader field of Artificial Intelligence and Machine Learning, but not directly related to your core research themes.
...
### 28. Perturb Your Data: Paraphrase-Guided Training Data Watermarking
...
- **Topic Keywords**: ctr
- **Reason**: This paper focuses on training data watermarking for copyright enforcement, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing. While it involves Large Language Models, the context is more about data ownership and licensing rather than query understanding, ranking models, or user behavior modeling.
...
### 29. Intelligent Knowledge Mining Framework: Bridging AI Analysis and Trustworthy Preservation
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be more focused on knowledge mining and preservation, rather than information retrieval or search technologies. While it does involve data processing and transformation, the primary goal is to create a framework for trustworthy preservation, which is not directly related to the user's core research themes.
...
### 30. AncientBench: Towards Comprehensive Evaluation on Excavated and Transmitted Chinese Corpora
...
- **Topic Keywords**: search
- **Reason**: This paper appears to be focused on the development of a benchmark for evaluating the comprehension of ancient Chinese characters by large language models. While it touches on the topic of language understanding, it is primarily concerned with the evaluation of language models in the context of archaeology and ancient Chinese language, which is not directly related to the user's core research themes in Information Retrieval and Search technologies.
...
### 31. Affect, Body, Cognition, Demographics, and Emotion: The ABCDE of Text Features for Computational Affective Science
...
- **Topic Keywords**: search
- **Reason**: This paper focuses on computational affective science and the creation of a large-scale dataset for text analysis, which is not directly related to the user's core research interests in Information Retrieval, Search technologies, and Natural Language Processing.
...
### 32. Simulstream: Open-Source Toolkit for Evaluation and Demonstration of Streaming Speech-to-Text Translation Systems
...
- **Topic Keywords**: search
- **Reason**: This paper is not relevant to your research interests as it focuses on Speech-to-Text Translation Systems, which is outside your primary areas of Information Retrieval and Search technologies. Although it involves deep semantic understanding, the context is not related to your core themes of query understanding, ranking models, or user behavior modeling.
...
### 33. A Systematic Reproducibility Study of BSARec for Sequential Recommendation
...
- **Reason**: This paper focuses on sequential recommendation, which is related to information retrieval, but it primarily deals with recommender systems and signal processing techniques. While it involves some aspects of deep semantic understanding, the context is not directly related to the user's core research themes in information retrieval and search technologies.
...
### 34. Governance-Aware Hybrid Fine-Tuning for Multilingual Large Language Models
...
- **Topic Keywords**: rank
- **Reason**: This paper focuses on large language models, multilingual adaptation, and data governance, which are not directly related to the user's primary research interests in Information Retrieval, Search technologies, and Natural Language Processing, particularly in areas requiring deep semantic understanding and real-time relevance optimization.
...
### 35. XLM: A Python package for non-autoregressive language models
...
- **Topic Keywords**: search
- **Reason**: This paper is about a Python package for non-autoregressive language models, which is not directly related to Information Retrieval, Search technologies, or query understanding. While it touches on Natural Language Processing, it's more focused on model implementation rather than deep semantic understanding or real-time relevance optimization.
...
### 36. Seed-Prover 1.5: Mastering Undergraduate-Level Theorem Proving via Learning from Experience
...
- **Comment**: 21 pages
- **Topic Keywords**: rag
- **Reason**: This paper focuses on formal theorem proving using large language models, which is outside the scope of information retrieval and search technologies. Although it involves natural language processing, the context is specific to formal mathematical reasoning and does not align with the user's primary research interests.
